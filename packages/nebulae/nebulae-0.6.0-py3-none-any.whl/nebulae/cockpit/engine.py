...
class Engine(object):
    ...
    def coat(self, datum, as_const=True, async=False):
        ...
        if self.device == GPU:
            if self.rank<0:
                return datum.cuda(non_blocking=async)
            else:
                return datum.to(self.chip[self.rank], non_blocking=async )
        elif self.device == CPU:
            return datum.cpu()
        else:
            raise KeyError('NEBULAE ERROR ⨷ given device should be either cpu or gpu.')

    def shell(self, datum, as_np=True, async=True):
        datum = datum.detach()
        # add a sync point
        if not async and self.device == GPU:
            if self.rank < 0:
