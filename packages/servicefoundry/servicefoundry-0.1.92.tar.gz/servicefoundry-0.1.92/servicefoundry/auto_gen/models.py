...
#   timestamp: 2022-09-09T09:32:44+00:00
...
class Autoscaling(BaseModel):
    min_replicas: int = Field(
        1, description="+usage=Minimum number of replicas to keep available"
    )
    max_replicas: conint(gt=1) = Field(
        ..., description="+usage=Maximum number of replicas allowed for the component."
    )
    cpu_utilization_percentage: conint(lt=100)


class DefaultResources(BaseModel):
    cpu_request: Literal["0.2"]
    cpu_limit: Literal["0.5"]
    memory_request: Literal["200"]
    memory_limit: Literal["500"]
...
class Job(BaseModel):
    ...
    trigger: Union[Manual, Schedule] = Field(
        {"type": "manual", "run": True}, description="+sort=3"
    )
