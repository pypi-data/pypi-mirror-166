...
@click.command(name="application", cls=COMMAND_CLS, help="List Applications")
@click.option(
    "-w", "--workspace", type=click.STRING, default=None, help="workspace fqn"
)
@handle_exception_wrapper
def list_application(workspace):
    applications = application_lib.list_application(
        workspace_fqn=workspace,
    )
    if not applications:
        console.print(PROMPT_NO_APPLICATIONS)
    else:
        applications.sort(key=lambda s: (s.fqn))

    applications = [s.to_dict() for s in applications]
    if CliConfig.get("json"):
...
def get_list_command():
    ...
    list_command.add_command(list_application)
