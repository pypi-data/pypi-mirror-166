...
# === Taken from compose begin ===
# Taken from https://github.com/docker/compose under Apache-2.0 license
# https://github.com/docker/compose/blob/1.28.x/LICENSE
...
# === Taken from compose end ===


def update_required_limit(resource):
    if resource.required is None and resource.limit is not None:
        resource.required = resource.limit
    elif resource.required is not None and resource.limit is None:
        resource.limit = resource.required * 2
    elif resource.required is None and resource.limit is None:
        raise ValueError("Both `required` and `limit` cannot be None")
