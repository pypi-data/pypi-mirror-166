...
class AcFun(VideoExtractor):
    ...
    def download(self, **kwargs):
        if 'json_output' in kwargs and kwargs['json_output']:
            ...
            download_url_ffmpeg(url, self.title, ext,
                                output_dir=kwargs['output_dir'], merge=kwargs['merge'], stream=False)
    ...
    def acfun_download(self, url, output_dir='.', merge=True, info_only=False, **kwargs):
        ...
        if not info_only:
            download_url_ffmpeg(m3u8_url, title, 'mp4', output_dir=output_dir, merge=merge, stream=False)
