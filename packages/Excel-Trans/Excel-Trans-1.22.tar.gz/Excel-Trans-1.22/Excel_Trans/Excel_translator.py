    ...
    for sheet in SHEETS:
        ...
        for title in TITLES:
            ...
            if title[0:7] == 'Unnamed':
                ...
                TRANS_TITLE = to_cyrillic(title)
            DataList = []
            for data in DATA:
                if str(data) == 'nan':
                    DataList.append(" ")
                else:
                    Data = to_cyrillic(data)
            ...
            for title in TITLES:
                ...
                for data in DATA:
                    if str(data) == 'nan':
                        ...
                        Data = GoogleTranslator(source='auto', target=target).translate(data)
                        DataList.append(Data)
                TRANS_DATA[TRANS_TITLE] = DataList
            save_xlsx = pd.DataFrame(TRANS_DATA)
            with pd.ExcelWriter(save_file, mode='a', if_sheet_exists="replace") as writer:
