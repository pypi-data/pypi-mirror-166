...
async def loop(
    ...
    tag_to_comments = _populate_comments(first_run)
    ...
    while count_wo_change < MAX_RERUNS_WO_CHANGE:
        ...
        tag_to_comments = _populate_comments(current_run, tag_to_comments)
...
def _populate_comments(
    run, tag_to_comments: Dict[str, tuple] = None
) -> Dict[str, tuple]:
    ...
    for tag in run:
        if run[tag].get("comment"):
            if tag_to_comments.get(tag) and not run[tag].get("comment")[
                0
            ] in tag_to_comments.get(tag):
                tag_to_comments[tag] += run[tag].get("comment")
            else:
                tag_to_comments[tag] = run[tag].get("comment")
