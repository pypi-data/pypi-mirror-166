...
# ]
default_colors = ['r','g','b','c','gold','grey']

def draw_ellipse(fig, ax, x, y, w, h, a, fillcolor,alpha):
    e = patches.Ellipse(
        xy=(x, y),
        width=w,
        height=h,
...
##----------------------------------------------------
def venn(data, names, fig, ax, set_colors, matVenn = True, legend=False, **options):
    """
    data: a list
    names: names of groups in data
    matVenn: import matplotlib_venn.venn2, venn3 if True else draw_circle
    fill = ["number"|"logic"|"both"], fill with number, logic label, or both
    legend = [True|False]
   
    """
    real_ind = np.where(np.array([len(data[i]) for i in range(len(data))])>0)[0]
    data = list(np.array(data)[real_ind])
    names = list(np.array(names)[real_ind])
    set_colors = list(np.array(set_colors)[real_ind])
    fontsize = options.get('fontsize', 14)
    alpha = options.get('alpha', 0.5)
    
    if len(data) == 0:
        black(data, names, fig, ax, set_colors, legend=False, **options)
    elif len(data) == 1:
        circle(data, names, fig, ax, set_colors, legend=False, **options)
    elif len(data) == 2:
        if matVenn:
            matplotlib_venn.venn2(subsets = data, set_labels = names, set_colors=set_colors, 
            ax = ax, alpha =alpha)
        else:
            venn2(data, names, fig, ax, set_colors, legend=False, **options)
    elif len(data) == 3:
        if matVenn:
            matplotlib_venn.venn3(subsets = data, set_labels = names, set_colors=set_colors, 
            ax = ax,alpha =alpha)
        else:
            venn3(data, names, fig, ax,set_colors, legend=False, **options)
    elif len(data) == 4:
        venn4(data, names, fig, ax, set_colors,legend=False, **options)
    elif len(data) == 5:
        venn5(data, names, fig, ax, set_colors,legend=False, **options)
    elif len(data) == 6:
        venn6(data, names, fig, ax, set_colors,legend=False, **options)
    else:
        raise Exception("currently only 2-6 sets venn diagrams are supported")
## ------------------------------------------------------------------------
def black(data, names, fig, ax, set_colors, legend=False, **options):
    # fontsize = options.get('fontsize', 14)
    # alpha = options.get('alpha', 0.5)
    ax.set_axis_off()

def circle(data, names, fig, ax, set_colors, legend=False, **options):
    fontsize = options.get('fontsize', 14)
    alpha = options.get('alpha', 0.5)
    c1 = plt.Circle((0.5,0.5), radius=0.2, alpha=alpha, color=set_colors[0])
    ax.add_artist(c1)
    ax.set_aspect("equal")
    ax.set_axis_off()
    draw_text(fig, ax, 0.5, 0.5, len(data[0]), fontsize=fontsize)
    draw_text(fig,ax,0.6,0.75,names[0], set_colors[0], fontsize=fontsize, ha="right", va="bottom")

def venn2(data, names, fig, ax, set_colors, legend=False, **options):
    """
    ...
    input
      data: data to get label for
      names: a label dict where keys are identified via binary codes ('01', '10', '11'),
    ...
    colors= set_colors
    # colors = options.get('set_colors', [default_colors[i] for i in range(2)])
    ...
    # body
    draw_ellipse(fig, ax, 0.375, 0.3, 0.5, 0.5, 0.0, colors[0],alpha=alpha)
    draw_ellipse(fig, ax, 0.625, 0.3, 0.5, 0.5, 0.0, colors[1],alpha=alpha)
...
def venn3(data, names, fig, ax, set_colors, legend=False, **options):
    """
    ...
    labels = get_labels(data, fill=['number'])
    colors = set_colors
    # colors = options.get('set_colors', [default_colors[i] for i in range(3)])
    ...
    # body
    draw_ellipse(fig, ax, 0.333, 0.633, 0.5, 0.5, 0.0, colors[0],alpha=alpha)
    draw_ellipse(fig, ax, 0.666, 0.633, 0.5, 0.5, 0.0, colors[1],alpha=alpha)
    draw_ellipse(fig, ax, 0.500, 0.310, 0.5, 0.5, 0.0, colors[2],alpha=alpha)
...
def venn4(data, names, fig, ax,set_colors, legend=False, **options):
    """
    ...
    labels = get_labels(data, fill=['number'])
    colors = set_colors
    # colors = options.get('set_colors', [default_colors[i] for i in range(4)])
    ...
    # body
    draw_ellipse(fig, ax, 0.350, 0.400, 0.72, 0.45, 140.0, colors[0],alpha=alpha)
    draw_ellipse(fig, ax, 0.450, 0.500, 0.72, 0.45, 140.0, colors[1],alpha=alpha)
    draw_ellipse(fig, ax, 0.544, 0.500, 0.72, 0.45, 40.0, colors[2],alpha=alpha)
    draw_ellipse(fig, ax, 0.644, 0.400, 0.72, 0.45, 40.0, colors[3],alpha=alpha)
...
def venn5(data, names, fig, ax, set_colors,legend=False, **options):
    """
    ...
    labels = get_labels(data, fill=['number'])
    colors= set_colors
    # colors = options.get('set_colors', [default_colors[i] for i in range(5)])
    ...
    # body
    draw_ellipse(fig, ax, 0.428, 0.449, 0.87, 0.50, 155.0, colors[0],alpha=alpha)
    draw_ellipse(fig, ax, 0.469, 0.543, 0.87, 0.50, 82.0, colors[1],alpha=alpha)
    draw_ellipse(fig, ax, 0.558, 0.523, 0.87, 0.50, 10.0, colors[2],alpha=alpha)
    draw_ellipse(fig, ax, 0.578, 0.432, 0.87, 0.50, 118.0, colors[3],alpha=alpha)
    draw_ellipse(fig, ax, 0.489, 0.383, 0.87, 0.50, 46.0, colors[4],alpha=alpha)
...
def venn6(data, names, fig, ax, set_colors,legend=False, **options):
    """
    ...
    labels = get_labels(data, fill=['number'])
    colors = set_colors
    # colors = options.get('set_colors', [default_colors[i] for i in range(6)])
    ...
    draw_triangle(fig, ax, 0.637, 0.921, 0.649, 0.274, 0.188, 0.667, colors[0], alpha=alpha)
    draw_triangle(fig, ax, 0.981, 0.769, 0.335, 0.191, 0.393, 0.671, colors[1], alpha=alpha)
    draw_triangle(fig, ax, 0.941, 0.397, 0.292, 0.475, 0.456, 0.747, colors[2], alpha=alpha)
    draw_triangle(fig, ax, 0.662, 0.119, 0.316, 0.548, 0.662, 0.700, colors[3], alpha=alpha)
    draw_triangle(fig, ax, 0.309, 0.081, 0.374, 0.718, 0.681, 0.488, colors[4], alpha=alpha)
    draw_triangle(fig, ax, 0.016, 0.626, 0.726, 0.687, 0.522, 0.327, colors[5], alpha=alpha)
