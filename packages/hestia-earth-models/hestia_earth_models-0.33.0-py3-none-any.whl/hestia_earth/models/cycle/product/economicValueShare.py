...
def _run_by_default(cycle: dict, products: list):
    ...
    debugValues(cycle, model=MODEL, key=MODEL_KEY, by='incomplete',
...
def _should_run_default(cycle: dict, products: list):
    run_by = 'incomplete'
    missing_products = [p for p in products if p.get(MODEL_KEY) is None]
    has_missing_evs = len(missing_products) > 0

    logRequirements(cycle, model=MODEL, key=MODEL_KEY, by=run_by,
                    has_missing_evs=has_missing_evs)

    for p in missing_products:
        logShouldRun(cycle, MODEL, p.get('term', {}).get('@id'), True, key=MODEL_KEY, by=run_by)

    should_run = all([has_missing_evs])
    logShouldRun(cycle, MODEL, None, should_run, key=MODEL_KEY, by=run_by)
    return should_run
...
def run(cycle: dict):
    ...
    return (
        ...
        _run_by_default(cycle, products) if _should_run_default(cycle, products) else []
    )
