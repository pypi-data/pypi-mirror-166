...
def _should_run_emission(impact_assessment: dict):
    ...
    def exec(emission: dict):
        ...
        has_value = convert_value_from_cycle(product, list_sum(emission.get('value', [0]))) is not None
        not_deleted = emission.get('deleted', False) is not True

        logRequirements(impact_assessment, model=MODEL, term=term_id, key=MODEL_KEY,
                        has_value=has_value,
