...
def getContainerItems(graph, node, predicate):
    return [s[2] for s in xmptools.getContainerStatements(graph, node, predicate)]

# Borrowed, with modifications, from xmptools (for now)
def setContainerItems(graph, node, predicate, values, newtype=RDF.Seq):
    ...
    if values:
        statements = xmptools.getContainerStatements(graph, node, predicate)
        if statements:
        ...
        i = 1
        for value in values:
            graph.add(container, URIRef(xmptools.makeContainerItemPredicate(i)), value)
            i += 1
    else:
