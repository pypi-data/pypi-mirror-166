...
from bigt.pprint import pprint, assert_issubset


def test_assert_issubset():
    a = {"a":[{'z': {'a': '21', 'b': False, 'd': '20'}}, {'z': {'a': '1', 'b': False, 'd': '2'}}]}
    b = {"a":[{'z': {'a': '1', 'b': False, 'd': {"a": '20'}, "y": {"a": 2}}}]}
    pprint(a, b, weights={"a": {'z': {"a": 2}}})
    try:
        assert_issubset(b, a, weights={"a": {'z': {"a": 2}}})
    except:
        pass
...
def test_weights_2():
    a = {"a":[{'z': {'a': '21', 'b': False, 'd': '20'}}, {'z': {'a': '1', 'b': False, 'd': '2'}}]}
    b = {"a":[{'z': {'a': '1', 'b': False, 'd': {"a": '20'}, "y": {"a": 2}}}]}
    result, not_present, changed = DictExt(a).issubset(b, weights={"a": {'z': {"a": 2}}})
    assert not result
    assert not_present == {"a":[{'z': {'a': '21', 'b': False, 'd': '20'}}]}
    assert changed == {"a":[{'z': {'d': {"old": "2", "new": {"a": "20"}}}}]}
    
def test_diff_types():
    a = {"a":[{'z': {'a': '21', 'b': False, 'd': '20'}}, {'z': {'a': '1', 'b': False, 'd': '2'}}]}
    b = {"a":[{'z': {'a': '1', 'b': False, 'd': {"a": '20'}, "y": 123}}]}
    result, not_present, changed = DictExt(b).issubset(a, weights={"a": {'z': {"a": 2, "y": {"v": 4}}}})
    assert not result
    assert not_present == {'a': [{'z': {'y': 123}}]}
    assert changed == {"a":[{'z': {'d': {"old": {"a": '20'}, "new": "2"}}}]}

def test_weights():
    a = {"a":[{'z': {'a': '21', 'b': False, 'd': '20'}}, {'z': {'a': '1', 'b': False, 'd': '2'}}]}
    b = {"a":[{'z': {'a': '1', 'b': False, 'd': '20', "y": 123}}]}
    result, not_present, changed = DictExt(b).issubset(a, weights={"a": {'z': {"a": 2, "y": {"v": 4}}}})
    assert not result
    assert not_present == {'a': [{'z': {'y': 123}}]}
