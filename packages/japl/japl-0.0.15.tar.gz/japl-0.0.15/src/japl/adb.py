...
class DeltaManager:
    ...
    def mount(self, 
        ...
        def __mount(mount_name: str):
            ...
            """
            if not f"{mount_point}{mount_name}{postfix}" in list(map(lambda mount: mount.mountPoint, spark.dbutils.fs.mounts())):
                spark.dbutils.fs.mount(
    ...
    def delete(self, 
        ...
        if table_name in [table.name for table in spark.catalog.listTables()]:
            ...
            spark.dbutils.fs.rm(f'{path}/{table_name}', recurse=True)
