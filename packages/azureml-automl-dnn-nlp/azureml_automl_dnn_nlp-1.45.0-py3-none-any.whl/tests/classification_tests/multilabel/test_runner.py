...
class MultilabelRunnerTests(unittest.TestCase):
    ...
    def test_runner(
        ...
        self.assertEquals(trainer_mock.call_args[1]['resource_path_resolver'].dataset_language, "eng")
        self.assertEquals(trainer_mock.call_args[1]['resource_path_resolver'].model_name, ModelNames.BERT_BASE_UNCASED)
        assert np.array_equal(trainer_mock.call_args[1]['train_label_list'],
                              np.array(["['label', 'A']", "['LabelA', '1', '2']", '[]']))
    ...
    def test_runner_mltable_data_json(
        ...
        self.assertTrue(load_and_validate_multilabel_dataset_mock.call_args[0][5] == mltable_data_json)
