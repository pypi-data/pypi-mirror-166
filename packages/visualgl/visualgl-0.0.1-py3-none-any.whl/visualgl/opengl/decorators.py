...
def flatten(
    iterable: Iterable, iterable_size: int, element_size: int, acceptable_types: Iterable
) -> Iterable[Number]:
    if isinstance(iterable, acceptable_types) or (
        isinstance(iterable, (list, tuple)) and isinstance(iterable[0], Number)
    ):
        iterable = [iterable]
