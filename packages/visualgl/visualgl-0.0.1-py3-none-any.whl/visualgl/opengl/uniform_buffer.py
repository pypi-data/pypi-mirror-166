...
alignments = {float: 4, int: 4, Matrix4: 16, Transform: 16, Vector3: 16}

sizes = {float: 4, int: 4, Matrix4: 64, Transform: 64, Vector3: 12}
...
class UniformBuffer:
    ...
    def fields(self, mapping: Mapping):
        ...
        for field in mapping.fields:
            ...
            fields.append(
                Field(underlying_type, alignments[underlying_type], sizes[underlying_type])
            )

            assert (
                fields[-1].size_in_bytes % 4 == 0
            ), "Field size must be divisible by 4 since we're only padding with 4-byte floats"
