...
class dUsrType:
    ...
    async def lock_one_by_key(self, _key: tUsrTypeKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        async with dUsrTypeLockOne(self._dbcon) as _qry:
            ...
            while _dex >= 0:
                try:
                    ...
                    if not await _qry.fetch():
                        ...
                        raise mettle.lib.xMettle(f"Lock not found: [table:dUsrType, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
...
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_by_key(self, _key: tUsrTypeKey) -> bool:
        ...
        """
        async with dUsrTypeSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def select_one(self) -> "dUsrType":
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dUsrType, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_deft(self,
        ...
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dUsrType, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_by_key(self, _key: tUsrTypeKey) -> "dUsrType":
        ...
        """
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dUsrType, row:{self.irec}]", xMettle.eCode.DBNotFound)
    ...
    async def update(self, irec: tUsrType = None):
        ...
        async with dUsrTypeUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tUsrType = None):
        ...
        async with dUsrTypeInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
    ...
    async def delete_one(self, irec: tUsrTypeKey = None):
        ...
        async with dUsrTypeDeleteOne(self._dbcon) as _qry:
            await _qry.exec(_key)
