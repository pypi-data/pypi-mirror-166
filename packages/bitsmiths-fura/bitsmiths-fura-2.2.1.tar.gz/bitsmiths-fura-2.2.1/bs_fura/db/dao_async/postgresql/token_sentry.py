...
class dTokenSentry:
    ...
    async def try_select_one_by_key(self, _key: tTokenSentryKey) -> bool:
        ...
        """
        async with dTokenSentrySelectOne(self._dbcon) as _qry:
    ...
    async def select_one(self) -> "dTokenSentry":
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dTokenSentry, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_deft(self,
        ...
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dTokenSentry, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_by_key(self, _key: tTokenSentryKey) -> "dTokenSentry":
        ...
        """
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dTokenSentry, row:{self.irec}]", xMettle.eCode.DBNotFound)
    ...
    async def update(self, irec: tTokenSentry = None):
        ...
        async with dTokenSentryUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tTokenSentry = None):
        ...
        async with dTokenSentryInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
