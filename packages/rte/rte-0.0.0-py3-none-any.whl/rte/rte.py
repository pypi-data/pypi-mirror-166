...
class rte:
    ...
    def __init__(self, p_project_name, p_dir, p_parent_log_name='', p_verbose=True):
        ...
        if p_parent_log_name:
            self.log_name = '{}.{}'.format(p_parent_log_name, rte)
...
def do_examples(p_cls=True):
    '''A collection of implementation examples for rte.

    A collection of implementation examples for rte. The examples
...
def do_example1(p_cls=True):
    '''A working example of the implementation of rte.
    ...
    t_rte = rte("rte", t_dir)
    t_rte.method_1('This is do_example1')
...
def do_example2(p_cls=True):
    '''Another working example of the implementation of rte.
    ...
    t_dir = beeutils.get_tmp_dir()
    t_rte = rte("rte", t_dir)
    t_rte.method_1('This is do_example2')
    beeutils.rm_tree(t_dir)
    archiver.print_footer()
    return success
