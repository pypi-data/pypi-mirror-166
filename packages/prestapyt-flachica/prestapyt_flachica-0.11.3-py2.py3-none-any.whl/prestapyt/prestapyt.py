...
class PrestaShopWebService(object):
    ...
    def __init__(self, api_url, api_key, debug=False, session=None,
                 verbose=False, custome_headers=None):
        """
        ...
        if session is None:
            ...
            self.client.headers.update(custome_headers)
        else:
            self.client = session

        if not self.client.auth:
            self.client.auth = (api_key, '')
        self.custome_headers = custome_headers
    ...
    def _execute(self, url, method, data=None, add_headers=None):
        ...
        if self.custome_headers:
            request_headers.update(self.custome_headers)
