...
from SigProfilerTopographyCombiner.source.Combined_Common import COSMIC_NUCLEOSOME_OCCUPANCY
from SigProfilerTopographyCombiner.source.Combined_Common import COSMIC_CTCF_OCCUPANCY
from SigProfilerTopographyCombiner.source.Combined_Common import COSMIC_OCCUPANCY
from SigProfilerTopographyCombiner.source.Combined_Common import COSMIC_REPLICATION_TIME
from SigProfilerTopographyCombiner.source.Combined_Common import COSMIC_PROCESSIVITY
...
from SigProfilerTopographyCombiner.source.Combined_Common import TABLE_SBS_SIGNATURE_CUTOFF_NUMBEROFMUTATIONS_AVERAGEPROBABILITY_FILE
from SigProfilerTopographyCombiner.source.Combined_Common import TABLE_DBS_SIGNATURE_CUTOFF_NUMBEROFMUTATIONS_AVERAGEPROBABILITY_FILE
from SigProfilerTopographyCombiner.source.Combined_Common import TABLE_ID_SIGNATURE_CUTOFF_NUMBEROFMUTATIONS_AVERAGEPROBABILITY_FILE
from SigProfilerTopographyCombiner.source.Combined_Common import TABLE_MUTATIONTYPE_NUMBEROFMUTATIONS_NUMBEROFSAMPLES_SAMPLESLIST
from SigProfilerTopographyCombiner.source.Combined_Common import get_number_of_mutations_filename
from SigProfilerTopographyCombiner.source.Combined_Common import SBS
from SigProfilerTopographyCombiner.source.Combined_Common import DBS
from SigProfilerTopographyCombiner.source.Combined_Common import ID
from SigProfilerTopographyCombiner.source.Combined_Common import SUBS
from SigProfilerTopographyCombiner.source.Combined_Common import DINUCS
from SigProfilerTopographyCombiner.source.Combined_Common import INDELS
from SigProfilerTopographyCombiner.source.Combined_Common import AGGREGATEDDINUCS
from SigProfilerTopographyCombiner.source.Combined_Common import AGGREGATEDSUBSTITUTIONS
from SigProfilerTopographyCombiner.source.Combined_Common import AGGREGATEDINDELS
                ...
                if ((signature_cancer_type_occupancy_average_signal_array is not None) and (np.any(signature_cancer_type_occupancy_average_signal_array))):
                    ...
                    if is_eligible(fold_change,
                        ...
                        if (spearman_corr and (spearman_corr >= pearson_spearman_correlation_cutoff)):
                            ...
                            spearman_element_names.append((signature, cancer_type, average_signal_array_file_name)) # cancer_type

                        if (pearson_corr and (pearson_corr >= pearson_spearman_correlation_cutoff)):
                            pearson_p_values.append(pearson_p_value)
                            pearson_element_names.append((signature, cancer_type, average_signal_array_file_name)) # cancer_type
...
def accumulate_replication_time_across_all_cancer_types_plot_figure(plot_output_dir,
        ...
        if number_of_mutations >= number_of_mutations_required:
            ...
            if (os.path.exists(typebased_tissuebased_replication_time_number_of_mutations_filepath) and
                ...
                plot_replication_time_figure(plot_output_dir,
                                            ...
                                            cosmic_release_version,
                                            figure_file_extension,
                                            cancer_type = cancer_type, # formerly it was cancer_type
                                            num_of_increasing = 0,
                                            num_of_flat = 0,
                                            num_of_decreasing = 0,
                                            cosmic_legend = cosmic_legend,
            ...
            if (os.path.exists(typebased_tissuebased_replication_time_normalized_mutation_densisty_filepath)):
                ...
                if (np.any(signature_cancer_type_normalized_mutation_density_array)) and (np.any(across_all_cancer_types_real_normalized_mutation_density_array)):
                    ...
                    if cancer_type_m > replication_time_slope_cutoff and monotonic(cancer_type_y):
                        ...
                        tissues_with_slope_increasing.append(cancer_type) # cancer_type
                        decision = INCREASING
                    ...
                    # Negative Slope and abs(slope) greater than cutoff
                        ...
                        tissues_with_slope_decreasing.append(cancer_type) #cancer_type
                        decision = DECREASING
                        replication_timing_diff_btw_max_and_min = (np.max(cancer_type_y) - np.min(cancer_type_y))
                        abs_replication_timing_diff_btw_medians = abs(np.median(cancer_type_y[0:3]) - np.median(cancer_type_y[7:]))
                    # Case4
                    ...
                    # Slope less than cutoff
                        ...
                        tissues_with_slope_flat.append(cancer_type) # cancer_type
                        decision = FLAT
                        replication_timing_diff_btw_max_and_min = (np.max(cancer_type_y) - np.min(cancer_type_y))
                        abs_replication_timing_diff_btw_medians = abs(np.median(cancer_type_y[0:3]) - np.median(cancer_type_y[7:]))
                    # Case6
                    ...
                    if (spearman_corr >= pearson_spearman_correlation_cutoff):
                        ...
                        spearman_p_values.append(spearman_p_value)
                        spearman_element_names.append((signature, cancer_type)) # cancer_type

                    if (pearson_corr >= pearson_spearman_correlation_cutoff):
                        num_of_tissues_with_pearson_corr_ge_cutoff += 1
                        pearson_p_values.append(pearson_p_value)
                        pearson_element_names.append((signature, cancer_type)) # cancer_type

                    # Cancer type
                    if signature_cancer_type_replication_time_df[(signature_cancer_type_replication_time_df['signature'] == signature) &
                                                              (signature_cancer_type_replication_time_df['cancer_type'] == cancer_type)].values.any(): # cancer_type

                        signature_cancer_type_replication_time_df.loc[((signature_cancer_type_replication_time_df['signature'] == signature) &
                                    (signature_cancer_type_replication_time_df['cancer_type'] == cancer_type)), 'spearman_corr' ] = spearman_corr # cancer_type

                        signature_cancer_type_replication_time_df.loc[((signature_cancer_type_replication_time_df['signature'] == signature) &
                                    (signature_cancer_type_replication_time_df['cancer_type'] == cancer_type)), 'spearman_p_value' ] = spearman_p_value # cancer_type

                        signature_cancer_type_replication_time_df.loc[((signature_cancer_type_replication_time_df['signature'] == signature) &
                                    (signature_cancer_type_replication_time_df['cancer_type'] == cancer_type)), 'pearson_corr' ] = pearson_corr # cancer_type

                        signature_cancer_type_replication_time_df.loc[((signature_cancer_type_replication_time_df['signature'] == signature) &
                                    (signature_cancer_type_replication_time_df['cancer_type'] == cancer_type)), 'pearson_p_value' ] = pearson_p_value # cancer_type

                        signature_cancer_type_replication_time_df.loc[((signature_cancer_type_replication_time_df['signature'] == signature) &
                                    (signature_cancer_type_replication_time_df['cancer_type'] == cancer_type)), 'cancer_type_slope' ] = np.around(cancer_type_m, NUMBER_OF_DECIMAL_PLACES_TO_ROUND) if cancer_type_m is not None else np.nan # cancer_type

                        signature_cancer_type_replication_time_df.loc[((signature_cancer_type_replication_time_df['signature'] == signature) &
                                    (signature_cancer_type_replication_time_df['cancer_type'] == cancer_type)), 'replication_timing_diff_btw_max_and_min' ] = np.around(replication_timing_diff_btw_max_and_min, NUMBER_OF_DECIMAL_PLACES_TO_ROUND) if replication_timing_diff_btw_max_and_min is not None else np.nan # cancer_type

                        signature_cancer_type_replication_time_df.loc[((signature_cancer_type_replication_time_df['signature'] == signature) &
                                    (signature_cancer_type_replication_time_df['cancer_type'] == cancer_type)), 'abs_replication_timing_diff_btw_medians' ] = np.around(abs_replication_timing_diff_btw_medians, NUMBER_OF_DECIMAL_PLACES_TO_ROUND) if abs_replication_timing_diff_btw_medians is not None else np.nan # cancer_type

                        signature_cancer_type_replication_time_df.loc[((signature_cancer_type_replication_time_df['signature'] == signature) &
                                    (signature_cancer_type_replication_time_df['cancer_type'] == cancer_type)), 'cancer_type_decision' ] = decision # cancer_type

                        signature_cancer_type_replication_time_df.loc[((signature_cancer_type_replication_time_df['signature'] == signature) &
                                    (signature_cancer_type_replication_time_df['cancer_type'] == cancer_type)), 'cutoff'] = cutoff # cancer_type

                        signature_cancer_type_replication_time_df.loc[((signature_cancer_type_replication_time_df['signature'] == signature) &
                                    (signature_cancer_type_replication_time_df['cancer_type'] == cancer_type)), 'number_of_mutations'] = number_of_mutations # cancer_type

                        signature_cancer_type_replication_time_df.loc[((signature_cancer_type_replication_time_df['signature'] == signature) &
                                    (signature_cancer_type_replication_time_df['cancer_type'] == cancer_type)), 'average_probability'] = np.around(average_probability, NUMBER_OF_DECIMAL_PLACES_TO_ROUND) if average_probability is not None else np.nan # cancer_type
    ...
    if (np.any(across_all_cancer_types_real_normalized_mutation_density_array)):
        ...
        if (across_all_cancer_types_all_simulations_normalized_mutation_density_array.ndim == 2) and \
                (across_all_cancer_types_all_simulations_normalized_mutation_density_array.size > 0):
            simulations_lows, \
            simulations_means, \
...
def write_all_tissues_combined_dataframe(plot_output_dir,
    ...
    for file_index, interested_processivity_table_file in enumerate(interested_processivity_table_file_list):
        ...
        # empty file gives pandas.errors.EmptyDataError: No columns to parse from file
        # SigProfilerTopography writes one empty line which makes 1 byte of empty file
        if os.stat(interested_processivity_table_file).st_size > 1:
...
def generate_replication_time_pdfs(plot_output_dir,
    ...
    for figure_type in figure_types:
        ...
        if len(sbs_signatures) > 0:
            signature_tuples.append((AGGREGATEDSUBSTITUTIONS, SBS))
        if len(dbs_signatures) > 0:
            signature_tuples.append((AGGREGATEDDINUCS, DBS))
        if len(id_signatures) > 0:
            signature_tuples.append((AGGREGATEDINDELS, ID))
...
if __name__ == '__main__':
    ...
    # topography_combined_output_dir = os.path.join('/oasis/tscc/scratch/burcak/SigProfilerTopographyRuns/Mutographs_ESCC_552/combined/')
    #
    # cancer_types = [
