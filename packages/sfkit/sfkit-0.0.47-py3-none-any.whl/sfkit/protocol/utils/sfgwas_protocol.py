...
def install_sfgwas() -> None:
    ...
    commands = [
        ...
        "echo 'export PYTHONUNBUFFERED=TRUE' >> ~/.bashrc && source ~/.bashrc",
        "pip3 install numpy",
    ]
...
def build_sfgwas(configuration: str) -> None:
    ...
    command = """export PYTHONUNBUFFERED=TRUE && cd sfgwas-private && go get -t github.com/hhcho/sfgwas-private && go build && mkdir -p stdout"""
    run_command(command)
...
def start_sfgwas() -> None:
    ...
    command = "export PYTHONUNBUFFERED=TRUE && cd sfgwas-private && bash batch_run.sh"
    run_command(command)
