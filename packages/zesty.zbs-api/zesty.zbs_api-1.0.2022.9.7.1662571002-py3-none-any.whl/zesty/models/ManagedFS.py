...
try:
    from sqlalchemy import Column, engine, cast, String, case, func, or_
...
class ManagedFsMixin:
    ...
    @staticmethod
    def instance_name_filter(query: Query, value: str):
        query = query.filter(cast(ManagedFs.tags, String).contains(f'"Name": "{value}"'))
        return query

    # Custom query
    @staticmethod
    def custom_query(session: Union[Session, sessionmaker]) -> Query:
        q = session.query(ManagedFs)
        q = q.add_columns(case((or_(ManagedFs.policies == None, cast(ManagedFs.policies, String) == 'null'), ''),
                               else_=cast(ManagedFs.policies, String).regexp_replace(r'.+"name":\s"([^"]+).+', "\\1"))
                          .label("policies_name"),
                          case((ManagedFs.tags == None, ''), else_=cast(ManagedFs.tags, String)
                               .regexp_replace(r'.+"Name":\s"([^"]+).+', "\\1"))
                          .label('instance_name')
                          )
        return q
