...
class ManagedFsMixin:
    ...
    def instance_name_filter(cls ,query: Query, value: str):
        query = query.filter(cast(cls.tags, String).contains(f'"Name": "{value}"'))
        return query
    ...
    def custom_query(cls, session: Union[Session, sessionmaker]) -> Query:
        ...
        q = q.add_columns(case((or_(cls.policies == None, cast(cls.policies, String) == 'null'), ''),
                          ...
                          .label("policies_name"),
                          case((cls.instance_tags == None, ''), else_=cast(cls.instance_tags, String)
                               .regexp_replace(r'.+"Name":\s"([^"]+).+', "\\1"))
                          .label('instance_name'),
                          case((cls.instance_tags == None, ''), else_=cast(cls.instance_tags, String)
                               .regexp_replace(r'\{.*"([^"]+)":.+', "\\1")).label('instance_tags_keys')
                          )
