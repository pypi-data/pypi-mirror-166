...
def load(configpath: Path) -> list[Resource]:
    ...
    try:
        ...
        message = (
            ...
            f"An entry in the config may have an invalid attribute (key)."
        )
        raise YamlizationError(message) from None
    except yaml.YAMLError as err:
        message = f"Failed to load labctrl resources from {configpath}. Details: {err}."
        raise YamlizationError(message) from None
...
def dump(configpath: Path, *resources: Resource) -> None:
    ...
    try:
        ...
        message = f"Failed to save labctrl resources to {configpath}. Details: {err}."
        raise YamlizationError(message) from None
