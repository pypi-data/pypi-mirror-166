...
def Recursion_Lable(origin_data,convs,region,regions_record,eigenvalue,logV,bins,keig_bins,label_area_max,recursion_time):
    keig_bins -= 1
...
#         keig_bins = np.int(-bins/10)
    ...
    for region_l in ero_regions_l:
        ...
        if region_l.area <= label_area_max and recursion_time*region_l.area >= label_area_max and region_l.area >= logV:
            ...
            regions_record = Recursion_Lable(origin_data,convs,region,regions_record,eigenvalue,logV,bins,keig_bins,label_area_max,recursion_time)
...
def Get_CP_Dict(label_area_max,rc_dict,mountain_array,mountain_dict,peak_dict,region_mp_dict,origin_data):
    ...
    for key in rc_dict.keys():
        ...
        for com_Facet in com_Facets:
            ...
            core_id += 1
        peak_dict_temp,core_dict_center = Update_CP_Dict_Facet(label_area_max,new_peak_dict,peak_dict_center,new_core_dict,mountain_array,origin_data)
...
def Detect_Facet(RMS,Threshold,KBins,Vmm,origin_data):
    ...
    peak_dict_record,core_dict_record = Get_CP_Dict(label_area_max,rc_dict,mountain_array,mountain_dict,peak_dict,region_mp_dict,origin_data)
    detect_infor_dict = DID_Facet(peak_dict_record,core_dict_record,origin_data)
    return detect_infor_dict

if __name__ == '__main__':
    RMS = 0.23
    Threshold = 2 * RMS  # 阈值['mean','otsu',n*RMS]
    KBins = 35  # 总区间数[10,...,70]
    Vmm = 16  # 极大值区的最大体元[9,16,27,36]
