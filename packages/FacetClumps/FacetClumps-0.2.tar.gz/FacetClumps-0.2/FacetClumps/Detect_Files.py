...
from FacetClumps import FacetClumps_2D
from FacetClumps import FacetClumps_3D
...
def Table_Interface(did_table, ndim):
    ...
    if ndim == 2:
        ...
        Cen1 = np.array(did_table['clump_center'])[:, 1] + 1
        Cen2 = np.array(did_table['clump_center'])[:, 0] + 1
        Size1 = np.array(did_table['clump_size'])[:, 1]
        Size2 = np.array(did_table['clump_size'])[:, 0]
        index_id = np.array(range(1, len(Peak1) + 1, 1))
        d_outcat = []
        d_outcat = np.hstack(
            [[index_id, Peak1, Peak2, Cen1, Cen2, Size1, Size2, Peak, Sum, Volume]]).T
        ...
        Cen1 = np.array(did_table['clump_center'])[:, 2] + 1
        Cen2 = np.array(did_table['clump_center'])[:, 1] + 1
        Cen3 = np.array(did_table['clump_center'])[:, 0] + 1
...
def Detect(file_name, parameters, mask_name, outcat_name, outcat_wcs_name):
    ...
    if ndim==2:
        did_table = FacetClumps_2D.Detect_Facet(RMS, Threshold, KBins, Vmm, origin_data)
    elif ndim==3:
        did_table = FacetClumps_3D.Detect_Facet(RMS, Threshold, KBins, Vmm, origin_data)
    else:
    ...
    time_record.to_csv(outcat_name[:-4] + '_time_record.txt', sep='\t', index=False)
