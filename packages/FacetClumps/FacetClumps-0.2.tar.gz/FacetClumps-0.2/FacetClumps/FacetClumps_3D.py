...
def Get_Regions_Facet(origin_data, RMS=0.1, threshold='otsu', temp_array=None):
    kopen_radius = 1
    regions_array = np.zeros_like(origin_data, dtype='uint16')
    if threshold == 'mean':
...
def Convolve(origin_data):
    ...
    yt = w * y
    zt = w * z
    k1 = (5 * s ** 2 - (xt ** 2 + yt ** 2 + zt ** 2)) / (2 * math.sqrt(2 * math.pi) * s ** 3)
    k2 = xt * (7 * s ** 2 - (xt ** 2 + yt ** 2 + zt ** 2)) / (2 * math.sqrt(2 * math.pi) * s ** 5)
    k3 = yt * (7 * s ** 2 - (xt ** 2 + yt ** 2 + zt ** 2)) / (2 * math.sqrt(2 * math.pi) * s ** 5)
    k4 = zt * (7 * s ** 2 - (xt ** 2 + yt ** 2 + zt ** 2)) / (2 * math.sqrt(2 * math.pi) * s ** 5)
    k5 = -(s ** 2 - xt ** 2) / (2 * math.sqrt(2 * math.pi) * s ** 5)
    k6 = -(s ** 2 - yt ** 2) / (2 * math.sqrt(2 * math.pi) * s ** 5)
    k7 = -(s ** 2 - zt ** 2) / (2 * math.sqrt(2 * math.pi) * s ** 5)
    k8 = xt * yt / (math.sqrt(2 * math.pi) * s ** 5)
    k9 = xt * zt / (math.sqrt(2 * math.pi) * s ** 5)
    k10 = yt * zt / (math.sqrt(2 * math.pi) * s ** 5)
    k11 = xt * yt * zt / (math.sqrt(2 * math.pi) * s ** 7)
    k12 = -xt * (s ** 2 - yt ** 2) / (2 * math.sqrt(2 * math.pi) * s ** 7)
    k13 = -xt * (s ** 2 - zt ** 2) / (2 * math.sqrt(2 * math.pi) * s ** 7)
    k14 = -yt * (s ** 2 - xt ** 2) / (2 * math.sqrt(2 * math.pi) * s ** 7)
    k15 = -yt * (s ** 2 - zt ** 2) / (2 * math.sqrt(2 * math.pi) * s ** 7)
    k16 = -zt * (s ** 2 - xt ** 2) / (2 * math.sqrt(2 * math.pi) * s ** 7)
    k17 = -zt * (s ** 2 - yt ** 2) / (2 * math.sqrt(2 * math.pi) * s ** 7)
    k18 = -xt * (3 * s ** 2 - xt ** 2) / (6 * math.sqrt(2 * math.pi) * s ** 7)
    k19 = -yt * (3 * s ** 2 - yt ** 2) / (6 * math.sqrt(2 * math.pi) * s ** 7)
    k20 = -zt * (3 * s ** 2 - zt ** 2) / (6 * math.sqrt(2 * math.pi) * s ** 7)
...
def Recursion_Lable(origin_data, convs, region, regions_record, eigenvalue, logV, bins, keig_bins, label_area_max,
                    recursion_time):
    keig_bins -= 1
    ...
    for region_l in ero_regions_l:
        ...
        if region_l.area <= label_area_max and recursion_time * region_l.area >= label_area_max and region_l.area >= logV:
            regions_record.append(region)
        elif recursion_time == 0 and region_l.area > label_area_max / 2 and region_l.area <= label_area_max:
            regions_record.append(region)
        elif region_l.area > label_area_max:
            recursion_time += 1
            region = Get_Lable(origin_data, convs, region, eigenvalue, bins, keig_bins)
            regions_record = Recursion_Lable(origin_data, convs, region, regions_record, eigenvalue, logV, bins,
                                             keig_bins, label_area_max, recursion_time)
...
def Get_Total_COM(origin_data, regions, convs, kbins, label_area_max):
    keig_bins = -1
    recursion_time = 0
    regions_record = []
    for i in range(len(regions)):
        ...
        regions_record = Recursion_Lable(origin_data, convs, region, regions_record, eigenvalue, logV, bins, keig_bins,
                                         label_area_max, recursion_time)
...
def Build_MPR_Dict(origin_data, regions):
    ...
    peak_array = np.zeros_like(origin_data)
...
def Update_CP_Dict_Facet(label_area_max, new_peak_dict, peak_dict_center, new_core_dict, mountain_array, origin_data):
    peak_dict_temp = {}
    core_dict_center = {}
    key_mountain_record = []
    center_mountain = {}
        ...
        if len(new_core_dict) > len(peak_dict_center.keys()):
            for key_center in peak_dict_center.keys():
                if len(new_core_dict) > 0:
                    ...
                    Delect(new_peak_dict, new_core_dict, peak_dict_center, core_dict_center, key_peak_record, near_k_2,
                           origin_data)
        else:
            key_peak_record = np.array(list(new_peak_dict.keys()))
            Delect(new_peak_dict, new_core_dict, peak_dict_center, core_dict_center, key_peak_record, near_k_1,
                   origin_data)
    ...
    #         new_core_dict = {}
    #         key_peak_record = []
    #         if len(core_dict_center[key_center])<label_area_max and len(peak_dict_temp)==1:
    #             del peak_dict_temp[key_center]
    #             del core_dict_center[key_center]
    #         elif len(core_dict_center[key_center])<label_area_max and len(peak_dict_temp)>1:
...
def Get_CP_Dict(label_area_max, rc_dict, mountain_array, mountain_dict, peak_dict, region_mp_dict, origin_data):
    core_id = 0
    peak_dict_record = {}
    core_dict_record = {}
    for key in rc_dict.keys():
        ...
        for com_Facet in com_Facets:
            ...
            core_id += 1
        peak_dict_temp, core_dict_center = Update_CP_Dict_Facet(label_area_max, new_peak_dict, peak_dict_center,
                                                                new_core_dict, mountain_array, origin_data)
...
# def DID_Facet(peak_dict,core_dict,origin_data):
#     peak_value = []
#     peak_location = []
#     clump_center=[]
#     clump_com = []
...
def Detect_Facet(RMS, Threshold, KBins, Vmm, origin_data):
    ...
    com = Get_Total_COM(origin_data, regions_1, convs, KBins, label_area_max)
    ...
    peak_dict_record, core_dict_record = Get_CP_Dict(label_area_max, rc_dict, mountain_array, mountain_dict, peak_dict,
                                                     region_mp_dict, origin_data)
...
if __name__ == '__main__':
    ...
    did_Facet = Detect_Facet(RMS,Threshold,KBins,Vmm,origin_data)
