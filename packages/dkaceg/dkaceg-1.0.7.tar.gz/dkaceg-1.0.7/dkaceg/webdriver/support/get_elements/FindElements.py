...
def get_the_elements_by_bot_id(page_source,bot_id,driver,multiple_element):
    ...
    analysis=change_analysis(analysis)
    elements=find_the_elements_by_event(reqsoup,page_source,analysis)
    if(multiple_element==False):
        element=driver.find_element_by_xpath(xpath_soupreq(elements[0]))
    else:
        element=[]
        for el in elements:
            element.append(driver.find_element_by_xpath(xpath_soupreq(el)))
    return element