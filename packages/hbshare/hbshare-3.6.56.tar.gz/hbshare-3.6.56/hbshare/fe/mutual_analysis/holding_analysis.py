...
class holding_analysis_v3:
    ...
    def index_compare(self):
        ...
        index_daily_k = index_daily_k.rename(columns={'885001': '偏股混合型基金指数（885001）', '881001': '万得全A（881001）'})
        ...
        ax[0].plot(index_daily_k_2019.index, index_daily_k_2019['偏股混合型基金指数（885001）'].values, label='偏股混合型基金指数（885001）', color='#F04950')
        ax[0].plot(index_daily_k_2019.index, index_daily_k_2019['万得全A（881001）'].values, label='万得全A（881001）', color='#6268A2')
        ax[1].plot(index_daily_k_2022.index, index_daily_k_2022['偏股混合型基金指数（885001）'].values, label='偏股混合型基金指数（885001）', color='#F04950')
        ax[1].plot(index_daily_k_2022.index, index_daily_k_2022['万得全A（881001）'].values, label='万得全A（881001）', color='#6268A2')
        ax[0].legend(loc=2)
        ax[1].legend(loc=2)
        plt.tight_layout()
    ...
    def turnover(self):
        ...
        fund_turnover['TURNOVER'] = fund_turnover['TURNOVER'] * 2.0 / 100.0
