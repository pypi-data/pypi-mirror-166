...
def fund_style_plot(date, pic_path):
    ...
    # fig, ax = plt.subplots(figsize=(6, 6))
    # ax.plot(fund_style1_latest.index, fund_style1_latest['成长'], color='#F04950', label='成长')
    # ax.plot(fund_style1_latest.index, fund_style1_latest['平衡'], color='#6268A2', label='平衡')
    # ax.plot(fund_style1_latest.index, fund_style1_latest['价值'], color='#959595', label='价值')
    # ax.legend(loc=2)
    # ax.set_ylim([0.0, 1.0])
    # ax.set_xticklabels(labels=fund_style1_latest.index, rotation=90)
    # plt.xticks(rotation=90)
    # ax.yaxis.set_major_formatter(FuncFormatter(to_100percent))
    # plt.title('风格统计')
    # plt.tight_layout()
