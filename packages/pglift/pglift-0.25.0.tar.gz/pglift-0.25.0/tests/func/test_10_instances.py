...
def test_logs(
    ...
    with reconfigure_instance(
        ctx, instance_manifest, logging_collector=True
    ) as changes:
        assert changes == {"logging_collector": (False, True)}
        with instances.postgresql_running(ctx, instance):
            pass
        logs = list(instances.logs(ctx, instance))
        assert "database system is shut down" in logs[-1]
    assert changes == {"logging_collector": (True, False)}
