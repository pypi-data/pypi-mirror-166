...
class PatroniSettings(BaseSettings):
    ...
    bin_dir: DirectoryPath = Field(
        default=Path("/usr/bin"),
        description="Path to the directory where Patroni executables are located.",
    )
    ...
    @validator("bin_dir", always=True)
    def __validate_bin_dir_(cls, value: Path) -> Path:
        for fname in ("patroni", "patronictl"):
            if not (value / fname).is_file():
                raise ValueError(f"'{fname}' file not found in {value}")
        return value

    @property
    def patroni(self) -> Path:
        return self.bin_dir / "patroni"

    @property
    def patronictl(self) -> Path:
        return self.bin_dir / "patronictl"
...
class PgBackRestSettings(BaseSettings):
    ...
    logpath: LogPath = Field(
        default=LogPath("pgbackrest/{name}/logs"),
        description="Path where log files are stored.",
    )
