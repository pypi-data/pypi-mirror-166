...
from file_utils import get_tmp_dir
...
class S3Artifact(Artifact):
    ...
    def get_full_name(self):
        return self.folder + '/' + self.get_simple_name()

    def get_s3_uri(self):
        return 's3://{bucket}/{artifact}'.format(bucket = self.bucket, artifact = self.get_full_name())
...
class ArtifactsManager:
    ...
    def __get_download_from_s3_script(self, artifact:S3Artifact, output_folder:str) -> str:
        ...
        script += 'aws s3 cp {artifact} {output_folder}\n' \
                    .format(s3_uri = artifact.get_s3_uri(), \
