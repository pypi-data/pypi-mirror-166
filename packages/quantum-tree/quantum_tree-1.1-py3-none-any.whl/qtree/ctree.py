...
class Node(object):
    ...
    def _split(self, tree_dict):
        if len(tree_dict) > 0 and type(list(tree_dict.values())[0]) is list and np.all([type(split) is dict for split in list(tree_dict.values())[0]]):
    ...
    def _to_str(self, indent=0, tab = "\t", title=""):
        ...
        if self._proba_prediction is not None:
            if self._default_y is None:
                ...
                undef_tag = "! y=[{}]".format(",".join(["{:d}".format(y) if y is not None else "?" for y in self._default_y]))
            ...
            for idx in range(self._y_dim):
                out += "{}{}".format(tab*indent, "y_{} = {} ({:.3f}/{:.3f}){}".format(idx, y_predict[idx], y_proba_0[idx], y_proba_1[idx], tag_str)) + "\n"
        return out
    ...
    def _predict_proba(self, x=None, omit_warnings=False, verbose=False): # verbose = True: print path through tree for each prediction
        if verbose:
            self._print("predict_proba: depth={}, split_index={}".format(self._depth, self._split_index))
        if self._proba_prediction is not None:
            if verbose:
                self._print("\tpredict_proba: prediction={}{}".format(self._proba_prediction, " (undefined)" if self._undefined_prediction else ""))
...
class ClassicalTreeBase(Node):
    ...
    def _get_y_dim(self, value):
        if type(value) is list:
            ...
            if len(value) == 0:
                return None, False
            elif all([type(value) is list and all([type(v) is float or type(v) is int for v in value]) for value in value.values()]):
...
class ClassicalTree(ClassicalTreeBase):
    ...
    def from_tree_dict(self, tree_dict, tree_dict_detailed={}):
        ...
        if tree_dict_detailed is None:
            tree_dict_detailed = {}
        self._tree_dict_detailed = copy.deepcopy(tree_dict_detailed) # additional information, not actually used for classifier
    ...
    def predict(self, X, omit_warnings=False, verbose=False):
        ...
        X = np.asarray(X)
        y = np.empty((X.shape[0], self._y_dim), dtype=np.bool)
    ...
    def calc_exact_probabilities(self, X, y, omit_warnings=False):
        ...
        for path in paths:
            ...
            if y_filter.shape[0] > 0:
                py0 = [y_filter[y_filter[:,y_idx]==0,y_idx].shape[0]/y_filter.shape[0] for y_idx in range(y_filter.shape[1])]
                py1 = [y_filter[y_filter[:,y_idx]==1,y_idx].shape[0]/y_filter.shape[0] for y_idx in range(y_filter.shape[1])]
        ...
        for path in paths:
            ...
            for split_idx, split_value in path:
                ...
                if split_idx not in node:
                    node[split_idx] = [{},{}]
                node = node[split_idx][split_value]
            X_filter = np.delete(X_filter, split_idx_list, axis=1) # enable: save only reduced, disable: save everything
    ...
    def calc_difference_to_exact(self, paths, tree_dict_exact, tree_dict_exact_data_probabilities={}, N=None, epsilon=None, omit_warnings=False):
