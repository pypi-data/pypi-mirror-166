...
The company doesn’t expect to meet analysts’ profit estimates of $3.9 to $4 million, or 76 cents a share to 79 cents a share, for its year ending Sept. 24, according to Pres. John Doe."""
    ...
    # With abbrev
    parameters = RulesSegmenterParameters(lang='en', engine=RuleEngine.Custom)
    docs: List[Document] = segmenter.segment([Document(text=TEXT)], parameters)
    doc0 = docs[0]
    ...
    assert sents[3] == "John Doe."
    # With Spacy
    parameters = RulesSegmenterParameters(lang='en', engine=RuleEngine.Spacy)
    docs: List[Document] = segmenter.segment([Document(text=TEXT)], parameters)
    doc0 = docs[0]
    assert len(doc0.sentences) == 5
    sents = [doc0.text[s.start:s.end] for s in doc0.sentences]
    assert sents[0] == "CLAIRSSON"
    assert sents[
               1] == "INTERNATIONAL REPORTS LOSS\n\nClairson International Corp. said it expects to report a net loss for its second quarter ended March 26."
    assert sents[
               2] == "\n"
    assert sents[
               3] == "The company doesn’t expect to meet analysts’ profit estimates of $3.9 to $4 million, or 76 cents a share to 79 cents a share, for its year ending Sept. 24, according to Pres."
    assert sents[4] == "John Doe."
