...
alf_data_path = "/opt/iParapheur/alf_data/contentstore/"
...
def user_handler():
    ...
    for i, user in enumerate(cusers):
        ...
        query = "SELECT anp1.string_value " \
                ...
                "AND anp2.string_value = '%s';" % (user['username'].replace("'","\\'"))
...
def advanced_handler():
    ...
    for seal in seals:
        ...
        for res in cursor:
            ...
            res = res[0]
            res = res[8:]
            src_dir = alf_data_path + res
            dst_dir = 'seals/' + seal['id'] + ".bin"
            shutil.copy(src_dir, dst_dir)
        ## Récupérer les MDP
        query = "select string_value from alf_node_properties where node_id=" \
                "(select id from alf_node where uuid='%s') " \
                "and qname_id=" \
