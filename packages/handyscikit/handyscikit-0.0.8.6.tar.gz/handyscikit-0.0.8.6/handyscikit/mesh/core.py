...
class Mesh2D(MeshBase):
    ...
    def _calc_face_area(self):
        self._face_area = np.zeros([self._face_num, 3], dtype=np.float32)

        for i in range(self.face_num):
            self._face_area[i] = (np.linalg.norm(self._nodes[self._faces[i, 0]] - self._nodes[self._faces[i, 1]])) * self._face_norm[i]
