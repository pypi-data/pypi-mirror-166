...
async def api_login(api_id, api_hash, session_name):
    client = await TelegramClient(session_name, api_id, api_hash).start()
    return client


async def near_entity(client, data_path, sleep_seconds, move_num, start_index=0):
    ...
    async def geo_entity(lat, lon, radius):
...
async def near_entity_resume(client, data_path, data_path_user, data_path_channel, sleep_seconds, move_num):
    ...
    await near_entity(client, data_path, sleep_seconds, move_num=move_num + start_index, start_index=start_index)
...
async def keywords_search_channel(client, data_path_channel, keywords, date_time, after_before, save_mode):
    await client.start()
    if save_mode == "all":
        ...
        async def search_in_channel(entity):
            try:
                ...
                for q in keywords:
                    ...
                    async for chat in chat_search:
                        ...
                        user_info = await client.get_entity(chat_user_id)
...
async def keywords_search_channel_resume(client, data_path_resume):
    date_record = pd.to_datetime("today").strftime("%Y-%m-%d_%H%M%S")
    client = client
    ...
    await keywords_search_channel(client=client,
