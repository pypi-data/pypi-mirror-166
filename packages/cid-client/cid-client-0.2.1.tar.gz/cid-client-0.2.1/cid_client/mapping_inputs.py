...
class LocationInput(BaseDictKey):
    ...
    def validate(self):
        ...
        if self.artifact_archival_copy:
            parse_res = urlparse(self.artifact_archival_copy)
            if parse_res.scheme not in ['http', 'https']:
                errors.append(
                    f'Artifact archival copy must be http link. '
                    f'{self.artifact_archival_copy}'
                )
        return errors
