...
from ..models.api import PlatformJourneyStepApiResponse, PlatformJourneyStepProcessInput

from ..models.step import PlatformJourneyStepComponent, PlatformJourneyStepDefinition, PlatformJourneyStepDefinitionUiAssets, PlatformJourneyStepStatus
from ..utils.utils import ApiInputs, _column_name_cap, get_journeys_endpoint
from .JourneyTask import JourneyTask

class JourneyStepDefinitionTask(JourneyTask):
    logger = None
    ...
    @property
    @abstractmethod
    def definition(self) -> PlatformJourneyStepDefinition:
        """"Structure of the Journey Step"""
        return None

    @abstractmethod
    def check_status(self, api_inputs: ApiInputs, journey_input: PlatformJourneyStepProcessInput) -> PlatformJourneyStepStatus:
        ...
        Returns:
            PlatformJourneyStepStatus: response containing the status of the step or None
        """
        return None
    
    @abstractmethod
    def check_ui_assets(self, step_status: PlatformJourneyStepStatus) -> PlatformJourneyStepDefinitionUiAssets:
        ...
        Args:
            step_status (PlatformJourneyStepStatus): The journey step status to evaluate.

        Returns:
            PlatformJourneyStepDefinitionUiAssets: response containing UiAsset overrides.
        """
        return None
    
    @abstractmethod
    def check_ui_component(self, api_inputs: ApiInputs, step_status: PlatformJourneyStepStatus, journey_input: PlatformJourneyStepProcessInput) -> PlatformJourneyStepComponent:
        ...
        Args:
            api_inputs (ApiInputs): Provides state for calling Switch Platform APIs.
            step_status (PlatformJourneyStepStatus): The journey step status to evaluate.
            journey_input (PlatformJourneyStepProcessInput): Information specific to a Journey

        Returns:
            PlatformJourneyStepComponent: response containing ui component override or None
        """
        return None

    @abstractmethod
    def process(self, api_inputs: ApiInputs, journey_input: PlatformJourneyStepProcessInput) -> PlatformJourneyStepApiResponse:
        return None
    
    def update_component(self, response: PlatformJourneyStepApiResponse, component: PlatformJourneyStepComponent):
        if (response == None or type(response) is not PlatformJourneyStepApiResponse):
            self.logger.error('Provided response model is not a valid')
            raise InvalidStepApiReponseException('Expected valid PlatformJourneyStepApiResponse')
        
        response.component = component
        
    def update_status(self, response: PlatformJourneyStepApiResponse, status: PlatformJourneyStepStatus):
        if (response == None or type(response) is not PlatformJourneyStepApiResponse):
            self.logger.error('Provided response model is not a valid')
            raise InvalidStepApiReponseException('Expected PlatformJourneyStepApiResponse')
        
        if (status == None):
            self.logger.error('Provided Step Status model is not a valid')
            raise InvalidStepStatusReponseException('Expected valid PlatformJourneyStepStatus')
