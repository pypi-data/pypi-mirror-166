...
def integral_range_step(start: _T, step: _T, stop: _T, zero: _T, add: Callable[[_T, _T], _T]) -> IEnumerable[_T]:
    ...
    def _arrow119(__unit: Literal[None]=None, start: _T=start, step: _T=step, stop: _T=stop, zero: _T=zero, add: Callable[[_T, _T], _T]=add) -> IEnumerable[_T]:
...
def range_int64(start: int64, step: int64, stop: int64) -> IEnumerable[int64]:
...
def range_uint64(start: uint64, step: uint64, stop: uint64) -> IEnumerable[uint64]:
...
def range_char(start: str, stop: str) -> IEnumerable[str]:
    ...
    def _arrow125(__unit: Literal[None]=None, start: str=start, stop: str=stop) -> IEnumerable[str]:
