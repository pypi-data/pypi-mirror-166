...
from typing import (TypeVar, Literal, Any, Optional, Callable, Generic, Tuple)
...
from .util import (equals, structural_hash, compare, IEnumerator, to_iterator, get_enumerator, IDisposable, ignore, IEnumerable, is_array_like, IEqualityComparer_1, IComparer_1)
...
class FSharpList(Record, Generic[_T]):
    ...
    def __eq__(self, other: Any=None) -> bool:
        ...
        if xs is other:
            ...
            def loop(xs_1_mut: FSharpList[Any], ys_1_mut: FSharpList[Any]) -> bool:
                while True:
                    (xs_1, ys_1) = (xs_1_mut, ys_1_mut)
                    matchValue: Optional[FSharpList[Any]] = xs_1.tail
                    matchValue_1: Optional[FSharpList[Any]] = ys_1.tail
                    if matchValue is not None:
                        if matchValue_1 is not None:
                            xt: FSharpList[Any] = matchValue
                            yt: FSharpList[Any] = matchValue_1
    ...
    def __cmp__(self, other: Any=None) -> int:
        xs: FSharpList[_T] = self
        def loop(xs_1_mut: FSharpList[Any], ys_1_mut: FSharpList[Any]) -> int:
            while True:
                (xs_1, ys_1) = (xs_1_mut, ys_1_mut)
                matchValue: Optional[FSharpList[Any]] = xs_1.tail
                matchValue_1: Optional[FSharpList[Any]] = ys_1.tail
...
def FSharpList__get_Length(xs: FSharpList[Any]) -> int:
    def loop(i_mut: int, xs_1_mut: FSharpList[Any], xs: FSharpList[_T]=xs) -> int:
        while True:
            ...
            match_value: Optional[FSharpList[Any]] = xs_1.tail
            if match_value is not None:
                i_mut = i + 1
...
def FSharpList__get_Item_Z524259A4(xs: FSharpList[_T], index: int) -> _T:
    def loop(i_mut: int, xs_1_mut: FSharpList[Any], xs: FSharpList[_T]=xs, index: int=index) -> Any:
        while True:
            (i, xs_1) = (i_mut, xs_1_mut)
            match_value: Optional[FSharpList[Any]] = xs_1.tail
            if match_value is not None:
    ...
    if is_array_like(xs):
        ...
        with get_enumerator(xs) as enumerator:
            while enumerator.System_Collections_IEnumerator_MoveNext():
                ...
                def _arrow77(__unit: Literal[None]=None, xs: IEnumerable[_T]=xs) -> FSharpList[_T]:
...
def insert_many_at(index: int, ys: IEnumerable[_T], xs: FSharpList[_T]) -> FSharpList[_T]:
    ...
    result: FSharpList[_T] = fold(folder, FSharpList_get_Empty(), xs)
    def _arrow117(__unit: Literal[None]=None, index: int=index, ys: IEnumerable[_T]=ys, xs: FSharpList[_T]=xs) -> FSharpList[_T]:
        raise Exception((SR_indexOutOfBounds + "\\nParameter name: ") + "index")
