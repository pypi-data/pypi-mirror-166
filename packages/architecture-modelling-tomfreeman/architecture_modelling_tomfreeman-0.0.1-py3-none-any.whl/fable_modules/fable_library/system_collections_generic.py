...
from .util import (compare, IComparer_1, equals, structural_hash, IEqualityComparer_1, get_enumerator, IEnumerable, IEnumerator, to_iterator, max, compare_primitives)
...
def Comparer_1_get_Default(__unit: Literal[None]=None) -> IComparer_1[Any]:
    class ObjectExpr9(IComparer_1[_T_]):
...
EqualityComparer_1_reflection = _expr10
...
def EqualityComparer_1_get_Default(__unit: Literal[None]=None) -> IEqualityComparer_1[Any]:
    class ObjectExpr11(IEqualityComparer_1[Any]):
...
class Stack_1(Generic[_T]):
    ...
    def GetEnumerator(self, __unit: Literal[None]=None) -> IEnumerator[_T]:
        _: Stack_1[_T] = self
        def _arrow15(__unit: Literal[None]=None) -> IEnumerable[_T]:
            index: int = (_.count - 1) or 0
            def _arrow12(__unit: Literal[None]=None) -> bool:
                return index >= 0

            def _arrow14(__unit: Literal[None]=None) -> IEnumerable[_T]:
                def _arrow13(__unit: Literal[None]=None) -> IEnumerable[_T]:
                    nonlocal index
                    index = (index - 1) or 0
                    return empty()

                return append(singleton(_.contents[index]), delay(_arrow13))

            return enumerate_while(_arrow12, delay(_arrow14))

        return get_enumerator(delay(_arrow15))
...
def Stack_1__Ensure_Z524259A4(_: Stack_1[Any], new_size: int) -> None:
    ...
    if new_size > old_size:
        ...
        def _arrow19(x: int, y: int, _: Stack_1[_T]=_, new_size: int=new_size) -> int:
            return compare_primitives(x, y)

        def _arrow20(x: int, y: int, _: Stack_1[_T]=_, new_size: int=new_size) -> int:
            return compare_primitives(x, y)

        _.contents = fill([0] * max(_arrow19, new_size, old_size * 2), 0, max(_arrow20, new_size, old_size * 2), None)
...
def Queue_1__toSeq(this: Queue_1[_T]) -> IEnumerable[_T]:
    def _arrow27(__unit: Literal[None]=None, this: Queue_1[_T]=this) -> IEnumerable[_T]:
        i: int = 0
        def _arrow24(__unit: Literal[None]=None) -> bool:
            return i < this.count

        def _arrow26(__unit: Literal[None]=None) -> IEnumerable[_T]:
            def _arrow25(__unit: Literal[None]=None) -> IEnumerable[_T]:
                nonlocal i
                i = (i + 1) or 0
                return empty()
