...
from mitos.trna.arwenscan import arwenfromdb
from mitos.trna.trnascan import trnascanfromdb
...
def deltrnafeat_ovlprotrrna(gb, tf, threshold):
    ...
    for i in range(len(tf.features) - 1, -1, -1):
        ...
        length = tf.features[i].len(True, gb.size)
        if float(capsum) / float(length) > threshold:
            del tf.features[i]
            ...
            if len(trnascanm[m]) == 1 and len(arwenm[m]) == 0:
                ...
                for i in arwenm[m]:
                    for j in trnascanm[m]:
                        if (trnascan.features[j].overlap(arwen.features[i], gb.size) >= threshold
                                and trnascan.features[j].name == arwen.features[i].name):
                            pairs.append((i, j))
                if len(pairs) == 1:
...
#        print gb.features[i], maxtf, maxt
        ...
        if maxaf is not None and gb.features[i].name != maxaf.name:
            if gb.features[i].name not in rename:
                rename[gb.features[i].name] = {}
            if maxaf.name not in rename[gb.features[i].name]:
                rename[gb.features[i].name][maxaf.name] = 0
            rename[gb.features[i].name][maxaf.name] += 1

        if maxtf is not None and gb.features[i].name != maxtf.name:
            if gb.features[i].name not in rename:
                rename[gb.features[i].name] = {}
            if maxtf.name not in rename[gb.features[i].name]:
...
def correct_signs(gb, arwen, trnascan, threshold):
    for i in range(len(gb.features)):
        ...
        if (maxaf is None and maxtf is not None and maxtf.name == gb.features[i].name):
        ...
        elif (maxtf is None and maxaf is not None and maxaf.name == gb.features[i].name):
        ...
        elif (maxaf is not None and maxtf is not None and maxaf.name == gb.features[i].name and maxtf.name == gb.features[i].name):
...
#        print "gb",g
        ...
        if exists("/tmp/%s.arw" % gb.accession):
            ...
            arwen = arwenfromdb(gb.accession)
        ...
        if exists("/tmp/%s.tsc" % gb.accession):
            ...
            trnascan = trnascanfromdb(gb.accession)
