...
from arkcloud.lib import get_x, get_y, get_width, get_profile
from arkcloud.lib import get_height, get_email, get_password, get_admin_password
from arkcloud.lib import get_server_name, get_remote, get_dark_mode, countdown
...
class ArkWebsite(Website):
    ...
    def __init__(self, remote=False, dark_mode=False, profile=None, **kwargs):
        remote = remote or get_remote()
        arguments = {
            'remote': remote or get_remote(),
            'dark_mode': (get_dark_mode() or dark_mode) and not remote,
            'x': get_x(),
            'y': get_y(),
            'width': get_width(),
            'height': get_height(),
        ...
        super().__init__(self.LOGIN_URL, **arguments)
        self.remote = get_remote() or remote
    ...
    def create_character(self, first_time=False):
    ...
    def menu_reset(self, timeout=2):
        ...
        if self.resolve_disconnections() or not self.is_intro_menu():
            print("Was unable to resolve or get to the intro menu")
...
if __name__ == "__main__":
    ...
    try:
        ...
        input(f"Press enter to join server {server_name}")
        ark.join_server()

        input("Press enter to enable admin")
        ark.enable_admin()
        while True:
            print("""
