...
class dNoTempl:
    ...
    async def lock_one_by_key(self, _key: tNoTemplKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        async with dNoTemplLockOne(self._dbcon) as _qry:
...
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_by_key(self, _key: tNoTemplKey) -> bool:
        ...
        """
        async with dNoTemplSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def update(self, irec: tNoTempl = None):
        ...
        async with dNoTemplUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tNoTempl = None):
        ...
        async with dNoTemplInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
