...
class dCorrProv:
    ...
    async def lock_one_by_key(self, _key: tCorrProvKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        async with dCorrProvLockOne(self._dbcon) as _qry:
            ...
            while _dex >= 0:
                try:
                    ...
                    if not await _qry.fetch():
                        ...
                        raise mettle.lib.xMettle(f"Lock not found: [table:dCorrProv, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
...
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_by_key(self, _key: tCorrProvKey) -> bool:
        ...
        """
        async with dCorrProvSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def select_one(self) -> "dCorrProv":
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dCorrProv, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_deft(self,
        ...
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dCorrProv, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_by_key(self, _key: tCorrProvKey) -> "dCorrProv":
        ...
        """
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dCorrProv, row:{self.irec}]", xMettle.eCode.DBNotFound)
    ...
    async def update(self, irec: tCorrProv = None):
        ...
        async with dCorrProvUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tCorrProv = None):
        ...
        async with dCorrProvInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
    ...
    async def delete_one(self, irec: tCorrProvKey = None):
        ...
        async with dCorrProvDeleteOne(self._dbcon) as _qry:
            await _qry.exec(_key)
