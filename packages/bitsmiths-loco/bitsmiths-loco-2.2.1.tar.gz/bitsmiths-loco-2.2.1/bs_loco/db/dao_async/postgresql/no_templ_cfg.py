...
class dNoTemplCfg:
    ...
    async def lock_one_by_key(self, _key: tNoTemplCfgKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        async with dNoTemplCfgLockOne(self._dbcon) as _qry:
...
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_by_key(self, _key: tNoTemplCfgKey) -> bool:
        ...
        """
        async with dNoTemplCfgSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def update(self, irec: tNoTemplCfg = None):
        ...
        async with dNoTemplCfgUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tNoTemplCfg = None):
        ...
        async with dNoTemplCfgInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
