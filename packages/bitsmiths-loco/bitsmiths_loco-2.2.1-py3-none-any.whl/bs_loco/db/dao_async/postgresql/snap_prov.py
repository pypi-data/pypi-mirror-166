...
class dSnapProv:
    ...
    async def lock_one_by_key(self, _key: tSnapProvKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        async with dSnapProvLockOne(self._dbcon) as _qry:
            ...
            while _dex >= 0:
                try:
                    ...
                    if not await _qry.fetch():
                        ...
                        raise mettle.lib.xMettle(f"Lock not found: [table:dSnapProv, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
...
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_by_key(self, _key: tSnapProvKey) -> bool:
        ...
        """
        async with dSnapProvSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def select_one(self) -> "dSnapProv":
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dSnapProv, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_deft(self,
        ...
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dSnapProv, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_by_key(self, _key: tSnapProvKey) -> "dSnapProv":
        ...
        """
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dSnapProv, row:{self.irec}]", xMettle.eCode.DBNotFound)
    ...
    async def update(self, irec: tSnapProv = None):
        ...
        async with dSnapProvUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tSnapProv = None):
        ...
        async with dSnapProvInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
    ...
    async def delete_one(self, irec: tSnapProvKey = None):
        ...
        async with dSnapProvDeleteOne(self._dbcon) as _qry:
            await _qry.exec(_key)
