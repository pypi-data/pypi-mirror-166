...
class dNoType:
    ...
    async def lock_one_by_key(self, _key: tNoTypeKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        async with dNoTypeLockOne(self._dbcon) as _qry:
            ...
            while _dex >= 0:
                try:
                    ...
                    if not await _qry.fetch():
                        ...
                        raise mettle.lib.xMettle(f"Lock not found: [table:dNoType, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
...
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_by_key(self, _key: tNoTypeKey) -> bool:
        ...
        """
        async with dNoTypeSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def select_one(self) -> "dNoType":
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle(f"Not found: [table:dNoType, row:{self.rec}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_deft(self,
        ...
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dNoType, row:{_key}]", err_code=mettle.lib.xMettle.eCode.DBNotFound)
    ...
    async def select_one_by_key(self, _key: tNoTypeKey) -> "dNoType":
        ...
        """
        if not await self.try_select_one_by_key(_key):
            raise mettle.lib.xMettle(f"Not found: [table:dNoType, row:{self.irec}]", xMettle.eCode.DBNotFound)
    ...
    async def update(self, irec: tNoType = None):
        ...
        async with dNoTypeUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tNoType = None):
        ...
        async with dNoTypeInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
    ...
    async def delete_one(self, irec: tNoTypeKey = None):
        ...
        async with dNoTypeDeleteOne(self._dbcon) as _qry:
            await _qry.exec(_key)
