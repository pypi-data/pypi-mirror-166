...
from .utils import deprecated_pickleObj, sizeOfFmt, printError, printHtml, getEnvOrDefault, deprecated_unpickleObj, isHexNotebook
from .environment import getInstalledPythonVersion, ALLOWED_PY_VERSIONS, listMissingPackages, mismatchedPackageWarning, differentPythonVerWarning
from .ux import LIST_TITLES_WARNINGS, errorStyle, makeTitledList, linkStyle
...
def _maybeGetEnvironmentConflictMessage():
  warnings: List[str] = []
  ...
  return makeTitledList("Heads up!", errorStyle(), warnings, LIST_TITLES_WARNINGS)
...
def _getAuthenticatedMsg():
  ...
  return (f"<div>You're {connectedTag} to Modelbit as {email}. Workspace: {workspace}. {inRegion}</div>" +
          _maybeGetUpgradeMessage() + _maybeGetEnvironmentConflictMessage())
...
def _getLoginMessage():
  ...
  aTag = f'<a style="{linkStyle()}" href="{linkUrl}" target="_blank">{displayUrl}</a>'
  helpTag = f'<a style="{linkStyle()}" href="https://doc.modelbit.com/" target="_blank">Learn more.</a>'
  return (
