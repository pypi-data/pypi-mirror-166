...
class IOTBaseCommon:
    ...
    class IOTNetworkBase:
        ...
        def receive(self, socket: socket, length: Optional[int] = None):
            '''接收固定长度的字节数组'''
            if length == 0:
            ...
            try:
                if isinstance(length, int):
                    while len(data) < length:
                        data.extend(socket.recv(length - len(data)))
                else:
        ...
        def send(self, socket: socket, data: bytes):
            '''发送消息给套接字，直到完成的时候返回'''
            try:
        ...
        def receive_msg(self, socket: socket, net_msg):
            ...
            if isinstance(net_msg, IOTBaseCommon.IOTNetMessage):
                ...
                head = self.receive(socket, net_msg.get_head_length())
                ...
                if content_length == 0:
                    net_msg.contens = bytearray(0)
                else:
                    content_result = self.receive(socket, content_length)
                    if content_result.is_success is False:
                        result.copy(content_result)
                        return result
                    net_msg.contens = content_result.contents[0]

                if net_msg.contens is None:
                    net_msg.contens = bytearray(0)
                return IOTBaseCommon.IOTNetResult.create_success([IOTBaseCommon.DataTransform.cat_bytes(net_msg.heads, net_msg.contens)])
    ...
    class IOTNetworkClient(IOTNetworkBase):
        ...
        def read_from_socket(self, socket: socket, send: bytearray, has_response: bool = True, pack_unpack: bool = True):
            ...
            send_result = self.send(socket, send_value)
            ...
            # 接收数据信息
            result_receive = self.receive_msg(socket, net_msg)
    ...
    class IOTNetworkServer(IOTNetworkBase):
        ...
        def receive_client(self, socket: socket):
            '''在其他指定的套接字上，使用报文来通讯，传入需要发送的消息，返回一条完整的数据指令'''
            while self.iot_socket is not None:
                net_msg = self.get_net_msg()
                ...
                self.extra_on_receive(socket, result_receive.contents[0])
