...
class Website:
    ...
    def write_metadata(self, stream_names):
        ...
        # TODO: Get all last actif website in last minute
        current_ts = parsed_key.get("ts")
        prev_unix_ts = datetime.fromtimestamp(current_ts / 1000) - timedelta(minutes=1)
        ids = execute('FT.SEARCH', 'I::W:::', f'@last_visited:[{prev_unix_ts} {current_ts}]', 'RETURN', '1', 'id')

        # TODO: write into 'notification streams' => [ids]
        tracker_log(f'Notifiy Merger Services at {current_ts} with ids {ids} ', f'Website - ')
