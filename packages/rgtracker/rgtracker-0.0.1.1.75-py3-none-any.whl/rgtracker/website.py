...
class Website:
    ...
    def log_new_minute(self):
        tracker_log(f'{parse_key_name(self.dimension_ts_key).get("ts")} - Create new bucket, Incr metrics, update last visited, send message to other services', f'Website - ')
...
def load_website(website, section, page, device, output_stream_names):
    if website.is_exists() != 1:
        ...
        if website.has_metrics() < 2:
            # New Minute? Write to TS? Is it always the same interval?
