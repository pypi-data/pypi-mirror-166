...
GB("StreamReader", desc=json.dumps(desc_json)). \
    foreach(lambda record: tracker_log(f'{record.get("value")}', f'{job.get("name")} - ')). \
    aggregate([],
              lambda a, r: a + [r['value']],
              lambda a, r: a + r). \
    foreach(lambda record: tracker_log(f'Group({record.get("key")}):{len(record.get("value"))}\n{record.get("value")}', f'{job.get("name")} - ', f'{job.get("name")} - ')). \
    map(lambda records: transform(records, job.get('number_of_rotated_keys'))). \
    foreach(lambda records: tracker_log(f'ids:{len(records.get("id"))} merged:{len(records.get("merge"))} reinjected:{len(records.get("reinject"))}\n{records}', f'{job.get("name")}-')). \
