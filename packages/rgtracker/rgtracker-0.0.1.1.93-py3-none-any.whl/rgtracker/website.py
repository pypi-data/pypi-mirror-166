...
class Website:
    ...
    def write_metadata(self, stream_names):
        ...
        tracker_log(f'Notifiy Merger Services at {current_ts} with alive ids[{prev_unix_ts} {current_ts}] {ids} ', f'Website - ')
        execute('XADD', stream_names.get('website_pageviews'), 'MAXLEN', '8640', '*', 'ts', parsed_key.get("ts"), 'ids', json.dumps(ids), 'merged_key', self.metric_pageviews_key)
        execute('XADD', stream_names.get('website_unique_devices'), 'MAXLEN', '8640', '*', 'ts', parsed_key.get("ts"), 'ids', json.dumps(ids), 'merged_key', self.metric_unique_device_key)
        ...
        # # tracker_log(
        # #     f'XADD {stream_names.get("website_pageviews")} * dimension {Dimension.WEBSITE.value} id {self.id} ts {parsed_key.get("ts")} merged_key {self.metric_pageviews_key}',
        # #     f'Website - ')
        ...
        # # tracker_log(
        # #     f'XADD {stream_names.get("website_unique_devices")} * dimension {Dimension.WEBSITE.value} id {self.id} ts {parsed_key.get("ts")} merged_key {self.metric_unique_device_key}',
        # #     f'Website - ')
    ...
    def log_new_minute(self):
        tracker_log(f'NEW MINUTE - {parse_key_name(self.dimension_ts_key).get("ts")}', f'Website - ')
