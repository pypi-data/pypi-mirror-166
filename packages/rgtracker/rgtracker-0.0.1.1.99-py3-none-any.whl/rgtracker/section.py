...
class Section:
    ...
    def write_metadata(self, stream_names):
        ...
        # tracker_log(f'Notify Merger Services at {current_ts} with alive ids[{prev_unix_ts} {current_ts}] {ids} ', f'Section - ')
        execute('XADD', stream_names.get('section_pageviews'), 'MAXLEN', '864000', '*', 'ts', parsed_key.get("ts"), 'ids', json.dumps(ids))
        execute('XADD', stream_names.get('section_unique_devices'), 'MAXLEN', '864000', '*', 'ts', parsed_key.get("ts"), 'ids', json.dumps(ids))
...
def load_section(website, section, page, device, output_stream_names):
    if section.is_exists() != 1:
        ...
        section.notification_of_active_websites(stream_names=output_stream_names)
