...
class Website:
    ...
    def write_metadata(self, stream_names):
        ...
        tracker_log(f'Notify Merger Services at {current_ts} with alive ids[{prev_unix_ts} {current_ts}] {ids} ', f'Website - ')
        execute('XADD', stream_names.get('website_pageviews'), 'MAXLEN', '8640', '*', 'ts', parsed_key.get("ts"), 'ids', json.dumps(ids))
        execute('XADD', stream_names.get('website_unique_devices'), 'MAXLEN', '8640', '*', 'ts', parsed_key.get("ts"), 'ids', json.dumps(ids))
