...
class Website:
    ...
    def write_metadata(self, stream_names):
        ...
        execute('XADD', stream_names.get('website_pageviews'), 'MAXLEN', '8640', '*', 'ts', parsed_key.get("ts"), 'ids', *ids, 'merged_key', self.metric_pageviews_key)
        execute('XADD', stream_names.get('website_unique_devices'), 'MAXLEN', '8640', '*', 'ts', parsed_key.get("ts"), 'ids', *ids, 'merged_key', self.metric_unique_device_key)
