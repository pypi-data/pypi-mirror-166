...
class Website:
    ...
    def write_metadata(self, stream_names):
        ...
        tracker_log(f'Notify Merger Services at {current_ts} with alive ids[{prev_unix_ts} {current_ts}] {ids} ',
                    f'Website - ')
        execute('XADD', stream_names.get('website_pageviews'), 'MAXLEN', '8640', '*', 'ts', parsed_key.get("ts"), 'ids',
                json.dumps(ids))
        execute('XADD', stream_names.get('website_unique_devices'), 'MAXLEN', '8640', '*', 'ts', parsed_key.get("ts"),
                'ids', json.dumps(ids))

    def notify_new_website(self, stream_names):
        parsed_key = parse_key_name(self.dimension_ts_key)
        tracker_log(f'Notify Merger Services at {parsed_key.get("ts")} with new id {self.id} ', f'Website - ')
        execute('XADD', stream_names.get('website_pageviews'), 'MAXLEN', '8640', '*', 'ts', parsed_key.get("ts"), 'ids',
                self.id)
        execute('XADD', stream_names.get('website_unique_devices'), 'MAXLEN', '8640', '*', 'ts', parsed_key.get("ts"),
                'ids', self.id)
...
def load_website(website, section, page, device, output_stream_names):
    if website.is_exists() != 1:
        ...
        # Update last_visited here to have new site into current minute?
        # website.write_metadata(stream_names=output_stream_names)
        website.notify_new_website(stream_names=output_stream_names)
