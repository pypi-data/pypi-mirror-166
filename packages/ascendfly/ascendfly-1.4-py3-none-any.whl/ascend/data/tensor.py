...
def imgs2tensor(ctx, imgs, tensor_fmt='NCHW', tensor_ptr=None):
    ...
    if not tensor_ptr:
        tensor = AscendArray(
            tensor_shape, dtype=imgs[0].dtype, format=tensor_fmt, context=ctx)
    ...
    for i, img in enumerate(imgs):
        ...
        if tensor_fmt == 'NCHW' and img.format not in yuv420:
            # swap channel using transform operator, only work for yuv422, yuv444, rgb
            '''
            to do transformer
            '''
            pass
