...
def mem_copy(dst_ptr, src_ptr, size, method):
    ...
    if not isinstance(src_ptr, int):
        raise TypeError(f"Input src_ptr expects an int, but got {type(src_ptr)}.")

    if not isinstance(size, int):
        raise TypeError(f"Input size expects an int, but got {type(size)}.")
...
class Memory():
    ...
    def mem_info(self, attr):
        ...
        frees, total, ret = acl.rt.get_mem_info(attr)
        if ret != const.ACL_SUCCESS:
