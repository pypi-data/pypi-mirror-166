...
def greater_than(lower_bound: Number) -> Validator[Number]:
    return Validator[Number](
        lambda x: x > lower_bound, f"must be greater than {lower_bound}"
    )


def lower_than(upper_bound: Number) -> Validator[Number]:
    return Validator[Number](
        lambda x: x < upper_bound, f"must be lower than" f" {upper_bound}"
    )


def is_positive() -> Validator[Number]:
    return Validator[Number](lambda x: x > 0, "must be positive")
