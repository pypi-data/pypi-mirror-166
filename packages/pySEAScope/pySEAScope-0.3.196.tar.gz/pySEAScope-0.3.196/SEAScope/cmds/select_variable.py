...
import SEAScope.API.SelectVariable
...
def serialize(builder, target, selected, exclusive):
    """
    ...
    target : dict
        A dict which identifies the variable to select/unselect. It must
    ...
    cls = SEAScope.API.SelectVariable
    cls.SelectVariableStart(builder)
    cls.SelectVariableAddId(builder, _rid)
    cls.SelectVariableAddSelected(builder, selected)
    cls.SelectVariableAddExclusive(builder, exclusive)
    cmd_args = cls.SelectVariableEnd(builder)

    opcode = SEAScope.API.OpCode.OpCode().selectVariable
    args_type = SEAScope.API.CommandArgs.CommandArgs().SelectVariable
