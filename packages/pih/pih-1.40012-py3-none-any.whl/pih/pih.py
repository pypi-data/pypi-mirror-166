...
class PIH:
    ...
    class RESULT:
        ...
        class MARK:
            ...
            def by_name(value: str, first_item: bool = False) -> Result[List[Mark]]:
                return DataTools.to_result(RPC_COMMANDS.MARK.get_mark_by_person_name(value), Mark, first_item)
    ...
    class CHECK:
        ...
        class TIME_TRACKING:
            ...
            @staticmethod
            def available() -> bool:
                user_list: List[User] = PIH.RESULT.USER.in_group(CONST.AD.Groups.TimeTrackingAdmin).data
    ...
    class MESSAGE_COMMAND:
        ...
        def it_notify_about_new_user(login: str, password: str) -> bool:
            ...
            return PIH.message_command(LogCommands.IT_NOTIFY_ABOUT_NEW_USER, (user.name, user.description, user.samAccountName, password, user.telephoneNumber, user.mail, mark.TabNumber))
        ...
        @staticmethod
        def task_create_email(new_user: User)-> bool:
    ...
    class ACTION:
        ...
        def generate_login(full_name: FullName) -> str:
            ...
            def show_user_which_login_is_exists_and_return_user_if_it_inactive(login_string: str) -> User:
                ...
                PR.red(f"Логин '{login_string}' занят ({user.name} - {'активный' if is_active else 'неактивный'})")
                ...
                if login_is_exists or PIH.CHECK.USER.is_exists_by_login(login_string):
                    ...
                    if login_is_exists or PIH.CHECK.USER.is_exists_by_login(login_string):
                        ...
                        if len(inactive_user_list) > 0:
                            enter_login = False
                            if PIH.INPUT.yes_no("Удалить неактивные пользователи, чтобы дать выборать логин", True):
                                user_for_remove: User = inactive_user_list[0]
                                PR.value(f"Пользователь:", user_for_remove.name)
                                if PIH.INPUT.yes_no("Удалить", True):
