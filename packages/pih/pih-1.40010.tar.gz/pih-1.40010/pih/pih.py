    ...
    class ACTION:
        ...
        def generate_login(full_name: FullName) -> str:
            ...
            def show_user_which_login_is_exists_and_return_user_if_it_inactive(login_string: str) -> User:
                user: User = ResultTools.get_first(PIH.RESULT.USER.by_login(login_string))
            ...
            if PIH.CHECK.USER.is_exsits_by_login(login_string):
                ...
                if not login_is_exists:
                    login_list.append(login_string)
                if login_is_exists or PIH.CHECK.USER.is_exsits_by_login(login_string):
                    if not login_is_exists:
                    ...
                    if login_is_exists or PIH.CHECK.USER.is_exsits_by_login(login_string):
                        ...
                        if len(inactive_user_list) > 0:
                            enter_login = False
                            if PIH.INPUT.yes_no("Удалить неактивные пользователи, чтобы освободить логин", True):
                                user_for_remove: User = inactive_user_list[0]
                                PR.red(f"Пользователь:", user_for_remove.name)
