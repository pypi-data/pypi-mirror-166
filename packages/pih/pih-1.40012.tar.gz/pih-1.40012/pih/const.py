...
class LogCommands(Enum):
    ...
    #
    IT_NOTIFY_ABOUT_NEW_USER: str = LogCommand("Добрый день, отдел Информационных технологий.\nДокументы для нового пользователя: {name} готовы!\nОписание: {description}\nЛогин: {login}\nПароль: {password}\nТелефон: {telephone}\nКорпоративная почта: {email}\nНомер карты доступа: {tab_number}", LogChannel.IT_BOT, LogLevel.NOTIFICATION.value, (ParamItem(
        "name", ""), ParamItem("user_description", ""), ParamItem("login", ""), ParamItem("password", ""), ParamItem("telephone", ""),  ParamItem("email", ""),  ParamItem("tab_number", ""))),
