...
class Wrapper:
    ...
    def __init__(self, path_to_exe: str, glob_pattern: str,
        ...
        verbosity : int, optional
            ...
            0 : no verbosity - all output is captured by subprocess and not printed. Both stdout and stderr can later be extracted from proc by `proc.communicate()` (which waits for the process to finish).
...
class CacheWorkspace:
    ...
    def cleanup(self) -> None:
        ...
        '''
        self.ws.ws_release(self.id).wait()
        self.name = None
...
def save_to_project(save_fn: callable, file_path: str, *args, file_path_arg_pos: int = 0,
                    cache_workspace: CacheWorkspace = None, path_to_datamover: str = '/sw/taurus/tools/slurmtools/default/bin/', path_to_workspace_tools: str = '/usr/bin/', **kw):
    ...
    with tempfile.TemporaryDirectory(prefix=cache_workspace.name + '/') as tmp:
        ...
        waitfor(dm.dtcp(str(temp_file), str(full_path)))
...
def waitfor(proc, timeout_in_s: float = -1,
    ...
    if discard_output:
        ...
        warning_message = 'process: {}\nexited with error: {}\nand output: {}'.format(
            ' '.join([str(arg) for arg in proc.args]), err, out)
    else:
        warning_message = 'process: {}\nexited with error.'.format(' '.join([str(arg) for arg in proc.args]))
    if exit_code > 0:
