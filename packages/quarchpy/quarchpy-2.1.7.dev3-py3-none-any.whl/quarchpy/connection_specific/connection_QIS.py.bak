...
#            time.sleep(0.001)
    ...
    def create_dir_structure(self, folder_name=None, directory=None):
        """
        Creates the QPS directory structure and (empty) files to be written to

        :param folder_name: String - Name of folder for QPS stream
        ...
        self.qps_stream_folder_name = f"{folder_name}-{time.time()}"

        in_foler = "data000"

        path = os.path.join(directory, self.qps_stream_folder_name)
        os.mkdir(path)
        inner_path = os.path.join(directory, self.qps_stream_folder_name, in_foler)
        os.mkdir(inner_path)
        ...
        for i in range(8):
            ...
            f = open(os.path.join(inner_path, file_name), "w")
            f.close()
        logging.debug("Finished creating qps data files")

        logging.debug("Creating qps upper level files")
        f = open(os.path.join(path, "annotations.xml"), "w")
        f.close()
        f = open(os.path.join(path, "data000.idx"), "wb")
        f.close()
        f = open(os.path.join(path, "notes.txt"), "w")
        f.close()
        f = open(os.path.join(path, "triggers.txt"), "w")
        f.close()
