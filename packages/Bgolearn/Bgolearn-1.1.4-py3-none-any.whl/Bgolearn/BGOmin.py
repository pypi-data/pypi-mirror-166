...
class Global_min(object):
    def __init__(self,Kriging_model,data_matrix, Measured_response, virtual_samples, opt_num, ret_noise):
        ...
        if ret_noise == 0:
            ...
            self.virtual_samples_mean, self.virtual_samples_std = self.Kriging_model().fit_pre(
                self.data_matrix, self.Measured_response, self.virtual_samples,_)
    ...
    def EI_plugin(self,):
        if self.ret_noise == 0:
            ...
            __train_ypre,_ = self.Kriging_model().fit_pre(self.data_matrix,self.Measured_response, self.data_matrix,_)
    ...
    def Augmented_EI(self, alpha = 1, tao = 0):
        ...
        if self.ret_noise == 0:
            ...
            Pre_response_means, Pre_response_std = self.Kriging_model().fit_pre(self.data_matrix,self.Measured_response, self.data_matrix,_)
    ...
    def Reinterpolation_EI(self, ):
        if self.ret_noise == 0: 
            ...
            __update_y,_ = self.Kriging_model().fit_pre(self.data_matrix,self.Measured_response, self.data_matrix,_)
        ...
        if self.ret_noise == 0: 
            ...
            __y,__std = self.Kriging_model().fit_pre(self.data_matrix,__update_y, self.virtual_samples,_)
        REI_list = []
