...
class RPCTask:
    def __init__(self, name, *args, **kwds):
        ...
        self.kwds = kwds
        self.channel = name
        LOG.info(f"RPCTask name: <{name}> args: <{args}> kwds: <{kwds}>")

    def run(f, *args, **kwds):
        # enqueue args/kwds now
        _channel.queue_declare(queue=f"{f.channel}")
        _channel.basic_publish(
            exchange="message",
            routing_key="example.text",
            body=json.dumps({"args": args, "kwds": kwds}),
        )
...
def task(channel):
    def task_wrapper(f):
        if channel not in _CHANNELS:
            _CHANNELS.update({channel: f})

        f.rpc = RPCTask(channel).run
        f.channel = channel

        @wraps(f)
        def wrapper(*args, **kwds):
            # do before:
            print(f"f is {f} and uses channel {channel} (non-rpc call)")
...
class Consumer(object):
    ...
    def on_connection_closed(self, _unused_connection, reason):
        ...
        if self._closing:
            ...
            LOG.warning(f"Connection closed, reconnect necessary: {reason}")
            self.reconnect()
    ...
    def on_channel_closed(self, channel, reason):
        ...
        """
        LOG.warning(f"Channel {channel} was closed: {reason}")
    ...
    def setup_exchange(self, exchange_name):
        ...
        """
        LOG.info(f"Declaring exchange: {exchange_name}")
    ...
    def on_exchange_declareok(self, _unused_frame, userdata):
        ...
        """
        LOG.info(f"Exchange declared: {userdata}")
    ...
    def setup_queue(self, queue_name):
        ...
        """
        LOG.info(f"Declaring queue {queue_name}")
    ...
    def on_queue_declareok(self, _unused_frame, userdata):
        ...
        """
        queue_name = userdata
        LOG.info(f"Binding {self.EXCHANGE} to {queue_name} with {self.ROUTING_KEY}")
    ...
    def on_bindok(self, _unused_frame, userdata):
        ...
        :param str|unicode userdata: Extra user data (queue name)
        """
        LOG.info(f"Queue bound: {userdata}")
        self.set_qos()

    def set_qos(self):
        """This method sets up the consumer prefetch. The consumer must
        acknowledge the message(s) before RabbitMQ will deliver more.
    ...
    def on_basic_qos_ok(self, _unused_frame):
        ...
        """
        LOG.info(f"QOS set to: {self._prefetch_count}")
    ...
    def on_consumer_cancelled(self, method_frame):
        ...
        """
        LOG.info(f"Consumer was cancelled remotely, shutting down: {method_frame}")
        if self._channel:
            self._channel.close()
    ...
    def acknowledge_message(self, delivery_tag):
        ...
        """
        LOG.info(f"Acknowledging message {delivery_tag}")
    ...
    def on_cancelok(self, _unused_frame, userdata):
        ...
        """
        self._consuming = False
        LOG.info(f"RabbitMQ acknowledged the cancellation of the consumer: {userdata}")
        self.close_channel()
...
class ReconnectingConsumer(object):
    """This is a consumer that will reconnect if the nested Consumer indicates
    that a reconnect is necessary.
    """
    ...
    def _maybe_reconnect(self):
        if self._consumer.should_reconnect:
            ...
            LOG.info(f"Reconnecting after {reconnect_delay} seconds")
