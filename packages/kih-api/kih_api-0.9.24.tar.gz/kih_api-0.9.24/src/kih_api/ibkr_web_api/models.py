...
class Instrument:
    ...
    str, name: str, open_price: str, close_price: str, high_price: str, low_price: str, last_price: str,
        ...
        if change_in_percentage is not None:
            self.change_in_percentage = common.get_number_from_text_with_suffixes(change_in_percentage)

        if change_in_currency is not None:
            self.change_in_currency = common.get_number_from_text_with_suffixes(change_in_currency)

        if bid_price is not None:
            self.bid_price = common.get_number_from_text_with_suffixes(bid_price)

        if bid_size is not None:
            self.bid_size = common.get_number_from_text_with_suffixes(bid_size)

        if ask_price is not None:
            self.ask_price = common.get_number_from_text_with_suffixes(ask_price)

        if ask_size is not None:
            self.ask_size = common.get_number_from_text_with_suffixes(ask_size)

        if volume is not None:
            self.volume = common.get_number_from_text_with_suffixes(volume)

        if dividend_amount is not None:
            self.dividend_amount = common.get_number_from_text_with_suffixes(dividend_amount)

        if earnings_per_share is not None:
            self.earnings_per_share = common.get_number_from_text_with_suffixes(earnings_per_share)

        if price_to_earnings_ratio is not None:
            self.price_to_earnings_ratio = common.get_number_from_text_with_suffixes(price_to_earnings_ratio)

        if market_capitalization is not None:
            self.market_capitalization = common.get_number_from_text_with_suffixes(market_capitalization)

        if open_price is not None:
            self.open_price = common.get_number_from_text_with_suffixes(open_price)

        if low_price is not None:
            self.low_price = common.get_number_from_text_with_suffixes(low_price)

        if high_price is not None:
            self.high_price = common.get_number_from_text_with_suffixes(high_price)

        if close_price is not None:
            self.close_price = common.get_number_from_text_with_suffixes(close_price)

        if option_volume is not None:
            self.option_volume = common.get_number_from_text_with_suffixes(option_volume)

        if dividend_yield is not None:
            self.dividend_yield = common.get_number_from_text_with_suffixes(str(dividend_yield).replace("%", ""))
