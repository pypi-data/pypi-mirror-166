...
class Perforations(BaseModel):
    ...
    def period_array(self, freq:str='M', as_int=True, fields=None):
        if self.perforations:
            df = pd.concat([self.perforations[f].period_array(freq=freq, as_int=as_int,fields=fields) for f in self.perforations], axis=0)
            return df
        return None
    
    def period_array_open(self, freq:str='M', fields=None):
        groupers = ['index'] if fields is None else ['index'] + fields
        period_array = self.period_array(freq=freq, as_int=True,fields=fields).reset_index()
        period_array['md_open'] = period_array['status'] * period_array['md_tick']
        period_array['tvd_open'] = period_array['status'] * period_array['tvd_tick']
        return period_array.groupby(groupers)[['md_tick','tvd_tick']].sum()
