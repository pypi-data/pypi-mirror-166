...
def main():
  ...
  find . -name '13CrMo4_RT.*' -exec rff.py {} \;
  '''
  def printHelp():
    ...
    print('r <i> <s>    : store text string s of item starting at address i; this automatically sets the important flag and the probability to 100.')
    print('d <i>        : draw the data found starting at i')
    print('y <seq>      : remove list of sections ')
    print('t <c> <s> <e>: mark periodicity in multiTest file: c-position of count; s-postion of periodicity start; e-position of periodicity end')
  ...
  if len(sys.argv)<2:
    ...
    for idx in range(numCommands):
      ...
      if command.startswith('s'):
        ...
        methodOrder = 'x_z_a_p' if len(command.split())==1 else command.split()[1]
