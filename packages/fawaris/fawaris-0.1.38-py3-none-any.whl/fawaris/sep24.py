...
from stellar_sdk.xdr.payment_result import (
    PaymentResult,
)
from stellar_sdk.xdr.path_payment_strict_send_result import (
    PathPaymentStrictSendResult,
)
from stellar_sdk.xdr.path_payment_strict_receive_result import (
    PathPaymentStrictReceiveResult,
)
from stellar_sdk.xdr.operation_result import (
    OperationResult,
)
from stellar_sdk.xdr.transaction_result import (
    TransactionResult,
)
...
class Sep24(ABC):
    ...
    async def http_post_transactions_withdraw_interactive(
        ...
        try:
            ...
            raise ValueError(
                f"Withdrawal is not enabled for asset {request.asset_code}"
            )
        tx = await self.create_transaction(request, token)
        url = await self.get_interactive_url(request, token, tx)
        return Sep24PostResponse(
    ...
    async def watch_withdrawals_to_receive(self) -> None:
        ...
        accounts = set(
            [
                ...
                for withdrawal in withdrawals_to_receive
            ]
        )
        await asyncio.gather(
            *[self.stream_withdraw_anchor_account(account) for account in accounts],
        )
    ...
    async def find_matching_payment_data(
        ...
        for idx, op_result in enumerate(result_ops):
            ...
            if maybe_payment_data:
                if ops[idx].source:
                    _source = (
                        ops[idx].source.account_muxed or ops[idx].source.account_id
                    )
                else:
                    _source = (
