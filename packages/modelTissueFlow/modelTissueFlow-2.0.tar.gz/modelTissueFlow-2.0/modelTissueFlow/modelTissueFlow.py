...
class Genotype1D:
    ...
    def __init__(self,*args,**kwarg):    
        if args:
            parametersFileName,self.ID,self.frameIndex_Maps,self.transition_cutOff_val,self.path = args
    ...
    # analyze-genotype #
    ####################
    def analysis_GENOTYPE(self,outPutDir):
        print('analysing genotype:',self.ID)
        animal_Store = []
        for animal_ID,frameIndex_Map in self.frameIndex_Maps.items(): 
            ANIMAL = self.analysis_ANIMAL(animal_ID,frameIndex_Map,self.path+'/'+self.ID)
        ...
        self.vitelline_space_alligned = vitelline_space_alligned_List
        ...
        self.vitelline_space_not_alligned = vitelline_space_raw_List
        self.transition_indiv_frame_reference = seg_frames_before_transition_List
        self.transition_merge_frame_reference = transition_frame_reference
        ...
        for time_counter,time_series_data in enumerate(time_animalTypes_average_data):
            ...
            self.update(time_val,s_avg,s_std,eh_avg,eh_std,eh_curv_avg,eh_curv_std,mom_avg,mom_std,curv_avg,curv_std,s_ref_avg,s_ref_std,piv_tan_avg,piv_tan_std,piv_tanGrad_avg,piv_tanGrad_std,piv_tanGrad_correction_avg,piv_tanGrad_correction_std,piv_norm_avg,piv_norm_std,total_myo_avg,total_myo_std,basal_myo_avg,basal_myo_std,apical_myo_avg,apical_myo_std,mom_curvGrad_avg,mom_curvGrad_std,curv_momGrad_avg,curv_momGrad_std,basal_myoGrad_avg,basal_myoGrad_std,total_myoGrad_avg,total_myoGrad_std,apical_myoGrad_avg,apical_myoGrad_std,basal_mom_curvGrad_avg,basal_mom_curvGrad_std,curv_basal_myoGrad_avg,curv_basal_myoGrad_std,apical_mom_curvGrad_avg,apical_mom_curvGrad_std,curv_apical_myoGrad_avg,curv_apical_myoGrad_std,piv_correction_factor_Grad_avg,piv_correction_factor_Grad_std,myo_frames_alligned[time_counter],mid_markers_alligned[time_counter])           
        # save-genotype-information 
        inOutTools.createDirectory(outPutDir)
        inOutTools.dump_data_via_pickle(outPutDir+'/output_'+self.ID,self)    
        return 

    ##################
    # analyze-animal #
    ##################
    def analysis_ANIMAL(self,animal_ID,frameIndex_Map,sysPath):
        print('analysing animal:',animal_ID)
        animal_reference,raw_frames = frameIndex_Map
        ...
        # initialize-animal #
        #*******************#
        ANIMAL = Animal1D(animal_ID,sysPath,posterior_domain,initial_frame,segmented_frames,reMarker_Number,apical_marker_position_off_set,basal_marker_position_off_set,posterior_pole_location,epithelium_orientation)
