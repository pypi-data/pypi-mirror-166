...
def stack_data(data, subjects_variable='', electrode_variable='component', single=False):
    '''
    ...
    if single:
        ...
        trial = {'trial':data.stack(trial=['participant','epochs']).dropna(dim="trial", how='all')}
        starts = xr.DataArray(starts, coords=trial)
        ends = durations-1
        ends = xr.DataArray(ends, coords=trial)
