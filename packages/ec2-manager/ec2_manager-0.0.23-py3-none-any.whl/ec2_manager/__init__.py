...
class EC2Manager:
    def __init__(self, **kwargs):
        ...
        self._github_email = os.environ.get('GITHUB_EMAIL', kwargs.get('github_email'))
    ...
    def _update_repo(self, name):
        ...
        self.run_command(name, [
            f'git config --global user.name "{self._github_user}"',
            f'git config --global user.email "{self._github_email}"',
            f'git config --global --add safe.directory /usr/bin/{self._repo}',
            f'cd ./{self._repo}',
            'git fetch',
            'git pull',
