...
def test_pull_success_in_datalab():
    ...
    with mock.patch('seeq.spy._session.ValidatedRequests.get', requests_get_mock()), \
...
def test_pull_success_in_executor():
    ...
    with mock.patch('seeq.spy._session.ValidatedRequests.get', mock_requests_get), \
            mock.patch('seeq.spy.jobs._pull.get_project_from_api', get_project_from_api_mock()), \
            mock.patch('seeq.spy.jobs._pull.load_pickle', load_pickle_mock):
...
def test_pull_failure_outside_datalab():
    ...
    with mock.patch('seeq.spy._session.ValidatedRequests.get', mock.Mock(return_value=mock_get_resp_403)), \
            mock.patch('seeq.spy.jobs._pull.get_project_from_api', get_project_from_api_mock()):
