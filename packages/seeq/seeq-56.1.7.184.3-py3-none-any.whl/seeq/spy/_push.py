...
def push(data=None, *, metadata=None, replace=None, workbook=_common.DEFAULT_WORKBOOK_PATH,
    ...
    if archive:
        ...
        _archive_tree_items(session, datasource_output, workbook_id, tree_root, sync_token, push_result_df,
                            status, errors)
...
def _push_workbooks(session: Session, push_result_df, folder_id, workbooks, datasource, status: Status) -> pd.DataFrame:
...
def _archive_tree_items(session: Session, datasource_output, workbook_id, tree_root, sync_token, push_result_df,
                        status, errors):
    ...
    archive_manually = potential_count > 19_000

    if archive_manually:
        all_items = spy.search({'Datasource Class': datasource_output.datasource_class,
                                'Datasource ID': datasource_output.datasource_id,
                                'Data ID': f'/{data_id_regex}/'},  # Wrap in `/`s to denote it's a regex search
                               workbook=workbook_id,
                               status=status.create_inner('Find stale tree items'))
        status.df = pd.DataFrame([{'Items Archived': 0}])
        status.update()
        pushed_item_ids = set(push_result_df['ID'])
        for item_id in all_items['ID']:
            if item_id not in pushed_item_ids:
                try:
                    items_api.archive_item(id=item_id)
                    status.df['Items Archived'] += 1
                    status.update()
