class serverInternalHandlers():
    ...
    def ulist(self, client, message, listener_detected, listener_id, room_id):
        for room in client.rooms:
            ulist = []
            for user in self.cloudlink.getUsernames(room):
                ulist.append(user)
            self.cloudlink.sendPacket(client, {"cmd": "ulist", "val": ulist}, rooms = room)

    # Status codes (For client-to-client)
    def statuscode(self, client, message, listener_detected, listener_id, room_id):
        ...
        if type(message["id"]) == list:
            ...
            if not(len(rx_client) == 0):
                ...
                self.cloudlink.sendPacket(rx_client, {"cmd": "statuscode", "code": message["val"], "origin": self.cloudlink.getUserObjectFromClientObj(client)})
            ...
            self.cloudlink.sendPacket(rx_client, {"cmd": "statuscode", "code": message["val"], "origin": self.cloudlink.getUserObjectFromClientObj(client)}, rooms = room_id)

            # Tell the client that the status code was successfully sent
            self.cloudlink.sendCode(client, "OK", listener_detected, listener_id)

    # Ping
    def ping(self, client, message, listener_detected, listener_id, room_id):
        ...
        if type(message["id"]) == list:
            ...
            if not(len(rx_client) == 0):
                ...
                self.cloudlink.sendPacket(rx_client, {"cmd": "ping", "val": message["val"], "origin": self.cloudlink.getUserObjectFromClientObj(client)})
            ...
            self.cloudlink.sendPacket(rx_client, {"cmd": "ping", "val": message["val"], "origin": self.cloudlink.getUserObjectFromClientObj(client)}, rooms = room_id)
            # Assume that the client's recipient will respond with a statuscode
    
    # Link client to a room/rooms
    def link(self, client, message, listener_detected, listener_id, room_id):
        ...
        if type(message["val"]) in [list, str]:
            ...
            if "default" in old_rooms:
                self.cloudlink.sendPacket(self.cloudlink.getAllUsersInRoom("default"), {"cmd": "ulist", "val": self.cloudlink.getUsernames()})

            # Update all client ulists in the rooms that the client joined
            self.cloudlink.sendPacket(self.cloudlink.getAllUsersInManyRooms(message["val"]), {"cmd": "ulist", "val": self.cloudlink.getUsernames(message["val"])}, rooms = message["val"])

        else:
            self.cloudlink.sendCode(client, "Datatype", listener_detected, listener_id)
    
    # Unlink client from all rooms, and then link the client to the default room
    def unlink(self, client, message, listener_detected, listener_id, room_id):
        ...
        if client.is_linked:
            ...
            self.cloudlink.sendPacket(self.cloudlink.getAllUsersInManyRooms(old_rooms), {"cmd": "ulist", "val": self.cloudlink.getUsernames(old_rooms)}, rooms = old_rooms)
        else:
            ...
            self.cloudlink.sendPacket(client, {"cmd": "ulist", "val": self.cloudlink.getUsernames()})
    ...
    def direct(self, client, message, listener_detected, listener_id, room_id):
        # Sanity check the message
        if not "id" in message:
            self.cloudlink.sendCode(client, "Syntax", listener_detected, listener_id)
            return
        ...
        if type(message["id"]) == list:
            ...
            if not(len(rx_client) == 0):
                ...
                self.cloudlink.sendPacket(rx_client, {"cmd": "direct", "val": message["val"], "origin": self.cloudlink.getUserObjectFromClientObj(client)})
            ...
            # Send the ping to the recipient ID
            self.cloudlink.sendPacket(rx_client, {"cmd": "direct", "val": message["val"], "name": message["name"], "origin": self.cloudlink.getUserObjectFromClientObj(client)}, rooms = room_id)

            # Tell the client that the direct data was successfully sent
            self.cloudlink.sendCode(client, "OK", listener_detected, listener_id)
    
    # Global messages
    def gmsg(self, client, message, listener_detected, listener_id, room_id):
        ...
        ulist = self.cloudlink.getAllUsersInManyRooms(room_id)
        ulist.remove(client)
        msg = {
            "cmd": "gmsg",
            "val": message["val"]
    ...
    def gvar(self, client, message, listener_detected, listener_id, room_id):
    ...
    def pvar(self, client, message, listener_detected, listener_id, room_id):
        # Prevent clients without usernames from using this command
        if not client.username_set:
            self.cloudlink.sendCode(client, "IDRequired", listener_detected, listener_id)
            return
        ...
        if type(message["id"]) == list:
            ...
            if not(len(rx_client) == 0):
                ...
                self.cloudlink.sendPacket(rx_client, {"cmd": "pvar", "val": message["val"], "name": message["name"], "origin": self.cloudlink.getUserObjectFromClientObj(client)}, rooms = room_id)
    ...
    # Private messages
    def pmsg(self, client, message, listener_detected, listener_id, room_id):
        # Prevent clients without usernames from using this command
        if not client.username_set:
            self.cloudlink.sendCode(client, "IDRequired", listener_detected, listener_id)
            return
        ...
        if type(message["id"]) == list:
            ...
            if not(len(rx_client) == 0):
                ...
                self.cloudlink.sendPacket(rx_client, {"cmd": "pmsg", "val": message["val"], "origin": self.cloudlink.getUserObjectFromClientObj(client)})
            ...
            self.cloudlink.sendPacket(rx_client, {"cmd": "pmsg", "val": message["val"], "origin": self.cloudlink.getUserObjectFromClientObj(client)}, rooms = room_id)

            # Tell the client that the message was successfully sent
            self.cloudlink.sendCode(client, "OK", listener_detected, listener_id)
    
    # Set username
    def setid(self, client, message, listener_detected, listener_id, room_id):
    ...
    # WIP
    def relay(self, client, message, listener_detected, listener_id, room_id):
        pass