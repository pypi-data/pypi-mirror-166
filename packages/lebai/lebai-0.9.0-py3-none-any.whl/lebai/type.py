...
class CartesianPose:
    ...
    def __init__(self, x=0, y=0, z=0, rz=0, ry=0, rx=0, base=None):
        ...
        if type(x) is msg.PR: # rpy -> ypr
            self.pos = [x.position.x, x.position.y, x.position.z,
                        x.rotation.y, x.rotation.p, x.rotation.r]
        elif hasattr(x, 'pos'):
            self.pos = list(x.pos[:])
            if hasattr(x, 'base'):
                base = x.base
    ...
    def _to_PR(self):
        ...
        r = msg.Rotation(y=self.pos[3], p=self.pos[4], r=self.pos[5]) # rpy -> ypr
        return msg.PR(position=p, rotation=r)

    def _base_set_PR(self, pr): # rpy -> ypr
        if self.base is not None:
            pr.position.x = self.base[0]
            pr.position.y = self.base[1]
            pr.position.z = self.base[2]
            pr.rotation.y = self.base[3]
            pr.rotation.p = self.base[4]
            pr.rotation.r = self.base[5]
