...
from ..models import UserToGroup, RoleGroup
...
class GroupTreeService(object):
    ...
    def getTrees(data_list, parent_group):
        tree = []
        if parent_group != 0:
            a = RoleGroup.objects.filter(parent_group_id=parent_group).to_json()
            for item in data_list:
                if str(item['parent_group_id']) == str(parent_group):
                    item['children'] = GroupTreeService.getTree(data_list, item['id'])
                    tree.append(item)
    ...
    def group_tree(group_id=0):
        data_list = RoleGroup.objects.filter().to_json()
...
class GroupService(object):
    ...
    def group_list(params):
        ...
        group_set = RoleGroup.objects.filter(**params).values()
    ...
    def add_group(params):
        ...
        if not params:
            return None, "参数不能为空"
        instance = RoleGroup.objects.create(**params)
    ...
    def edit_group(params):
        ...
        instance = RoleGroup.objects.filter(id=id)
        if params:
