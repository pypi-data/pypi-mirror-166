...
def validate_access_token(access_token: str, subject: str, access_token_tools: AccessTokenTools) -> bool:
    ...
    try:
        ...
        payload = None
    if payload is not None and payload["app_id"] == subject:
        is_valid = True
    return is_valid
...
def test_access_token():
    ...
    with TemporaryDirectory() as temp_directory:
        ...
        with freezegun.freeze_time("1970-01-01T00:00:00"):
            ...
            token = access_token_tools.create_token(
                app_id=test_id, app_ui_url="test", expires_after_seconds=valid_period
            )
            assert type(token) == str
            assert len(token) > 0
            payload = decode_payload(token, access_token_tools)
            assert payload["app_id"] == test_id
            ...
            token3 = access_token_tools.create_token(
                app_id=test_id_2, app_ui_url="test", expires_after_seconds=valid_period
            )
            assert type(token3) == str
            assert token3 != token
            assert token3 != token2
            payload3 = decode_payload(token3, access_token_tools)
            assert payload3["app_id"] == test_id_2
