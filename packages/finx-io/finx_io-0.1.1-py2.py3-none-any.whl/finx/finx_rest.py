...
class FinXRest:
    ...
    async def prepare_vol_surface_inputs(self, filename, timeslice_datestamp, timeslice_width_seconds, underlying_symbol) -> str:
        ...
        df['years_to_expiry'] = df.apply(lambda row: self._calc_years_to_expiry(row), axis=1).astype(str)
        df['strike_price'] = df.apply(lambda row: self._parse_strike_price(row), axis=1)
        output_df = df[['years_to_expiry', 'strike_price', 'mark_iv']]
        renamed_df = output_df.rename(columns={"years_to_expiry": "expiration", "strike_price": "strike", "mark_iv": "price"})
        ...
        unique_strikes_df = renamed_df.drop_duplicates(subset=['expiration', 'strike'], keep='last')
        sorted_df = unique_strikes_df.sort_values(by=['expiration', 'strike']).copy()
        grouped_df = sorted_df.groupby(by=['expiration'], as_index=False).count()
        expiration_count = len(grouped_df)
        sorted_df['action'] = sorted_df.apply(lambda row: self._remove_incomplete_observation_sets(expiration_count, grouped_df.loc[grouped_df['expiration'] == row.expiration]), axis=1)
        keeper_df = sorted_df.loc[sorted_df['action'] == 'keep'].copy()
        keeper_df.to_csv(filename, columns=['expiration', 'strike', 'price'], index=False)
        return filename
