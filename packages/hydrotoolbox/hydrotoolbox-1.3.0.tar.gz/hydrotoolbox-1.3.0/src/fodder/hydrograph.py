...
def sepBaseflow(x, dt, A, k=0.000546, dt_max=None, tp_min=None):
    ...
    df_final = pd.DataFrame(dr, columns=["Date"])
    dr = None
    ...
    df_final["Total runoff [m^3 s^-1]"] = x
    x = None
    ...
    if dt_max:
        if dt == 5:
            df_final["Total runoff interp. [m^3 s^-1]"] = df_final[
                "Total runoff [m^3 s^-1]"
            ].interpolate(method="time", limit=dt_max * 12)
        elif dt == 15:
            df_final["Total runoff interp. [m^3 s^-1]"] = df_final[
                "Total runoff [m^3 s^-1]"
            ].interpolate(method="time", limit=dt_max * 4)
        else:
            df_final["Total runoff interp. [m^3 s^-1]"] = df_final[
                "Total runoff [m^3 s^-1]"
            ].interpolate(method="time", limit=dt_max)
    else:
        df_final["Total runoff interp. [m^3 s^-1]"] = df_final[
            "Total runoff [m^3 s^-1]"
    ...
    df_final["Baseflow [m^3 s^-1]"] = df_final[
        ["Baseflow [m^3 s^-1]", "Total runoff [m^3 s^-1]"]
    ].min(axis=1)
    # df_final = df_final.astype(np.float)

    df_final["Peakflow [m^3 s^-1]"] = (
        df_final["Total runoff interp. [m^3 s^-1]"] - df_final["Baseflow [m^3 s^-1]"]
    )
    ...
    df_final = pd.merge(df_final, df, how="left", on="Peak nr.")
    # -Flow volume
    df_final["Flow volume [m^3]"] = (
        df_final["Total runoff interp. [m^3 s^-1]"] * 3600 * df_final["dt [hour]"]
    )
    ...
    for i in pd.unique(df_final["Peak nr."]):
        df_short = df_final.loc[
            df_final["Peak nr."] == i, ["Date", "Total runoff interp. [m^3 s^-1]"]
        ]  # ,'Max. flow [m^3 s^-1]']]
    ...
    df_final["Tp [hour]"] = (
        df_final["Date max. flow"] - df_final["Peakflow starts"]
    ).dt.seconds / 3600
    df_final.set_index("Date", inplace=True)
...
def filterpeaks(x, tp_min):
    ...
    for i in df_final.iterrows():
        ...
        if peakflow > 0.0 and oldPeakflow == 0.0:
            pcnt += 1
        ...
        if np.isnan(peakflow):
            ...
            peakflow == 0.0
        ):  # -this indicates an old event has ended and then it is time to check for nans in the peakflow
            oldPeakflow = 0.0
            j = df_final.loc[
                (df_final["Peakflow [m^3 s^-1]"] == -99.9)
                & (df_final["Peak nr."] == pcnt)
            ]
            if len(j) > 1:
                df_final.loc[
                    df_final["Peak nr."] == pcnt, ["Peakflow [m^3 s^-1]", "Peak nr."]
                ] = np.nan
                pcnt = pcnt - 1
        else:
            df_final.loc[df_final.index == i[0], "Peak nr."] = pcnt
    ...
    df_final.loc[
        df_final["Peakflow [m^3 s^-1]"] == -99.9, "Peakflow [m^3 s^-1]"
    ] = np.nan

    # -Select for minimum number of records to make it classify as a peak
    if tp_min:
        print(f"Selecting events >= {tp_min:.2f} hours")
...
def maxFlowVolStats(df):
    ...
    y_peak_vol = (
        df_new[["dt [hour]", "Flow volume [m^3]", "Peak nr."]].groupby("Peak nr.").sum()
    )
    y_peak_vol.reset_index(inplace=True)
    y_peak_vol = pd.merge(
        y_peak_vol,
        df_new[["Peak nr.", "Year max flow", "Max. flow [m^3 s^-1]"]],
        how="left",
        on="Peak nr.",
    )
    ...
    y_peak_maxvol = (
        y_peak_vol[
            ["dt [hour]", "Year max flow", "Flow volume [m^3]", "Max. flow [m^3 s^-1]"]
        ]
        .groupby("Year max flow")
        .max()
    )
    y_peak_vol = None
    ...
    vol_peak_combined = pd.merge(
        y_peak_maxvol, y_max, how="left", left_on="Year max flow", right_on="Year"
    )
    vol_peak_combined.drop("Year", axis=1, inplace=True)
    # -Calculate the average flow rate of the volume; volume/(hours*3600)
    vol_peak_combined["Avg. volume rate [m^3 s^-1]"] = vol_peak_combined[
        "Flow volume [m^3]"
    ] / (vol_peak_combined["dt [hour]"] * 3600)
    vol_peak_combined["Flow volume [MCM]"] = (
        vol_peak_combined["Flow volume [m^3]"] / 1000000
    )
