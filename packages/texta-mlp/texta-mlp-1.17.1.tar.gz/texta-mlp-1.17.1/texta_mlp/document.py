...
from .parsers import AddressParser, ContactEmailParser, ContactPhoneParserHighPrecision, ContactPhoneParserHighRecall, ContactPhoneParserStrict
...
class Document:
    ...
    @staticmethod
    def set_text_to_nested_dict(document: dict, doc_path: str, value) -> dict:
        """
        ...
        """
        keys = doc_path.split(".")
        path_to_bottom_layer = keys[:-2]
        for key in path_to_bottom_layer:
            document = document.setdefault(key, {})
        mlp_reserved_key = keys[-1]
        document[mlp_reserved_key] = value
        return document
    ...
    def document_to_json(self, use_default_doc_path=True) -> dict:
        ...
        Document.set_text_to_nested_dict(document=self.json_doc, doc_path=path_to_mlp, value=mlp_result["text_mlp"])
        Document.set_text_to_nested_dict(document=self.json_doc, doc_path="texta_facts", value=mlp_result["texta_facts"])
