...
class Crystal:
    ...
    structure : structure object
        ...
        * The exact type of the object is *pymatgen.core.structure.Structure*
              * This indicates that the structure read from CIF by means of
    ...
    def get_elements(self, structure):
        ...
        * The structure object = pymatgen.core.structure.Structure object.
        * The list of element names si obtained by a set of tricks
          specific to structure object. These tricks were revealed by
          inspecting original pymatgen.core.structure.Structure object.
...
class PXRDcalculation:
    ...
    crystal : ediff.pxrd.Crystal object
        ...
        - see Crystal class above.
    experiment : ediff.pxrd.Experiment object
        This object is usually prepared in advance
        - see Experiment class above.        
    plot_parameters : ediff.pxrd.PlotParameters object
        This object is usually prepared in advance
        - see PlotParametrs class above.
    peak_profile : None or ediff.pxrd.PeakProfiles object, optional
    ...
    peak_profile_sigma : float, optional
    ...
    def calculate_diffractogram(self):
        ...
        # (2) Line diffraction profile
        # = diffractogram with line intensities
        # ...read diffractions, rounded to 3 dec.places ~ step 0.001
        diffractions = np.round(
            np.array(self.diffractions[['TwoTheta','Ihkl']]),2)
        # ...insert line diffractions into zero profile from step (1)
        ...
        # = diffractogram with smeared intensities
        # ...prepare peak_profile
        if self.peak_profile == None:
            pass
        else:
            X = np.linspace(-10,10,2001,endpoint=True)
            peak_profile = self.peak_profile(X,0,self.peak_profile_sigma)    
    ...
    def show_diffractogram(self):
        self.show_diffractions()
        print('-----')
        print('* We show just diffractions, not the whole diffractogram.')
        print('* Reason: the diffractogram = profile is usually too long.')
        print('* Alternative: save_diffractogram saves profile to text file.')
