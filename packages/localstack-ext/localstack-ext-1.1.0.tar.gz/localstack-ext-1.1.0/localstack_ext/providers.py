...
def cognito_identity():from localstack_ext.services.cognito_identity.provider import CognitoIdentityAWSApiListener as B;A=B();return Service(_R,listener=A,lifecycle_hook=A.provider,backend_state_lifecycle=A.provider)
...
def timestream_write():from localstack_ext.services.timestream.provider import TimestreamWriteProvider as B;A=B();return Service(_N,listener=AwsApiListener(_N,A),backend_state_lifecycle=A)
@pro_aws_provider(api=_O)
def timestream_query():from localstack_ext.services.timestream.provider import TimestreamQueryProvider as B;A=B();return Service(_O,listener=AwsApiListener(_O,A),backend_state_lifecycle=A)
@pro_aws_provider()
...
def awslambda():from localstack.services.awslambda import lambda_starter as A;from localstack_ext.services.awslambda.lambda_extended import patch_lambda as B;from localstack_ext.services.awslambda.persistence import LambdaPersistenceLifeCycle as C;B();return Service(_P,start=A.start_lambda,stop=A.stop_lambda,check=A.check_lambda,backend_state_lifecycle=C(_P))
@pro_aws_provider(api=_Q)
def cloudwatch():from localstack_ext.services.cloudwatch.provider import CloudwatchProvider as B;A=B();return Service(_Q,listener=AwsApiListener(_Q,MotoFallbackDispatcher(A)),lifecycle_hook=A,backend_state_lifecycle=A)
@pro_aws_provider()
...
@pro_aws_provider()
def stepfunctions():B='stepfunctions';from localstack_ext.services.stepfunctions.stepfunctions_extended import StepFunctionsExtProvider as C,patch_stepfunctions as D;D();A=C();E=AwsApiListener(B,HttpFallbackDispatcher(A,A.get_forward_url));return Service(B,listener=E,lifecycle_hook=A,backend_state_lifecycle=A)
@pro_aws_provider()
