...
def adaptor(func, flow_id=None, *args, **kwargs):
    ...
    os.environ[f"FLOW_ID_{test_name}"] = flow_id
...
def logged_request(method, url, **kwargs):
    if os.environ["SKIP_ADAPTER"] == "False":
        ...
        test_name = os.environ.get("PYTEST_CURRENT_TEST").split(":")[-1].split(" ")[0]
        zone = os.environ.get(f"ZONE_{test_name}", None)
        flow_id = os.environ.get(f"FLOW_ID_{test_name}", None)
        ...
        try:
            ...
            # Hack for not generating token for PCI Zone for now
            if header_dict is not None and "-pcidss-" not in zone:
...
def adapter_data(*, start_time, end_time, request=None, response=None, method=None, url=None, flow_id=None, zone=None):
    if request is not None:
        ...
        if flow_id is not None and zone is not None:
            del os.environ[f"FLOW_ID_{test_name}"]
            del os.environ[f"ZONE_{test_name}"]
