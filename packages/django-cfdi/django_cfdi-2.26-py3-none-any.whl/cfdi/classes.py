    ...
    def generar_cfdi(self, configuracion, timbrado_prueba=None, pac=None):
        ...
        for rp in self.pagos:
            ...
            if rp["FormaDePagoP"] in PATRON_CUENTA_ORDENANTE:
                if not len(rp["CtaOrdenante"]) in PATRON_CUENTA_ORDENANTE[rp["FormaDePagoP"]]["patron"]:
            ...
            for documento in rp["documentos"]:
                if not documento["documento"]['cfdi_uuid']:
                    self.cfdi_instance.cfdi_status = (
                        u"La factura %s no tiene UUID o no "
                        "está timbrada." % documento["documento"]['folio_serie']
                    )
                    self.cfdi_instance.save()
                    return self.cfdi_instance

                saldo_actual = documento["documento"]['saldo']
                saldo_anterior = saldo_actual + documento["documento"]['importe_abono']
                tipo_cambio_dr = None
                equivalencia_dr = 1
                if documento["documento"]['moneda'] != rp["MonedaP"]:
                    tipo_cambio_dr = documento["documento"]['tipo_cambio']
                    equivalencia_dr =  round(to_decimal(documento["documento"]['importe_abono']) / to_decimal(rp["Monto"]), 2)

                metodo_de_pago_dr = documento["documento"].get('metodo_pago', "PPD")
                objeto_impuesto_dr = documento["documento"].get('objeto_impuesto', "01")

                documento_relacionado = {
                    "IdDocumento": documento["documento"]['cfdi_uuid'],
                    "Serie":documento["documento"]['serie'],
                    "Folio":documento["documento"]['folio'],
                    "MonedaDR":documento["documento"]['moneda'],
                    'TipoCambioDR':tipo_cambio_dr,
                    'MetodoDePagoDR':metodo_de_pago_dr,
                    'ObjetoImpDR':objeto_impuesto_dr,
                    'NumParcialidad':documento["documento"]['numero_parcialidad'],
                    'ImpSaldoAnt':"%.2f" % saldo_anterior,
                    'ImpPagado': "%.2f" % documento["documento"]['importe_abono'],
                    'ImpSaldoInsoluto':"%.2f" % saldo_actual,
                    'EquivalenciaDR': equivalencia_dr,
                    'ImpuestoDR_set': [],
                }
            
                for imp_dr in documento["impuestos_dr"]:
