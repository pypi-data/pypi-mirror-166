...
class TimeUtils:
    ...
    def datetime_strftime_now(tz_info:str="Asia/Shanghai", strftime:str="%Y-%m-%d %H:%M:%S") -> str:
        ...
        if tz_info:
            ...
            tz_info1 = pytz.timezone(tz_info)
            return datetime.datetime.utcnow().replace(tzinfo=utc_info).astimezone(tz_info1).strftime(strftime)
        else:
    ...
    @staticmethod
    def datetime_utc_now(date_type:str="seconds", timedelta:int="") -> datetime:
        """
        返回 UTC时间的偏移量 例如
