...
class TokenPixelBoardSensor(boardsensor.TokenBoardSensor):
    ...
    def detect_tokens(self, token_filter=None) -> list:
        ...
        detected_tokens = self.remove_self_from_token_list(tokens)
        if detected_tokens:
            detected_tokens = self._detect_token(detected_tokens, self.token.collision_type)
        if detected_tokens:
            detected_tokens = self.filter_token_list(detected_tokens, token_filter)
        if detected_tokens and len(detected_tokens) >= 1:
            return detected_tokens
        else:
            return []

    def detect_token(self, token_filter=None) -> Union["token_mod.Token", None]:
        self.token.board.init_display()
        tokens = pygame.sprite.spritecollide(self.token, self.token.board.camera.get_tokens_in_viewport(), False,
                                             pygame.sprite.collide_rect)
        detected_tokens = self.remove_self_from_token_list(tokens)
    ...
    def detect_tokens_at(self, direction: int = 0, distance: int = 1) -> list:
        if direction == 0:
            direction = self.token.direction
        destination = self.get_destination(self.token.center, direction, distance)
