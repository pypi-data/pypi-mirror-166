...
def create_shareuser(cli_dict):
    try:
        ...
        if permissions == 'rw':
            cli_args.append('rw')
        result, errmsg = run_syscli1('add', 'shareuser', check_command_successful, *cli_args, **cli_dict)
        if not result:
            error = build_error_model(
        ...
        cli_dict = dict(share=cli_dict['share'], user=cli_dict['user'])
        return list_shareusers(cli_dict, None, http_code.CREATED, True)
...
def list_shareusers(cli_dict, request_values=None, response_code=None, verify_one=False):
    try:
        if cli_dict.get('errors', None):
            error = build_error_model(
                error_message=build_error_message(cli_dict['errors']),
        ...
        username = cli_dict.pop('user', None)
        parse_func = parse_config_style
        result, errmsg = run_syscli1('list', 'shareuser', parse_func, **cli_dict)
...
def get_shareuser(cli_dict):
    return list_shareusers(cli_dict, None, http_code.OK, True)

def delete_shareuser(cli_dict):
    try:
        result, errmsg = run_syscli1('del', 'shareuser', check_command_successful, **cli_dict)
        if not result:
            error = build_error_model(
            error_message=[errmsg],
            error_code='COMMAND FAILED')
        ...
        data = dict(message=messages.DELETE_SUCCESS_MSG.format(cli_dict.get('user')))
...
def delete_shareusers(cli_dict):
    try:
        cli_args = ['sure']
        result, errmsg = run_syscli1('deleteall', 'shareuser', check_command_successful, *cli_args, **cli_dict)
        if not result:
            error = build_error_model(
            error_message=[errmsg],
            error_code='COMMAND FAILED')
            return (build_entity_response(error=error), 400)
        output = 'all users for '.format(cli_dict.get('share'))
