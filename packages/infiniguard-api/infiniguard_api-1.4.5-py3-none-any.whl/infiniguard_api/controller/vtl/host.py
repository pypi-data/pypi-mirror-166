...
def create_host(cli_dict):
    try:
        ...
        result, errmsg = run_syscli1('add', 'host', check_command_successful, **cli_dict)
        if not result:
            error = build_error_model(
        ...
        cli_dict = dict(wwpn=cli_dict['wwpn'])
        return list_hosts(cli_dict, None, http_code.CREATED, True)
...
def list_hosts(cli_dict, request_values=None, response_code=None, verify_one=False):
    try:
        if cli_dict.get('errors', None):
            error = build_error_model(
                error_message=build_error_message(cli_dict['errors']),
        ...
        wwpn = cli_dict.pop('wwpn', None)
        parse_func = parse_config_style
        result, errmsg = run_syscli1('list', 'host', parse_func, **cli_dict)
...
def get_host(cli_dict):
    return list_hosts(cli_dict, None, http_code.OK, True)

def update_host(cli_dict):
    try:
        if cli_dict.get('errors', None):
            error = build_error_model(
                error_message=build_error_message(cli_dict['errors']),
        ...
        wwpn = cli_dict.get('wwpn', None)
        cli_dict['wwpn'] = wwpn

        result, errmsg = run_syscli1('edit', 'host', check_command_successful, **cli_dict)
        if not result:
            error = build_error_model(
            error_message=[errmsg],
            error_code='COMMAND FAILED')
...
def delete_host(cli_dict):
    try:
        result, errmsg = run_syscli1('del', 'host', check_command_successful, **cli_dict)
        if not result:
            error = build_error_model(
            error_message=[errmsg],
            error_code='COMMAND FAILED')
        ...
        data = dict(message=messages.DELETE_SUCCESS_MSG.format(cli_dict.get('wwpn', None)))
