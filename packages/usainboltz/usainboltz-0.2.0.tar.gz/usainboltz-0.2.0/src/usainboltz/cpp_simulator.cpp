...
CRule *make_union(const std::vector<CRule *> &args,
  ...
  new (&rule->_as_union.args) std::vector<CRule *>(args);
  new (&rule->_as_union.weights) std::vector<std::vector<double>>(weights);
  rule->_as_union.weights = weights;
...
CRule *make_seq(const CRule *arg, const std::vector<double> &weight,
  ...
  new (&seq_rule->_as_iterated_rule.weight) std::vector<double>(weight);
  new (&seq_rule->_as_iterated_rule.arg_weight) std::vector<double>(arg_weight);
  // Do not forget to allocate this field!
  new (&seq_rule->_as_iterated_rule.partitions_weight)
...
CRule *make_mset(const CRule *arg, const std::vector<double> &weight,
  ...
  new (&mset_rule->_as_iterated_rule.partitions_weight)
...
std::vector<int> c_simulate(const CRule *first_rule,
  ...
  while (!todo.empty()) {
    ...
    case SET: {
      ...
      const int k =
          bnd_poisson(set.weight[multiplicity], set.arg_weight[multiplicity],
                      set.lower_size, set.upper_size);
      for (int i = 0; i < k; i++)
        todo.push(rule->_as_iterated_rule.arg);
    } break;
    case MSET: {
      ...
      if (mset.upper_size >= 0) {
        ...
        const int k = bnd_mset_dist(mset.weight, mset.arg_weight, multiplicity);
