    ...
    def get_linked_resources_multiprocessing(self,percentage:float=1.0):
        ...
        **NOTE**: Starting parallel processing takes a bit of time. As such, this method will only outperform get_linked_resources with 10+ linkedNodes
        ...
        with concurrent.futures.ProcessPoolExecutor() as executor:
            ...
            results1=[executor.submit(gt.e57_to_arrays,e57Path=n.path,e57Index=n.e57Index,percentage=percentage,tasknr=i) for i,n in enumerate(self.linkedNodes) 
                        if 'PointCloud' in str(type(n)) and 
                            n.resource is None and
                            n.path.endswith('.e57') and 
                            os.path.exists(n.path)]
            results2=[executor.submit(gt.pcd_to_arrays,path=n.path,percentage=percentage,tasknr=i) for i,n in enumerate(self.linkedNodes) 
                        if 'PointCloud' in str(type(n)) and 
                            n.resource is None and
                            n.path.endswith('.pcd') and 
                            os.path.exists(n.path)]
            results3=[executor.submit(gt.mesh_to_arrays,path=n.path,tasknr=i) for i,n in enumerate(self.linkedNodes) 
                        if ('MeshNode' in str(type(n)) or 'BIMNode' in str(type(n))) and 
                            n.resource is None and
                            os.path.exists(n.path)]
            results4=[executor.submit(gt.img_to_arrays,path=n.path,tasknr=i) for i,n in enumerate(self.linkedNodes) 
