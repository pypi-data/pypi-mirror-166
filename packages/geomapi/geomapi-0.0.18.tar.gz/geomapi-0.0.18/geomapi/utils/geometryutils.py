...
def e57_to_pcd(e57:pye57.e57.E57,e57Index : int = 0,percentage:float=1.0)->o3d.geometry.PointCloud:
    ...
    if all(elem in header.point_fields  for elem in ['cartesianX', 'cartesianY', 'cartesianZ']):   
        ...
        array=np.vstack(( x_ndarray,y_ndarray,z_ndarray)).flatten('F')
        array= np.reshape(array,(len(x_ndarray),3))
        
        #downnsample
        if percentage <1.0:
            indices=np.random.randint(0,len(array)-1,int(len(array)*percentage))
            array=array[indices]
    ...
    if all(elem in header.point_fields  for elem in ['nor:normalX', 'nor:normalY', 'nor:normalZ']): 
        ...
        array= np.reshape(np.vstack(( nx_ndarray,ny_ndarray,nz_ndarray)).flatten('F'),(len(x_ndarray),3))
        if percentage <1.0:
            array=array[indices]
...
def e57_get_xyz_from_spherical_raw_data(raw_data: dict) -> np.array:
    """
    ...
    Args:
        raw_data (e57 dict):  raw_data = e57.read_scan_raw(e57Index)   
        
    Returns:
        np.array (nx3): XYZ cartesian coordinates np.array
    """
    range = raw_data.get('sphericalRange')
    theta = raw_data.get('sphericalAzimuth')
    phi =  raw_data.get('sphericalElevation')
...
def e57_get_xyz_from_raw_data(raw_data: dict)->np.array:
    ...
    y_ndarray=raw_data.get('cartesianY')
    z_ndarray=raw_data.get('cartesianZ') 
    pointArray= np.concatenate((x_ndarray,y_ndarray,z_ndarray), axis=1) 
    pointArray=np.reshape(pointArray.flatten('F'),(len(pointArray),3))
    return pointArray
...
def e57_to_arrays(e57Path:str,e57Index : int = 0,percentage:float=1.0,tasknr:int=0)->Tuple[np.array,np.array,np.array,np.array,int]:
    ...
    raw_data = e57.read_scan_raw(e57Index)    
    if all(elem in header.point_fields  for elem in ['cartesianX', 'cartesianY', 'cartesianZ']):   
        pointArray=e57_get_xyz_from_raw_data(raw_data)
    elif all(elem in header.point_fields  for elem in ['sphericalRange', 'sphericalAzimuth', 'sphericalElevation']):
        pointArray=e57_get_xyz_from_spherical_raw_data(raw_data)
    ...
    if (all(elem in header.point_fields  for elem in ['colorRed', 'colorGreen', 'colorBlue'])
        ...
        if colorRed is not None and colorGreen is not None and colorBlue is not None:
            if np.amax(colorRed)<=1:
                ...
                colors=np.c_[colorRed , colorGreen,colorBlue ]  
            colorArray=np.reshape(colors.flatten('F'),(len(colorRed),3))
            if percentage <1.0:
                colorArray=colorArray[indices]
            ...
            if np.amax(intensity) <=1:
                ...
                colors=np.c_[intensity , intensity,intensity ]  
            colorArray=np.reshape(colors,(len(intensity),3))
            if percentage <1.0:
                colorArray=colorArray[indices]
    ...
    if all(elem in header.point_fields  for elem in ['nor:normalX', 'nor:normalY', 'nor:normalZ']): 
        ...
        ny_ndarray=raw_data.get('nor:normalY')
        nz_ndarray=raw_data.get('nor:normalZ')
        normalArray= np.reshape(np.vstack(( nx_ndarray,ny_ndarray,nz_ndarray)).flatten('F'),(len(nx_ndarray),3))
        if percentage <1.0:
            normalArray=normalArray[indices]
...
def e57_fix_rotation_order(rotation_matrix:np.array) -> np.array:
    """Switch the rotation from clockwise to counter-clockwise in e57 rotation matrix.\n
