...
def replace_str_index(text:str,index:int,replacement:str='_'):
    ...
    Args:
        0. text (str)\n
        1. index (int): index to replace. \n
        2. replacement (str, optional): replacement character. Defaults to '_'.\n
...
def dd2dms(deg) -> List[float,float,float]:
...
def get_graph_subject(graph:Graph,subject:URIRef) ->URIRef:
    """Returns graph subject with its appropriate prefix given the main body of a subject.\n
    This function is mainly used for local file serializations that mud the graph subjects.\n
    You can then use this function to use the correct graph subject regardless of the prefix.\n

    Args:
        graph (Graph): graph with subjects with prefix e.g. URIRef(D:\\DATA\\Subject1)
        subject (URIRef): subject to search for. e.g. URIREF(Subject1)
...
def clean_attributes_list(list:list) -> list:
    """Returns a 'cleaned' list of class attributes that are not to be serialized in the graph.\n
    This includes data attributes such as resources (point clouds, images), linkedNodes, etc.\n
