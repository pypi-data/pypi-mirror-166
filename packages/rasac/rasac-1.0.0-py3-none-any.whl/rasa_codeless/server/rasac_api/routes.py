...
from rasa_codeless.shared.constants import (
    ...
    DEFAULT_TENSORBOARD_LOGDIR, DEFAULT_DATA_PATH, TRAINING_QUEUE,
)
from rasa_codeless.shared.exceptions.server import (
    ModelTrainException,
    EpochsMismatchException,
)
from rasa_codeless.shared.nlu_data import NLUData
...
def config_update():
    if request.method == 'POST':
        ...
        try:
            file_list = get_model_list(model_path=DEFAULT_MODEL_PATH, file_suffixes=RASA_MODEL_EXTENSIONS)
            latest_model = get_latest_model_name(models_path=DEFAULT_MODEL_PATH)
...
def get_data():
    if request.method == 'GET':
        try:
            nlu_data = NLUData(testing_data_dir=DEFAULT_DATA_PATH, from_rasa=False)

            return {"Data": nlu_data.get_testing_data()}, 200
        except Exception as e:
