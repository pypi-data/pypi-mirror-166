#!/usr/bin/env python
# -*- coding: utf-8 -*-
""" pw_cli.py -- PerfectWORK client library and command line tool

Author: Florent Xicluna, Willy
"""
import _ast
import atexit
import csv
...
CONF_FILE = 'pw_cli.ini'
HIST_FILE = os.path.expanduser('~/.pw_cli_history')
DEFAULT_URL = 'http://localhost:8069/xmlrpc'
DEFAULT_DB = 'perfectwork'
DEFAULT_USER = 'admin'
SUPERUSER_ID = 1
...
def read_config(section=None):
    ...
    The config file ``pw_cli.ini`` contains a `section` for each environment.
...
def start_odoo_services(options=None, appname=None):
    ...
    Import the ``perfectwork`` Python package and load the Perfectwork services.
    The argument `options` receives the command line arguments
    for ``perfectwork``.  Example:

      ``['-c', '/path/to/pw.conf', '--without-demo', 'all']``.

    Return the ``perfectwork`` package.
    """
    try:
...
if os.getenv('pw_cli_SSL_UNVERIFIED'):
    import ssl
...
class Env(object):
    ...
    def _set_credentials(self, uid, password):
        ...
        if self.client._report:   # Perfectwork <= 10
...
class Client(object):
    ...
    If `server` is an ``perfectwork``/``openerp`` Python package, it is used to
    ...
    def from_config(cls, environment, user=None, verbose=False):
        ...
        ``pw_cli.ini`` file and return a connected :class:`Client`.
...
class Model(BaseModel):
    ...
    def create(self, values):
        ...
        accept either a list of ids or a RecordList or the extended Perfectwork
        syntax.  Relationship fields `many2one` and `reference` accept
        either a Record or the Perfectwork syntax.
...
class RecordList(BaseRecord):
    """A sequence of Perfectwork :class:`Record`.
...
class Record(BaseRecord):
    ...
    It maps any Perfectwork object.
...
def main(interact=_interact):
    description = ('Inspect data on Perfectwork objects.  Use interactively '
