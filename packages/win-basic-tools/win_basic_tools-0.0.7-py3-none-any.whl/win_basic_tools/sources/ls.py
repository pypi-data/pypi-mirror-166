...
class Ls:
    ...
    def echo(self, signal: int) -> None:
        try:
            with os.scandir(self.path) as dir:
                ...
                if 'l' in self.opt:
                    for i in dir:
                        filemode_str = self._windows_filemode(i.stat().st_file_attributes, 'c' in self.opt)
                        if (i.name.startswith('.') or 'h' in filemode_str) and 'a' not in self.opt:
                            continue
    ...
    def _windows_filemode(self, data: os.stat_result.st_file_attributes, colors: bool):
        str_res = ''
