...
class Repository:
    ...
    def _get_entity_by_address(self, address: str, cls, **url_params):
        data = self.connection.get_object_by_filter(
            cls.NETDOT_TABLE_NAME, 'address', address)
        matching_entities = [
        ...
        if len(matching_entities) > 1:
            logger.warning(
                f'Found more than one matching address for {address} ({cls.__name__}): {matching_entities}')
    ...
    def find_edge_port(self, mac_address: str) -> 'netdot.dataclasses.Interface':
        ...
        for entry in most_recent_entries:
            ...
            fwtableentries_for_interface = self.get_fwtableentries_by_interface(interface, fwtable=entry.fwtable_xlink)
            interface_counts[entry.interface] = len(fwtableentries_for_interface)

        interface_counts = dict(sorted(interface_counts.items(), key=lambda item: item[1]))
        most_recent_interface = list(interface_counts.keys())[0]
        most_recent_entity_list = list(filter(lambda entry: entry.interface == most_recent_interface, most_recent_entries))
        if (len(most_recent_entity_list) != 1):
            logger.warning(f'Expected to find one entity. Instead found: {most_recent_entity_list}')
        most_recent_entity = most_recent_entity_list[0]
        return most_recent_entity.get_interface()
