...
base_hive = {
    ...
    "variables": {
        "hostname": {"type": "url_replacement"},
        "port": {"type": "url_replacement"},
        "username": {
            "type": "http_basic_auth",
            "optional": True,
        },
        "password": {"type": "http_basic_auth", "optional": True},
        "jsonrpc": {"value": "2.0", "type": "json_rpc"},
        "request_id": {"type": "json_rpc"},
    },
    "endpoints": {"rpc": {"path": "/jsonrpc", "methods": ["POST"]}},
    "objects": {
        "API": {
            "actions": {
                "get": {
                    ...
                    "method": "POST",
                    "traverse": ["result"],
                    "variables": {"method": {"value": "JSONRPC.Introspect", "type": "json_rpc"}},
                }
            }
        }
    },
...
def Kodi(hostname, username="kodi", password=None, port=8080):
    ...
    return API(hive, hostname=hostname, port=port, username=username, password=password, request_id=random_id_gen)
