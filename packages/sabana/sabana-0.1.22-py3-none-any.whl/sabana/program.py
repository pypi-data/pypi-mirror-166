...
class Program:
    ...
    def dma_alloc(self, name=None):
        func_name = "dma_alloc"
        try:
            self.req.requests.append(sabana_req.dma_alloc(name))
        except CommonError as e:
            raise ProgramError(Program.err_msg_from_e(func_name, e))
        except Exception:
            raise ProgramError(Program.err_msg(func_name))

    def dma_write(self, name=None, src=None):
        func_name = "dma_write"
        try:
            self.req.requests.append(sabana_req.dma_write(name, src))
        except CommonError as e:
            raise ProgramError(Program.err_msg_from_e(func_name, e))
        except Exception:
            raise ProgramError(Program.err_msg(func_name))

    def dma_read(self, name=None, dst=None):
        func_name = "dma_read"
        try:
            self.req.requests.append(sabana_req.dma_read(name, dst))
        except CommonError as e:
            raise ProgramError(Program.err_msg_from_e(func_name, e))
        except Exception:
            raise ProgramError(Program.err_msg(func_name))

    def dma_dealloc(self, name=None):
        func_name = "dma_dealloc"
        try:
            self.req.requests.append(sabana_req.dma_dealloc(name))
        except CommonError as e:
            raise ProgramError(Program.err_msg_from_e(func_name, e))
        except Exception:
            raise ProgramError(Program.err_msg(func_name))
