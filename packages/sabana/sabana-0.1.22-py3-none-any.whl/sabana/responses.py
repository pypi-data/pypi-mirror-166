...
def dma_res(res):
    res.resource = proto.RESOURCE_DMA
    return res
...
def dma_write(name):
    return dma_res(write(name))


def dma_read(name, value):
    return dma_res(read(name, value))


def dma_wait(name):
    return dma_res(wait(name))


def dma_alloc(name):
    return dma_res(alloc(name, zero))


def dma_dealloc(name):
    return dma_res(dealloc(name))
...
def is_dma(res):
    return (
        (res.resource == proto.RESOURCE_DMA)
        or (res.resource == proto.RESOURCE_DMA_WRITE)
        or (res.resource == proto.RESOURCE_DMA_READ)
    )


def is_dma_write(res):
    return res.resource == proto.RESOURCE_DMA_WRITE


def is_dma_read(res):
    return res.resource == proto.RESOURCE_DMA_READ
