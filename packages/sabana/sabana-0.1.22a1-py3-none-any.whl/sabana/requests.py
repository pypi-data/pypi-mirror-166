...
def dma_req(req):
    req.resource = proto.RESOURCE_DMA
    return req
...
def dma_write(name, src):
    return dma_req(write(name=name, src=src))


def dma_read(name, dst):
    if dst is None:
        raise RuntimeError("dst needs be a non-empty string")
    return dma_req(read(name=name, dst=dst))


def dma_alloc(name):
    return dma_req(alloc(name, 0, "0", 0))


def dma_dealloc(name):
    return dma_req(dealloc(name))


def dma_wait(name, offset, value, timeout):
    return buffer_req(wait(name, offset, value, timeout))
...
def is_dma(req):
    return req.resource == proto.RESOURCE_DMA
