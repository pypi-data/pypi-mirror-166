...
class CopySharedLibrary(Command):
    ...
    def run(self) -> None:
        ...
        if sys.platform.startswith('linux'):
            for file_name in os.listdir(os.path.join(self.build_dir, 'grapa-lib/aws')):
                self.copy_file(os.path.join(os.path.join(self.build_dir, 'grapa-lib/aws'),file_name), os.path.join(lib_target_path, self.filename))
