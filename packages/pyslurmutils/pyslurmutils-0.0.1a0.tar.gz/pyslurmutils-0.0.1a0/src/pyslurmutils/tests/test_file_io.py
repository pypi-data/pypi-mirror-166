...
def test_file_io(tmpdir):
    ...
    with file_io.JobFileIoHandler() as handler:
        ...
        with open(outfile, "wb") as f:
            pickle.dump({"1", "2"}, f)
        assert handler.get_job_result(1234) == {"1", "2"}
...
def test_file_io_cleanup(tmpdir):
    with file_io.JobFileIoHandler() as handler:
        ...
        with open(outfile, "wb") as f:
            pickle.dump({"1", "2"}, f)
        assert handler.get_job_result(1) == {"1", "2"}
        ...
        assert set(handler.get_job_ids()) == {1, 2}
    ...
    # future = handler._local_results[2]
    # import objgraph
    # objgraph.show_backrefs([future], filename='test.png', max_depth=10)
