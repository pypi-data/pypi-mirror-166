...
class APIException(Exception):
    def __init__(self, response: t.Union[Response, ClientResponse], status_code: int, text: str):
        ...
        try:
            ...
            self.message = 'Invalid JSON error message from Tornado: {}'.format(response.text)
        else:
            self.code = json_res['code']
            self.message = json_res['message']

            try:
                self.errors = json_res['errors']
            except KeyError:
    ...
    def __str__(self):  # pragma: no cover
        return 'APIError(code=%s): %s | %s' % (self.code, self.message, self.errors)
