...
class Request:
    ...
    def _convert_fields(
        ...
        current_dependency: list[tuple[str, ScenarioBase]] = agent.workflow_ctx.dependency.get(
    ...
    async def execute(
        ...
        try:
            async with agent.workflow_ctx.session.request(
                method=self.method,
                ...
                for extract_field in self.extract_fields:
                    ...
                    agent.workflow_ctx.context[target_key] = data

                    print(f'[{self.url}] - {target_key} : {agent.workflow_ctx.context[target_key]}')
    ...
    async def execute(self, agent: Agent) -> None:
        ...
        for agent_name, scn_class in current_dependency:
            while asyncio.get_event_loop().time() - start_time < config.WAIT_FOR_SCENARIO_TIMEOUT:
                if (agent_name, scn_class.__name__, config.FINISHED_SUFFIX) in agent.workflow_ctx.context:
                    break
...
class Agent:
    def __init__(self, name: str, scenarios: list[ScenarioBase] | None = None) -> None:
        self.name = name
        self.scenarios = scenarios
        self.workflow_ctx = None

    async def execute(self, workflow_ctx: WorkflowCtx) -> None:
