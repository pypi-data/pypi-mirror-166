...
def pwd4wifi_service(pwd_len, pwd_list):
    ...
    while not exit_flag:
        try:
            ...
            t1 = threading.Thread(target=wifi_password_crack, args=(wifi_list[target_num][1], pwd_len, pwd_list,))  # target是要执行的函数名（不是函数），args是函数对应的参数，以元组的形式存在
            t2 = threading.Thread(target=wifi_password_crack, args=(wifi_list[target_num][1], pwd_len, pwd_list,))  # target是要执行的函数名（不是函数），args是函数对应的参数，以元组的形式存在
            t1.start()
            t2.start()
