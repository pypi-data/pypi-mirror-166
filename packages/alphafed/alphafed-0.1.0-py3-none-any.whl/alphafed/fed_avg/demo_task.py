...
from .fed_avg import DEV_TASK_ID
...
class DemoScheduler(FedAvgScheduler):
    ...
    def test(self) -> Dict[str, Any]:
        if not self._time_metrics:
            self._time_metrics = MetricsTrace(headers=['run_time'])
            self.register_metrics(name='timer', metrics=self._time_metrics)
        ...
        self._time_metrics.append_metrics_item({'run_time': end - start})
...
def get_scheduler() -> DemoScheduler:
    ...
    if not os.path.exists(pickle_file):
        scheduler = DemoScheduler(min_clients=3,
                                  max_clients=3,
                                  name='demo_task',
                                  max_rounds=5,
                                  log_rounds=1,
...
# def get_scheduler() -> DemoScheduler:
#     return DemoScheduler(min_clients=3,
#                          max_clients=3,
#                          name='demo_task',
#                          max_rounds=1,
#                          log_rounds=1,
