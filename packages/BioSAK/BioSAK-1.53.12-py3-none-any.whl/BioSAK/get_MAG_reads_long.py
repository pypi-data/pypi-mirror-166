...
BioSAK get_MAG_reads_long -r LongReads.fq -s assembly.sam -m MAG_1.fa -o MAG_1_reads -l 3000
BioSAK get_MAG_reads_long -r LongReads.fq -s assembly.sam -m MAG_dir -x fa -o extracted_reads -l 0
BioSAK get_MAG_reads_long -r LongReads.fq -s assembly.sam -m MAG_dir -x fa -o extracted_reads -l 10000

# Note
The input sam file is obtained by mapping filtered reads to all flye produced contigs
minimap2 -d assembly.mmi assembly.fasta
minimap2 -t 12 --secondary=no -a assembly.mmi long_reads.fastq > assembly.no.secondary.sam 
