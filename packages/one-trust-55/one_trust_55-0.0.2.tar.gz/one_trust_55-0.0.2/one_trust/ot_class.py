...
class ot:
    def __init__(self,token):   
        self.url= 'https://app-fr.onetrust.com'
    ...
    def get_scan_sites(self,col_num_site,col_num_org,number_of_pages):
        site_list = file.read_by_col(col_num_site)
        org_list = file.read_by_col(col_num_org)
        url= self.url + '/api/cookiemanager/v2/websites'
        site_org_map= (list((zip_longest(site_list, org_list))))
        for index,site in enumerate (site_org_map):
            site_to_scan=(site_org_map[index][0]).split(' ')
            org_to_scan = (site_org_map[index][1])
            payload = {
            ...
            if (r.status_code ==200) :
                ...
                print (f'Error: {r.status_code} API key has probably expired or request is incorrrect. Create a new API key on the OT interface and try again') 
                exit()  
