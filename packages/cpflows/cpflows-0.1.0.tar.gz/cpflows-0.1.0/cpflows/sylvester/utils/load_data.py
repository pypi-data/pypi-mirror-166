...
def load_static_mnist(args, **kwargs):
    ...
    with open(os.path.join(args.data_root, 'MNIST_static', 'binarized_mnist_train.amat')) as f:
        lines = f.readlines()
    x_train = lines_to_np_array(lines).astype('float32')
    with open(os.path.join(args.data_root, 'MNIST_static', 'binarized_mnist_valid.amat')) as f:
        lines = f.readlines()
    x_val = lines_to_np_array(lines).astype('float32')
    with open(os.path.join(args.data_root, 'MNIST_static', 'binarized_mnist_test.amat')) as f:
        lines = f.readlines()
...
def load_freyfaces(args, **kwargs):
    ...
    # start processing
    with open(f'{args.data_root}/Freyfaces/freyfaces.pkl', 'rb') as f:
...
def load_omniglot(args, **kwargs):
    ...
    omni_raw = loadmat(os.path.join(args.data_root, 'OMNIGLOT', 'chardata.mat'))
...
def load_caltech101silhouettes(args, **kwargs):
    ...
    caltech_raw = loadmat(os.path.join(args.data_root, 'Caltech101Silhouettes', 'caltech101_silhouettes_28_split1.mat'))
