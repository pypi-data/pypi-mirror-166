...
from mmdet_taeuk4958.core import BitmapMasks, PolygonMasks, find_inside_bboxes
from mmdet_taeuk4958.core.evaluation.bbox_overlaps import bbox_overlaps
from mmdet_taeuk4958.utils import log_img_scale
...
class Resize:
    ...
    def __call__(self, results):
        ...
        if 'scale' not in results:
            print(f"if 'scale' not in results")
            if 'scale_factor' in results:   
                print(f"if 'scale_factor' in results: ")        
                ...
                print(f"not if 'scale_factor' in results: ")
                self._random_scale(results)
            exit()
        else:
            print(f"not if 'scale' not in results")
            exit()
