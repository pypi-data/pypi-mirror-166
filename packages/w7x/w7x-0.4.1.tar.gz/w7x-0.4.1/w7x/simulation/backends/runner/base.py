...
class RunnerBackendMixin:
    ...
    def runner(self, runner):
        ...
        self.folder = get(f"{code}.{runner}", "folder", fallback=True)
        self.exe = get(f"{code}.{runner}", "exe")
...
class Job:
    ...
    timeout: int = int(get("job", "timeout"))  # [s]
    ...
    tasks: int = int(get("job", "tasks"))
    #: number of computer nodes to use
    nodes: int = int(get("job", "nodes"))
