...
__config = rna.config.Config(
    rna.path.resolve("~", ".w7x", "local.yaml"),
    rna.path.resolve("~", ".w7x", "config.yaml"),
    rna.path.resolve(os.path.dirname(__file__), "config.yaml"),
    **VARIABLES,
)
...
def get_mgrid_options(return_type=None):
    ...
    """
    nfp = int(w7x.config.get("mgrid", "nfp"))
    base_vectors = []
    iter_order = []
    for var in ["r", "phi", "z"]:
        xmin = w7x.config.get("mgrid." + var, "min")
        xmin = float(xmin) if xmin != "None" else 0
        xmax = w7x.config.get("mgrid." + var, "max")
        xmax = float(xmax) if xmax != "None" else 2 * np.pi / nfp
        num = complex(0, w7x.config.get("mgrid." + var, "num"))
        base_vectors.append((xmin, xmax, num))
        iter_order.append(int(w7x.config.get("mgrid." + var, "iter")))
...
def _get_mgrid_path():
    ...
    backend = get("vmec", "backend", fallback=True)
    return get(f"vmec.{backend}", "mgrid", fallback=True)
