...
def lsdb(mixdb: MixtureDatabase,
    ...
    if mixid == '*':
        mixid = np.array(range(len(mixdb.mixtures)))
        logger.info(f'Mixture id list expanded from {mixid[0]} to {mixid[-1]} ...')
    else:
        mixid = np.array(expand_range(str(mixid)))
    ...
    if len(mixid) == 1:
        ...
        if truth_f_shape is not None:
            logger.info(f'Calculating statistics from .h5 file truth for {len(mixid)} mixtures ...')
            with h5py.File(datafile, 'r') as f:
                truth_f = np.array(f['truth_f'])

            truth_mean = np.zeros((truth_f_shape[1]))
            truth_var = np.zeros((truth_f_shape[1]))
            truth_dbmean = np.zeros((truth_f_shape[1]))
            truth_dbvar = np.zeros((truth_f_shape[1]))
            for ci in range(truth_f_shape[1]):
                tmp = truth_f[mixid, ci]
                tmp2 = tmp[np.isfinite(tmp)]
                if len(tmp2) == 0:
                    logger.info(f'Warning: class {ci} has no finite values ...')
                truth_mean[ci] = np.mean(tmp2)
                truth_var[ci] = np.var(tmp2)
                tmp3 = 10 * np.ma.log10(tmp2).filled(-np.inf)
                tmp4 = tmp3[np.isfinite(tmp3)]
                if len(tmp4) == 0:
                    truth_dbmean[ci] = -np.inf
                    truth_dbvar[ci] = -np.inf
                else:
                    truth_dbmean[ci] = np.mean(tmp4)
                    truth_dbvar[ci] = np.var(tmp4)
            logger.info('Truth mean:')
            logger.info(truth_mean)
            logger.info('Truth var:')
            logger.info(truth_var)
            logger.info('Truth dB mean:')
            logger.info(truth_dbmean)
            logger.info('Truth dB var:')
            logger.info(truth_dbvar)
        else:
