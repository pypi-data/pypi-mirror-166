...
def lsdb(mixdb: MixtureDatabase,
    ...
    from sonusai.metrics import calculate_snr_f_statistics
    ...
    if len(mixid) == 1:
        ...
        if truth_f_shape is not None:
            ...
            logger.info('Truth')
            logger.info(f'  {"mean":^8s}  {"var":^8s}  {"db_mean":^8s}  {"db_std":^8s}')
            for c in range(snr_mean.shape[0]):
                logger.info(f'  {snr_mean[c]:8.4f}  {snr_var[c]:8.4f}  {snr_db_mean[c]:8.4f}  {snr_db_std[c]:8.4f}')
        else:
