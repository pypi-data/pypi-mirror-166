...
def parse_workflow_description(
    wf_description: str, ws_root_path: Path, workflow_dir: str
    ...
    The workflow_file is the path of the file starting after the workflow_dir.
    The workflow_file string can then be joined to the `workflow_dir` in the current env
    ...
    workflow_dir_path: Path = ws_root_path.joinpath(workflow_dir)
    if not (workflow_dir_path.exists() and workflow_dir_path.is_dir()):
        logger.error("Invalid products dir: {}".format(workflow_dir_path))
