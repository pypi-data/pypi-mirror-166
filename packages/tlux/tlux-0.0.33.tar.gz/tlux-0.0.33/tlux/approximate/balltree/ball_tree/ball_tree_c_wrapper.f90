...
CONTAINS
  ...
  SUBROUTINE C_CONFIGURE(NUM_THREADS_PRESENT, NUM_THREADS, MAX_LEVELS_PRESENT, MAX_LEVELS, NESTED_PRESENT, NESTED) BIND(C)
    ...
    INTEGER, INTENT(INOUT) :: NUM_THREADS
    LOGICAL, INTENT(IN) :: MAX_LEVELS_PRESENT
    INTEGER, INTENT(INOUT) :: MAX_LEVELS
    LOGICAL, INTENT(IN) :: NESTED_PRESENT
    LOGICAL, INTENT(INOUT) :: NESTED
  ...
  SUBROUTINE C_BUILD_TREE(POINTS_DIM_1, POINTS_DIM_2, POINTS, SQ_SUMS_DIM_1, SQ_SUMS, RADII_DIM_1, RADII, MEDIANS_DIM_1, MEDIANS, O&
&RDER_DIM_1, ORDER, ROOT_PRESENT, ROOT, LEAF_SIZE_PRESENT, LEAF_SIZE) BIND(C)
    ...
    IF (ROOT_PRESENT) THEN
      IF (LEAF_SIZE_PRESENT) THEN
        CALL BUILD_TREE(POINTS=POINTS, SQ_SUMS=SQ_SUMS, RADII=RADII, MEDIANS=MEDIANS, ORDER=ORDER, ROOT=ROOT, LEAF_SIZE=LEAF_SIZE)
      ELSE
        CALL BUILD_TREE(POINTS=POINTS, SQ_SUMS=SQ_SUMS, RADII=RADII, MEDIANS=MEDIANS, ORDER=ORDER, ROOT=ROOT)
      END IF
    ELSE
      IF (LEAF_SIZE_PRESENT) THEN
        CALL BUILD_TREE(POINTS=POINTS, SQ_SUMS=SQ_SUMS, RADII=RADII, MEDIANS=MEDIANS, ORDER=ORDER, LEAF_SIZE=LEAF_SIZE)
      ELSE
        CALL BUILD_TREE(POINTS=POINTS, SQ_SUMS=SQ_SUMS, RADII=RADII, MEDIANS=MEDIANS, ORDER=ORDER)
      END IF
    END IF
