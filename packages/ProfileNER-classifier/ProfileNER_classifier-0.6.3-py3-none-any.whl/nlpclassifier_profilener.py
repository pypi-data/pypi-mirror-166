...
class NLPClassifier:
    ...
    def __init__(self, use, youtube: bool = False):
        ...
        ner_data = pd.read_csv(resources.open_binary('profileclassifier.nlpclassifier', 'ner_data.csv'))
        ...
        self.politico = pd.read_csv(resources.open_binary('profileclassifier.nlpclassifier', 'lista_politico.csv'), encoding='utf8')['politicos'].tolist()
        self.artista = ['singer',r'\bcantor',r'\batriz',r'\bator','comediante','youtuber', r'influenciado.* digital']
        midia_tradicional2 = pd.read_csv(resources.open_binary('profileclassifier.nlpclassifier', 'lista_midia_trad.csv'), encoding ='utf-8')['midias'].apply(lambda x:str(x).replace( ' ', '')).tolist()
        self.veiculo_info = pd.read_csv(resources.open_binary('profileclassifier.nlpclassifier', 'nlpclassifier/lista_midia_trad.csv'), encoding ='utf-8')['midias'].tolist() +midia_tradicional2
        ...
        universidades = pd.read_csv(resources.open_binary('profileclassifier.nlpclassifier', 'nlpclassifier/lista_unifederal.csv'), encoding ='utf-8', sep =',')['universidade'].tolist()
        self.org = pd.read_csv('nlpclassifier/lista_org.csv', encoding='utf8')['org'].tolist() + ['universidade federal','universidade estadual','universidade de sao paulo']
        ...
        parti = ['brasil247']
        midias_partidarias2 = pd.read_csv(resources.open_binary('profileclassifier.nlpclassifier', 'nlpclassifier/lista_midia_partidaria.csv'), encoding= 'utf-8')['midias'].apply(lambda x:str(x).replace( ' ', '')).tolist()
        self.midias_partidarias = pd.read_csv(resources.open_binary('profileclassifier.nlpclassifier', 'nlpclassifier/lista_midia_partidaria.csv'), encoding= 'utf-8')['midias'].tolist() + parti +midias_partidarias2
    ...
    def att_profile_list(self,profile,new_words = []):
        ...
        if profile == 'veiculo_jornalistico':
            for word in new_words:
                self.veiculo_jornalistico.append(word)
        if profile == 'saude':
            for word in new_words:
    ...
    def classifier(self, bio):
        ...
        if self.use == 'profile':
            ...
            for text in self.artista:
                if re.compile(text).search(bio):
                    return 'Artista'
            for text in self.veiculo_jornalistico:
                if re.compile(text).search(bio):
