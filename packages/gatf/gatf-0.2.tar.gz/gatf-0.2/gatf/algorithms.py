...
class NSGA3:
    ...
    A few metrics are calculated by this algorithm, including mutual domination
    rate (MDR), non-dominated hypervolume (HV), and the % of filled reference
    ...
    :class:`gatf.population.Population`
        ...
        object with the keys 'gen', 'mdr', 'hv', and 'refs'.
    ...
    def __init__(self,
        ...
        print('Population size: {}, reference directions: {}, partitions: {}'.format(population_size,int(n_references),partitions))
    ...
    def _calculate_hypervolume(self,f):
        '''
        Calculate the non-dominated hypervolume
        '''
        ...
        # Normalize f_last
        f_nd = self._adaptive_normalization(f_nd)
        # Sort first column ascending
        sort_idx = tf.argsort(f_nd[:,0], axis=0)
        f_nd = tf.gather(f_nd, sort_idx, axis=0)
        # Separate first column from remaining and calcuate distance to reference point
        D1 = tf.reshape(self.hv_ref[:,0] - f_nd[:,0],(-1,1))
        # Concat reference point then select columns 1:
        D2 = tf.concat([self.hv_ref,f_nd], axis=0)[:,1:]
        # Calculate difference per adjacent row for each column
        #D2 = tf.abs(D2[:-1,:]-D2[1:,:])
        D2 = D2[:-1,:]-D2[1:,:]
        # Concat columns
        D = tf.concat([D1,D2], axis=-1)
        return tf.reduce_sum(tf.math.reduce_prod(D,axis=1))
    ...
    def __call__(self):
        ...
        hv = self._calculate_hypervolume(f_new)
        mdr = self._calculate_mdr(f_new,f_new)
        refs = self._calculate_filled_references(f_new)

        print('Generation 0 – HV {} – MDR {} – Ref dirs {}%     '.format(np.round(hv.numpy(),2),np.round(mdr.numpy(),2),np.round(100*refs.numpy())), end = '\r')

        # Add statistics
        self.population.statistics['gen'] = 0
        self.population.statistics['mdr'] = mdr
        self.population.statistics['hv'] = hv
        ...
        for gen in range(1, self.generations+1):
            ...
            if gen % self.record_gens == 0: 
                ...
                print('Generation {} – HV {} – MDR {} – Ref dirs {}%     '.format(gen,np.round(hv.numpy(),2),np.round(mdr.numpy(),2),np.round(100*refs.numpy())), end = '\r')
...
class muPlusLambda:
    ...
    def __call__(self):
        ...
        print('Generation 0 – min {} - mean {} – max {} – best {}   '.format(np.round(f_min.numpy(),2),np.round(f_mean.numpy(),2),np.round(f_max.numpy(),2),np.round(tf.squeeze(f_best).numpy(),2)), end = '\r')
        ...
        for gen in range(1, self.generations+1):
            ...
            if gen % self.record_gens == 0: 
                ...
                print('Generation {} – min {} - mean {} – max {} – best {}   '.format(gen,np.round(f_min.numpy(),2),np.round(f_mean.numpy(),2),np.round(f_max.numpy(),2),np.round(tf.squeeze(f_best).numpy(),2)), end = '\r')
...
class PSO:
    ...
    def __call__(self):
        ...
        print('Generation 0 – min {} - mean {} – max {} – best {}   '.format(np.round(f_min.numpy(),2),np.round(f_mean.numpy(),2),np.round(f_max.numpy(),2),np.round(tf.squeeze(gbest_f).numpy(),2)), end = '\r')
        ...
        for gen in range(1, self.generations+1):
            ...
            if gen % self.record_gens == 0: 
                ...
                print('Generation {} – min {} - mean {} – max {} – best {}   '.format(gen,np.round(f_min.numpy(),2),np.round(f_mean.numpy(),2),np.round(f_max.numpy(),2),np.round(tf.squeeze(gbest_f).numpy(),2)), end = '\r')
