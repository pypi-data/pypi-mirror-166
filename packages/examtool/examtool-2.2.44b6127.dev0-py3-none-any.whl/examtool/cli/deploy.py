...
@click.option(
    "--copyable-secret/--any-secret",
    default=False,
    type=bool,
    help="Generate an alphanumeric secret (for double-click-to-highlight).",
)
def deploy(exam, json, roster, start_time, enable_clarifications, copyable_secret):
    """
    ...
    try:
        ...
        if old_secret:
            print(f"Reusing old secret ({old_secret})")
            secret = old_secret
    except Exception:
        pass

    while not secret or (copyable_secret and ("-" in secret or "_" in secret)):
        if secret:
            print(f"Detected non-alphanumeric secret, trying again ({secret})")
            continue
        secret = Fernet.generate_key().decode("utf-8")
    if not old_secret and not click.confirm(
        f"Generated new secret ({secret}), use it?"
    ):
        exit(1)

    exam_content["secret"] = secret
