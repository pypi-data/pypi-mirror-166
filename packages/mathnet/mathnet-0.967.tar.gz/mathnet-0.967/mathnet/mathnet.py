...
class dataSet():
    def __init__(self, encoding='gbk'):
        ...
        self.dataDir=os.path.join(os.path.abspath('..'), 'data')   #data目录
        True
    def _list(self,):
        return os.listdir(self.dataDir)
    def load(self, fileName,):
        file=os.path.join(self.dataDir, fileName)    # 文件格式为[.csv, .xlsx]
        fileFormat=fileName.split('.')[-1]
        if fileFormat=='csv':
            data=pd.read_csv(file, encoding=self.encoding)
        elif fileFormat=='xlsx':
            data=pd.read_excel(file,)
        else:
            raise Exception('Please provide a appropriate file format.')
        return data       
#------------------------------------------------------------------------------------------2022-9-8
