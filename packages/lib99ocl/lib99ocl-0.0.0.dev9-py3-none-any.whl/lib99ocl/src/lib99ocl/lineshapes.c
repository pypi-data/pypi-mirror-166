                   ...
                   const ftype yae, const ftype res)
{
  ...
  //   printf("mu=%.2f, sigma=%.2f, dmu=%.2f, dsigma=%.2f, yae=%.2f, res=%.2f\n", mu, sigma, dmu, dsigma, yae, res);
  // }
...
WITHIN_KERNEL
ftype double_gaussian_integral(const ftype xL, const ftype xU, const ftype mu, const ftype sigma, 
                   const ftype dmu, const ftype dsigma, 
                   const ftype yae, const ftype res)
{
  const ftype mup = mu + dmu;
...
{
  int useLog = 0;
  if(fabs(n - 1.0) < 1.0e-14)  useLog = 1;
