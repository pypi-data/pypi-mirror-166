...
class ReadOnlyEntryDialog(QDialog, Ui_OverwriteEntryDialog):
    ...
    def setMessage(self, protocolID):
        """
        Update the message of the dialog window using the current protocolID.
...
# TODO: move this constant into an enumerator
...
sample_full_name_role = QtCore.Qt.UserRole + 12
sample_last_name_role = QtCore.Qt.UserRole + 13
...
class SampleItem(QStandardItem):
    """
    Saample item derived from QStandardItem.
    ...
    def __init__(self, sample_full_name):
        ...
        self.setData('Sample', item_type)
        self.setData(sample_full_name, sample_full_name_role)
        self.setData(sample_last_name, sample_last_name_role)
    ...
    def getDataFromYAML(self, yamlDict):
        ...
        """
        key = self.data(sample_full_name_role)
        if key in yamlDict.keys():
            if 'Description' in yamlDict[key].keys():
                self.setData(yamlDict[key]['Description'], description_role)
            if 'Extra' in yamlDict[key].keys():
...
class ProtocolEditor(QDialog, Ui_tree_viewer_dialog):
    ...
    def generate_model(self):
        ...
        for full_name, sample in self.autolog.samples.items():
            if sample.parent is None:
                ...
                entry.getDataFromYAML(self.autolog.yamlDict)
    ...
    def add_element(self, element_type, element_name, parent_name):
        ...
        if element_type == 'NavPic':
            ...
            good_item = None
            for item in items:
                if item.data(sample_full_name_role) == sample_full_name:
                    good_item = item
                    break
            ...
            if parent_name == 'Samples':
                ...
                good_item = None
                for item in items:
                    if item.data(sample_full_name_role) == parent_full_name:
                        good_item = item
                        break
    ...
    def update_values(self, index):
        ...
        elif index.data(item_type) == 'Sample':
            key = index.data(sample_full_name_role)
        else:
            key = index.data()
    ...
    def get_values(self, index):
        ...
        elif index.data(item_type) in ['Text', 'Section', 'Sample']:
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def is_elog_entry_editable(self, protocolID):
        """
        Check if the entry with protocolID is editable.

        This method is checking if there is an entry with Protocol ID exactly
        matching the protocolID of the current protocol.
        ...
        msg_ids = elog_instance.search({'Protocol ID': protocolID})
        real_ids = []
        for id in msg_ids:
            message, attributes, attachments = elog_instance.read(msg_ids[0])
            if attributes['Protocol ID'] == protocolID:
                real_ids.append(id)
        ...
        if real_ids:
            ...
            if attributes.get('Edit Lock', 'Unprotected') == 'Protected':
                log.error(
                    'An entry with protocol ID %s already exists and it is read-only' % protocolID)
                # we need to ask the user how to recover this situation
                return self.handle_readonly_entry(protocolID, elog_instance, msg_ids[0])
            else:
    ...
    def handle_readonly_entry(self, protocolID, elog_instance, msg_id):
        """
    ...
    def force_overwrite_readonly(self, elog_instance, msg_id):
        """
    ...
    def backup_readonly(self, elog_instance, msg_id):
        """
