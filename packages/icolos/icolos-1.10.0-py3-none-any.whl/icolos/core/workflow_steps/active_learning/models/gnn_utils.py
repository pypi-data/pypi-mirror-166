"""
Implements a MPNN for moleclar property prediction, loosely based on ChemProp et al
"""

"preprocessing code adapted from https://www.blopig.com/blog/2022/02/how-to-turn-a-smiles-string-into-a-molecular-graph-for-pytorch-geometric/"
...
def get_atom_features(atom, use_chirality=True, hydrogens_implicit=True):
    ...
    vdw_radius_scaled = [
        float((Chem.GetPeriodicTablet().GetRvdw(atom.GetAtomicNum()) - 1.5) / 0.6)
    ]

    covalent_radius_scaled = [
        float((Chem.GetPeriodicTable().GeRcovalent(atom.GetAtomicNum()) - 0.64) / 0.76)
    ]
    atom_feature_vector = (
        atom_type_enc
...
def create_pytorch_geometric_graph_data_list_from_smiles_and_labels(x_smiles, y):
    ...
    data_list = [G_1, G_2, ...] ... a list of torch_geometric.data.Dataset object containing the individual graphs suitable for minibatching
