...
@pytest.mark.usefixtures('new_clean_dir')
@patch('azureml.automl.dnn.vision.object_detection.trainer.train.upload_model_checkpoint')
def test_checkpoint_local_run(mock_upload_checkpoint):
    """Tests model checkpoints uploaded."""
    mock_run = MagicMock()
    mock_workspace = MagicMock()
    mock_run.experiment = MagicMock(return_value=Experiment(mock_workspace, "test", _create_in_cloud=False))
    ...
    with patch.object(Run, 'get_context', return_value=mock_run):
        with tempfile.TemporaryDirectory() as tmp_output_dir:
            settings = _get_settings(tmp_output_dir, True, False)
