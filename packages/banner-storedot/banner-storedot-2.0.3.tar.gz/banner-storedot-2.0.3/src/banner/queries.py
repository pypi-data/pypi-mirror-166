...
class AugmentQueries(object):
    ...
    def set_features(self, table: str, features: pd.DataFrame, columns: list): # TABLE, CELL, TEST_ID, CYCLE 
    ...
    def add_feature(self, table: str, name: str, dtype: str, index=False, description=False, updated_on=False):
        ...
        if description:
            self.__add_column(table, f'{str(name)}_description', 'VARCHAR(255)')

        if updated_on:
            self.__add_column(table, f'{str(name)}_updated_on', 'DATETIME')
