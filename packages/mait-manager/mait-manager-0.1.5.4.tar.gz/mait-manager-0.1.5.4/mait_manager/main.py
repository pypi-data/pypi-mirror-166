...
# TODO: Convert to UDP-like commands.
...
def play(game : str):
    def execute(game):
        with open(f'./packages/{game}/mait.json', 'r') as r:
            data = json.load(r)
...
{long}
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
        ...
        with alive_bar(3) as bar:
            ...
            f = open(f'./packages/{game}.zip', 'r+b')
            data = f.read()
        ...
        try:
            os.mkdir(f'./packages/{game}')
        except FileExistsError:
            s.close()
            execute(game)

        shutil.unpack_archive(f'./packages/{game}.zip', f'./packages/{game}')
        os.remove(f'./packages/{game}.zip')
        bar()

        s.close()
        execute(game)

@app.command(help="Uploads a package to mait's registry.")
def upload(path : str):
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
        ...
        with alive_bar(5) as bar:
            ...
            name = path.rfind("/")
            name = path[name:]; name = name[1:]
            bar()
            s.sendto(b'10', SERVER)
