...
def play(game: str):
    ...
    def execute(game):
        with open(f'{temp_dir}/{game}/mait.json', 'r') as r:
            data = json.load(r)
...
{long}
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
        ...
        with alive_bar(3) as bar:
            ...
            f = open(f'{temp_dir}/{game}.zip', 'r+b')
            data = f.read()
        ...
        try:
            os.mkdir(f'{temp_dir}/{game}')
        except FileExistsError:
            s.close()
            execute(game)

        shutil.unpack_archive(f'.{temp_dir}/{game}.zip', f'{temp_dir}/{game}')
        os.remove(f'{temp_dir}/{game}.zip')
        bar()
