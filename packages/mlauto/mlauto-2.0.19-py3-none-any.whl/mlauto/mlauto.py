...
def new_project(project_name):
  ...
  if response.text == '0':
    ...
    if response_dict['exists'] == 0:
      ...
      os.environ["project_id"] = str(response_dic['project_id'])
    else:
      print("Using a project that already exists.")
      os.environ["project_id"] = str(response_dict['project_id'])
  
def start(block_name, block_order, import_block=False):
  data = {'block_name': block_name, 'block_order': block_order, 'username': os.environ['username'], 'key': os.environ['key'], 'project_id': os.environ['project_id']}
