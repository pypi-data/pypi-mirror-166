...
# pylint: disable=E1101,R0902,R0903,R0912,R0904,R0915,W0212
...
from .dbs import Class, find, last
from .dft import Default
from .evt import Event
from .jsn import save
from .obj import Object, edit, keys, printable, update
from .tbl import Callbacks
from .thr import launch
from .utl import elapsed, fntime, locked
...
class NoUser(Exception):
    ...
    "no matching user (origin) could be found."
...
class TextWrap(textwrap.TextWrapper):
    ...
    "irc textwrapping at 450 characters."
...
class Output(Object):
    ...
    def dosay(self, channel, txt):
        "used by output loop to display text."
    ...
    def oput(self, channel, txt):
        "put (channel, txt) to output queue."
...
class IRC(Client, Output):
    ...
    def command(self, cmd, *args):
        "send a command (with arguments to the server."
        if not args:
            self.raw(cmd)
        elif len(args) == 1:
    ...
    def dosay(self, channel, txt):
        "called from output loop."
        txt = str(txt).replace("\n", "")
        txt = txt.replace("  ", " ")
        self.command("PRIVMSG", channel, txt)

    def event(self, txt):
        "create irc event from text received on the socket."
    ...
    def poll(self):
        "poll the socket for input and return event from gotten input."
        self.connected.wait()
        if not self.buffer:
            self.some()
...
class Users(Object):
    ...
    def allowed(origin, perm):
        "check whether user (origin) has permission."
        perm = perm.upper()
    ...
    def perm(origin, permission):
        "return permission of a user (origin)."
        user = Users.get_user(origin)
        if not user:
            raise NoUser(origin)
...
def pwd(event):
    "create password from nickserv pass and nick."
    if len(event.args) != 2:
