...
def get_client(
    ...
    return MlFoundry(tracking_uri=tracking_uri)
...
def get_monitoring_client(
    ...
    if tracking_uri.startswith("file:"):
        raise MlFoundryException("Tracking URI for monitoring must be a rest endpoint")
    monitoring_uri = append_monitoring_path_to_rest_tracking_uri(tracking_uri)

    auth_service = AuthService(
        auth_server_info=RestMonitoringStore.get_auth_server_info(
            monitoring_uri=monitoring_uri
        )
    )
    session = Session(auth_service=auth_service, tracking_uri=monitoring_uri)
    session.init_session(api_key)

    amplitude.track(
        amplitude.Event.GET_MONITORING_CLIENT,
        # tracking whether user is using file:// or https://
        event_properties={
            "tracking_scheme": urllib.parse.urlparse(tracking_uri).scheme
        },
    ...
    return MonitoringClient(
        monitoring_uri=monitoring_uri, model_version_id=model_version_id
    )
