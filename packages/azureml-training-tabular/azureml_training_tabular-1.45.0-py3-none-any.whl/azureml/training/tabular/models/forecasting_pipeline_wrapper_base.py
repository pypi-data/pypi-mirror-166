...
class ForecastingPipelineWrapperBase(ABC):
    ...
    def _update_params(self, ts_transformer: 'TimeSeriesTransformer') -> None:
        ...
        self.forecast_origin = ts_transformer.dict_latest_date.copy()
    ...
    def _get_y_imputer_for_tsid(self, tsid: GrainType) -> 'TimeSeriesImputer':
        ...
        Contract.assert_value(imputer, 'ts_transformer.y_imputers[tsid]', log_safe=True)

        return cast('TimeSeriesImputer', imputer)
    ...
    def _prepare_prediction_input_common(self,
                                         ...
                                         ignore_data_errors: bool = False) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """
        ...
        return Xy_pred, dict_rename_back
        ...
        for tsid, df_one in Xy_pred.groupby(self.grain_column_names):
            ...
            if self.forecast_origin.get(tsid) is None:
                if not self.short_grain_handling():
                    ...
                    raise AzureMLError.create(
                        TimeseriesGrainAbsentNoGrainInTrain, target='X_pred',
            ...
            except AzureMLException as exc:
                # If failure is due to frequency misalignment. Attempt to fix it and try filling gaps again.
                if exc._azureml_error is not None and \
                   isinstance(exc._azureml_error.error_definition, TimeseriesDfFrequencyError):
    ...
    def rolling_forecast(self, X_pred: pd.DataFrame, y_pred: np.ndarray, step: int = 1,
        ...
        * Actual values column. Column name: stored as the object member actual_column_name
        ...
        Xy_pred = \
            self._prepare_prediction_data_for_rolling_forecast(Xy_pred_in, ignore_data_errors=ignore_data_errors)
        ...
        _, Xy_ts_features = \
            self._apply_preprocessors(preprocessors[:1], Xy_pred,
                                      select_latest_origin_times=False)

        # Call internal method for generating rolling forecasts; base classes must implement this method
        X_fcst = \
            self._rolling_forecast_internal(preprocessors[1:], forecaster, Xy_ts_features, step, ignore_data_errors)
        ...
        final_column_order = sort_columns + [self.forecast_column_name, self.actual_column_name]
        X_fcst = X_fcst[final_column_order]
        X_fcst.sort_values(sort_columns, inplace=True, ignore_index=True)
    ...
    def _warn_or_raise(
            self,
            error_definition_class: Type['ErrorDefinition'],
            ref_code: str,
            ignore_data_errors: bool,
            **kwargs: Any) -> None:
        """
    ...
    def preaggregate_data_set(
        ...
        **Note:** This method does not guarantee that the data set will be aggregated.
        ...
        """
        return self.static_preaggregate_data_set(
    ...
    def _check_convert_grain_types(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        tst = cast('TimeSeriesTransformer', self._ts_transformer)  # for mypy
    ...
    def _convert_time_column_name_safe(self, X: pd.DataFrame, reference_code: str) -> pd.DataFrame:
        ...
        try:
            ...
            raise AzureMLError.create(
                PandasDatetimeConversion, column=self.time_column_name,
    ...
    def _create_prediction_data_frame(self,
        ...
        if X_pred is not None:
            ...
            if self._get_effective_time_series_ids(X_copy) is None:
                ...
                X_copy[TimeSeriesInternal.DUMMY_GRAIN_COLUMN] = \
