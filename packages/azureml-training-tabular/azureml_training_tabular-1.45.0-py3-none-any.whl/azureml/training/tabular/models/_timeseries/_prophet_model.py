...
class ProphetModel(_MultiGrainForecastBase):
    ...
    def __init__(
        ...
        'prophet_param_dict' (or what `TimeSeriesInternal.PROPHET_PARAM_DICT` says)
    ...
    def _get_forecast_single_grain_impl(
        ...
        if (
            TimeSeriesInternal.DUMMY_GRAIN_COLUMN in df.columns
            and TimeSeriesInternal.DUMMY_GRAIN_COLUMN in df.index.names
        ):
            df.drop(TimeSeriesInternal.DUMMY_GRAIN_COLUMN, axis=1, inplace=True)
    ...
    def _fit_single_grain_impl(self, series_frame: TimeSeriesDataSet, grain_level: GrainType) -> Any:
        ...
        df.rename(columns={TimeSeriesInternal.DUMMY_TARGET_COLUMN: ProphetModel.PROPHET_TARGET_COLUMN}, inplace=True)
