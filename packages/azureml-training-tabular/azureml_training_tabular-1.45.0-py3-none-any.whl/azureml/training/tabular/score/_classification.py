...
from .._diagnostics.error_definitions import DataShapeMismatch, InvalidMetricForSingleValuedColumn
    ...
    def _validate_one_class(self):
        ...
        if is_one_class and (self._use_binary or is_class_averaged):
            raise AzureMLError.create(
                InvalidMetricForSingleValuedColumn, column="y_test", metric=f"AUC {self._average_type}"
            )
...
class AUCClasswise(ClassificationMetric, ClassMetric):
    ...
    def _validate_one_class(self):
        ...
        if is_one_class:
            raise AzureMLError.create(InvalidMetricForSingleValuedColumn, column="y_test", metric="AUC")
...
class IOUClasswise(ClassificationMetric, ClassMetric):
    ...
    def compute(self):
        """Compute the score for the metric."""
        classwise_iou_score = sklearn.metrics.jaccard_score(
            y_true=self._y_test,
            y_pred=self._y_pred,
            average=ClassificationMetric.NO_AVERAGE,
            sample_weight=self._sample_weight,
