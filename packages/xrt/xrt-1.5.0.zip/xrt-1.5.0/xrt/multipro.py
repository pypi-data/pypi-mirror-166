...
if useFastHistograms:
    import fast_histogram as fh
    histogram = fh.histogram1d
    histogram2d = fh.histogram2d
else:
    ...
    def do_hist1d(self, x, intensity, cDataRGB, axis):
        ...
        *axis*: XYCAxis instance
        ...
        if axis.density.lower() == 'kde':
            ...
            if cDataRGB is not None:
                for i in range(3):  # over RGB components
                    res = histogram(x, bins=axis.bins, range=axis.limits,
                                    weights=cDataRGB[:, i])
                    hist1dRGB[:, i] = res if useFastHistograms else res[0]
    ...
    def do_hist2d(self, x, y, intensity, cDataRGB, plot):
        ...
        *plot* instance of :class:`XYCPlot`: the plot hosting the 2D histogram.
        ...
        if plot.fluxKind.startswith('E'):
            ...
            hist2dr = res if useFastHistograms else res[0]

            res = histogram2d(
                y, x, bins=xybins, range=xyrange, weights=intensity.imag)
            hist2di = res if useFastHistograms else res[0]
            ...
            hist2d = res if useFastHistograms else res[0]
        ...
        if len(x) > 0:
            for i in range(3):  # over RGB components
                ...
                hist2dRGB[:, :, i] = res if useFastHistograms else res[0]
