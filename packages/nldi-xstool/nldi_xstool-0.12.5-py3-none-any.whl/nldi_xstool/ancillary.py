...
def get_dem(bbox: Tuple[float, float, float, float], res_type: str) -> bool:
    ...
    """
    url = "https://index.nationalmap.gov/arcgis/rest/services/3DEPElevationIndex/MapServer"
    wms = ArcGISRESTful(url, res_types[res_type])
    with contextlib.suppress(ServiceError, TypeError):
        return len(list(wms.oids_bygeom(bbox))[0]) > 0
