...
class ConanFileTextLoader(object):
    ...
    @property
    def docker(self):
        """returns a docker version
        EX: "sense2/1.0.0"
        """
        return [r.strip() for r in self._config_parser.docker.splitlines()]
    
    @property
    def sense_system(self):
        """returns sense2 build libs & bins
        EX: "sense-base/1.1.0"
        """
        return [r.strip() for r in self._config_parser.sense_system.splitlines()]
