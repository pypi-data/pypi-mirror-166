...
from sense2.client.conan_api import Conan, default_manifest_folder, _make_abs_path, ProfileData
...
class Command(object):
    ...
    def new(self, *args):
        ...
        parser.add_argument("-m", "--template",
                            help='Use the given template to generate a sense2 project')
        ...
        try:
            ...
            if packages_list:
                self._out.warn("Usage of `--package` argument is deprecated."
                               ...
                               "`sense2 download [...] {}:{}`".format(reference, packages_list[0]))
        else:
    ...
    def info(self, *args):
        ...
        if args.build_order:
            self._out.warn("Usage of `--build-order` argument is deprecated and can return"
                           " wrong results. Use `sense2 lock build-order ...` instead.")
        ...
        try:
            if "@" in args.path and ConanFileReference.loads(args.path):
                raise ArgumentError(None,
                                    "'sense2 source' doesn't accept a reference anymore. "
                                    "If you were using it as a concurrency workaround, "
                                    "you can call 'sense2 install' simultaneously from several "
    ...
    def package_deb(self, *args):
        """
        Calls your local conanfile.py 'package_deb()' method.
        """

        parser = argparse.ArgumentParser(description=self.build.__doc__,
                                         prog="sense2 package_deb",
                                         formatter_class=SmartFormatter)
        parser.add_argument("conanfile_path", help=_PATH_HELP)
        args = parser.parse_args(*args)
        self._warn_python_version()
        
        return self._conan.package_deb(conanfile_path=args.conanfile_path)
    ...
    def upload_deb(self, *args):
        ...
        return self._conan.upload_deb(conanfile_path=args.conanfile_path)
    ...
    def upload(self, *args):
        ...
        try:
            ...
            if package_id:
                self._out.warn("Usage of `--package` argument is deprecated."
                               " Use a full reference instead: "
                               "`sense2 upload [...] {}:{}`".format(reference, package_id))
    ...
    def _show_help(self):
        ...
        grps = [("Consumer commands", ("install", "config", "get", "info", "search")),
                ...
                ("Package development commands", ("source", "build", "package", "editable",
                                                  "workspace")),
                ("deb development commands", ("install_sense_system", "package_deb", 
                                              "install_deb", "deploy_deb", "install_docker",
                                              "upload_deb")),
