...
class CueLightMovementAnalyzer(object):
    ...
    def find_frames_when_cue_light_on(self):
        ...
        for cue_light in self.cue_light_names:
            ...
            light_bouts['Start_pre_window'] = light_bouts['Start_frame'] - self.prior_window_frames_cnt
            ...
            light_bouts['End_post_window'] = light_bouts['End_frame'] + self.post_window
            light_bouts['End_post_window'] = light_bouts['End_post_window'].clip(upper=len(self.data_df))
            self.light_on_dict[cue_light]['df'] = light_bouts
            self.light_on_dict[cue_light]['light_on_frames'] = list(self.data_df.index[self.data_df[cue_light] == 1])
            ...
            self.light_on_dict[cue_light]['pre_window_frames'] = pre_window_frames
            self.light_on_dict[cue_light]['post_window_frames'] = post_window_frames

    def calculate_whole_session_movement(self):
        self.results = {}
        if self.roi_setting: self.roi_results = {}
    ...
    def organize_results(self):
        self.results_df = pd.DataFrame(columns=['Video', 'Animal', 'Cue light', 'Time period', 'Meassure', 'Value'])
