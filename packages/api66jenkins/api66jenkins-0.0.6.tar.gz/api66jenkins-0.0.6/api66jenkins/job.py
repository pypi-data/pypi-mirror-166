...
class Project(Job, EnableMixIn):
    ...
    def build(self, **params):
        ...
        reserved_tow=["custom"]
        print(params)
        if not params or all(k in reserved for k in params): #检查传入参数是否包含关键字，或者没有参数
            entry = 'build'
        elif all(k in reserved_tow for k in params):  #检查k是否包括关键字custom，拿v ，**params 相当于将k=v 变为{k:v}转换后也是字典
            entry = 'buildWithParameters'
            if isinstance(params,dict):  #判断是不是字典
                params=params.get(reserved_tow[0])
        else:
            entry = 'buildWithParameters'
