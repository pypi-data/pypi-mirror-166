...
async def bootstrap(
    ...
    **kwargs,
    ...
    Args:
        ...
        tunnel_plugin(Text, Optional): The heist tunnel plugin to use to ssh into the instance. Defaults to asyncssh
...
async def verify_ssh(hub, ctx, instance_id: str, auto_configure: bool = False):
    ...
    Args:
        ...
        ctx:
        instance_id: An AWS EC2 Instance ID
        auto_configure: automatically apply internet_gateway/subnet/vpc/route_table changes needed to SSH into an instance
...
async def connect(
    ...
    **kwargs,
    ...
    Args:
        ...
        port(int, Optional): The port to connect to on the host.  Defaults to 22.
        ssh_public_key(Text, Optional): A public ssh key or path to send to the instance
        private_key_file(Text, Optional): The path to a private ssh key file
        availability_zone(Text, Optional): The Availability Zone in which the EC2 instance was launched
        tunnel_plugin(Text, Optional): The heist tunnel plugin to use to ssh into the instance. Defaults to asyncssh
        kwargs: All extra kwargs are used in asyncssh.SSHClientConnectionOptions
