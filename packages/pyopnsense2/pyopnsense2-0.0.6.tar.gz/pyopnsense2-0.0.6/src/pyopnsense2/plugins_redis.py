...
class redis_service(client.BaseClient):
    """A client for interacting with the OPNSense's redisservice API.
    for more information on used parameters, see : ['https://github.com/opnsense/plugins/blob/master/databases/redis/src/opnsense/mvc/app/models/OPNsense/Redis/Redis.xml']
    ...
    def stop(self, args={}, params={}):
        """"""
        url = "redis/service/stop/"
        data = {}

        data = self._get( url, args)
        return data

class redis_settings(client.BaseClient):
    """A client for interacting with the OPNSense's redissettings API.
    for more information on used parameters, see : ['https://github.com/opnsense/plugins/blob/master/databases/redis/src/opnsense/mvc/app/models/OPNsense/Redis/Redis.xml']
