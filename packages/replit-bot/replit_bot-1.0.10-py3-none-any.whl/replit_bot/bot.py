...
<hr><center><h1>Commands</h1></center><blockquote>{self.bio}</blockquote><hr>""".strip()
        for i in self.commands:
            ...
            html += f"""<center><h2>/{data['name']} parameters</h2></center>
...
<li>{j}</li>
    ...
    def run(self) -> None:
        """mainest runner code"""
        self.client.once("ready", lambda: None)
        self.create_docs()
        ...
        def _run(notif) -> None:
            ...
            if (getattr(notif, "comment", False)):
                ...
                self._call_when_followed(self.client, notif.creator["username"])
