...
class tc_filelist extends s {
  ...
  render() {
    ...
    return y`${this.menu} <strong>Path:${decodeURIComponent(this.url)}</strong><br><div>${prev} ${files.map(file => y`${o(file.type, [["dir", () => y`<a id="file-${file.name}" tc-filename="${file.name}" class="dir" href="#${this.urlRoot}/${this.url}/${file.name}/">${file.name}/</a>`], ["broken", () => y`<a id="file-${file.name}" tc-filename="${file.name}" class="broken" href="#${this.urlRoot}/${this.url}/${file.name}/">${file.name}/</a>`], ["file", () => y`<a class="file" id="file-${file.name}" tc-filename="${file.name}" href="${this.apiBase}/${this.url}/${file.name}" download="${file.name}">${file.name}</a>`], ["uploading", () => y`<a class="file" id="file-${file.name}" tc-filename="${file.name}" style="background-image:linear-gradient(to right,gray ${file.finished}%,var(--tc-background) ${file.finished}%)">${file.name}</a> - ${file.speed}`], ["mountpoint", () => y`<a class="mountpoint" id="file-${file.name}" tc-filename="${file.name}" href="#${this.urlRoot}/${this.url}/${file.name}">${file.name}</a>`]])}<br>`)}</div>`;
  }
...
export { setCookie as a, cleanPath as b, configureLocalization as c, tc_fileupload as d, i$1 as i, msg as m, s, tc_filelist as t, updateWhenLocaleChanges as u, y };
