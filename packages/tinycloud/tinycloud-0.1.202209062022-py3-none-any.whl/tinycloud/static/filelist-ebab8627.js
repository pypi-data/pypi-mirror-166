...
class tc_newshare extends s {
  ...
  render() {
    return y`<div id="newshare">${msg("Create share")}$nbsp${this.path}<br><input id="write" type="checkbox">${msg("Write access")}<button @click="${this.remove}">${msg("Cancel")}</button><button @click="${this.createShare}">${msg("Create")}</button></div>`;
  }
