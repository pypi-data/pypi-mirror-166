...
class tc_shares extends s {
  ...
  render() {
    ...
    for (var i in this.shares) {
      ...
      h.push(y`<a href=/shares/${i}>${location.origin}/shares/${i}</a>&nbspPath: ${path}&nbsp|&nbspUser:${this.shares[i].username}<button @onclick={()=>{this.removeShare(i)}}>del</button></br>`);
    }
...
class tc_newshare extends s {
  ...
  render() {
    return y`<div id=newshare >${msg("Create share")}&nbsp${this.path}</br><input id=write type=checkbox>${msg("Write access")}</input></br><div align=center id=button><button @click=${this.remove}>${msg("Cancel")}</button><button @click=${this.createShare}>${msg("Create")}</button></div></div>`;
  }
