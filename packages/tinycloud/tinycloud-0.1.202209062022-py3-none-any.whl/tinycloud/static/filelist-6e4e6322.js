...
class tc_shares extends s {
  ...
  render() {
    ...
    for (var i in this.shares) {
      ...
      h.push(y`<a href="/shares/${i}">${location.origin}/shares/${i}</a> Path: ${path} | User:${this.shares[i].username}<button @click="${() => {
        this.removeShare(i);
      }}">del</button><br>`);
    }
...
class tc_newshare extends s {
  ...
  static styles = i$1`#newshare{width:30%;height:15%;overflow:auto;margin:auto;position:fixed;top:0;left:0;bottom:0;right:0;z-index:99999;background-color:inherit}`;
  ...
  render() {
    return y`<div id="newshare">${msg("Create share")} ${this.path}<br><input id="write" type="checkbox">${msg("Write access")}<br><div align="center" id="button"><button @click="${this.remove}">${msg("Cancel")}</button><button @click="${this.createShare}">${msg("Create")}</button></div></div>`;
  }
...
class tc_filelist extends s {
  ...
  delete_file = filename => {
    ...
    fetch(cleanPath(this.apiBase + this.url + "/" + filename) + "?json_mode=1", {
      method: "DELETE"
    }).then(res => {
      if (res.ok) {
        this.loadData();
