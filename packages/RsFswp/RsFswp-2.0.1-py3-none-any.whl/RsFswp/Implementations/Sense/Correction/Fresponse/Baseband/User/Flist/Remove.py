...
class Remove:
	...
	def set(self, fileList=repcap.FileList.Default) -> None:
		...
		Snippet: driver.sense.correction.fresponse.baseband.user.flist.remove.set(fileList = repcap.FileList.Default) \n
		...
		self._core.io.write(f'SENSe:CORRection:FRESponse:BASeband:USER:FLISt{fileList_cmd_val}:REMove')

	def set_with_opc(self, fileList=repcap.FileList.Default, opc_timeout_ms: int = -1) -> None:
		fileList_cmd_val = self._cmd_group.get_repcap_cmd_value(fileList, repcap.FileList)
		"""SCPI: [SENSe]:CORRection:FRESponse:BASeband:USER:FLISt<fli>:REMove \n
		Snippet: driver.sense.correction.fresponse.baseband.user.flist.remove.set_with_opc(fileList = repcap.FileList.Default) \n
		No command help available \n
		...
		self._core.io.write_with_opc(f'SENSe:CORRection:FRESponse:BASeband:USER:FLISt{fileList_cmd_val}:REMove', opc_timeout_ms)
