...
class Remove:
	...
	def set(self, touchStone=repcap.TouchStone.Default) -> None:
		...
		Snippet: driver.sense.correction.fresponse.user.slist.remove.set(touchStone = repcap.TouchStone.Default) \n
		...
		self._core.io.write(f'SENSe:CORRection:FRESponse:USER:SLISt{touchStone_cmd_val}:REMove')

	def set_with_opc(self, touchStone=repcap.TouchStone.Default, opc_timeout_ms: int = -1) -> None:
		touchStone_cmd_val = self._cmd_group.get_repcap_cmd_value(touchStone, repcap.TouchStone)
		"""SCPI: [SENSe]:CORRection:FRESponse:USER:SLISt<sli>:REMove \n
		Snippet: driver.sense.correction.fresponse.user.slist.remove.set_with_opc(touchStone = repcap.TouchStone.Default) \n
		No command help available \n
		...
		self._core.io.write_with_opc(f'SENSe:CORRection:FRESponse:USER:SLISt{touchStone_cmd_val}:REMove', opc_timeout_ms)
