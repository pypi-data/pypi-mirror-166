...
class Enable:
	...
	def set(self, summary_bit: int, channel_name: str = None, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.status.questionable.lmargin.enable.set(summary_bit = 1, channel_name = '1', window = repcap.Window.Default) \n
		...
		self._core.io.write(f'STATus:QUEStionable:LMARgin{window_cmd_val}:ENABle {param}'.rstrip())
	...
	def get(self, window=repcap.Window.Default) -> EnableStruct:
		...
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'STATus:QUEStionable:LMARgin{window_cmd_val}:ENABle?', self.__class__.EnableStruct())
