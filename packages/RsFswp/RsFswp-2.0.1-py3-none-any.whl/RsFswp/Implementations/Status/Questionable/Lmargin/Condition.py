...
class Condition:
	...
	def get(self, window=repcap.Window.Default) -> str:
		...
		Snippet: value: str = driver.status.questionable.lmargin.condition.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'STATus:QUEStionable:LMARgin{window_cmd_val}:CONDition?')
