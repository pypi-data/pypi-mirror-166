...
class Ptransition:
	...
	def set(self, summary_bit: int, channel_name: str = None, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.status.questionable.lmargin.ptransition.set(summary_bit = 1, channel_name = '1', window = repcap.Window.Default) \n
		...
		self._core.io.write(f'STATus:QUEStionable:LMARgin{window_cmd_val}:PTRansition {param}'.rstrip())
	...
	def get(self, window=repcap.Window.Default) -> PtransitionStruct:
		"""SCPI: STATus:QUEStionable:LMARgin<1|2|3|4>:PTRansition \n
		Snippet: value: PtransitionStruct = driver.status.questionable.lmargin.ptransition.get(window = repcap.Window.Default) \n
		These commands control the Positive TRansition part of a register. Setting a bit causes a 0 to 1 transition in the
		corresponding bit of the associated register. The transition also writes a 1 into the associated bit of the corresponding
		EVENt register. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Lmargin')
		...
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'STATus:QUEStionable:LMARgin{window_cmd_val}:PTRansition?', self.__class__.PtransitionStruct())
