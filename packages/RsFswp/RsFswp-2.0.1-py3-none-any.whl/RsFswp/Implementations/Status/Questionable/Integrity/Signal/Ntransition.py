...
class Ntransition:
	...
	def set(self, bit_definition: int, channel_name: str = None) -> None:
		...
		Snippet: driver.status.questionable.integrity.signal.ntransition.set(bit_definition = 1, channel_name = '1') \n
		...
		self._core.io.write(f'STATus:QUEStionable:INTegrity:SIGNal:NTRansition {param}'.rstrip())
	...
	def get(self) -> NtransitionStruct:
		"""SCPI: STATus:QUEStionable:INTegrity:SIGNal:NTRansition \n
		Snippet: value: NtransitionStruct = driver.status.questionable.integrity.signal.ntransition.get() \n
		No command help available \n
			:return: structure: for return value, see the help for NtransitionStruct structure arguments."""
		return self._core.io.query_struct(f'STATus:QUEStionable:INTegrity:SIGNal:NTRansition?', self.__class__.NtransitionStruct())
