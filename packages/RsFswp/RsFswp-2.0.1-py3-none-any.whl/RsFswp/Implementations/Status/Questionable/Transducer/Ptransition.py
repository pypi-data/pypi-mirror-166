...
class Ptransition:
	...
	def set(self, bit_definition: int, channel_name: str = None) -> None:
		...
		Snippet: driver.status.questionable.transducer.ptransition.set(bit_definition = 1, channel_name = '1') \n
		...
		self._core.io.write(f'STATus:QUEStionable:TRANsducer:PTRansition {param}'.rstrip())
	...
	def get(self) -> PtransitionStruct:
		"""SCPI: STATus:QUEStionable:TRANsducer:PTRansition \n
		Snippet: value: PtransitionStruct = driver.status.questionable.transducer.ptransition.get() \n
		No command help available \n
			:return: structure: for return value, see the help for PtransitionStruct structure arguments."""
		return self._core.io.query_struct(f'STATus:QUEStionable:TRANsducer:PTRansition?', self.__class__.PtransitionStruct())
