...
class Low:
	...
	def set(self, voltage_current: float, source=repcap.Source.Default) -> None:
		...
		Snippet: driver.source.voltage.power.level.limit.low.set(voltage_current = 1.0, source = repcap.Source.Default) \n
	...
	def get(self, source=repcap.Source.Default) -> float:
		"""SCPI: SOURce:VOLTage:POWer<1|2>:LEVel:LIMit:LOW \n
		Snippet: value: float = driver.source.voltage.power.level.limit.low.get(source = repcap.Source.Default) \n
		This command defines the minimum voltage that may be supplied by the Vsupply source. \n
			:param source: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Power')
