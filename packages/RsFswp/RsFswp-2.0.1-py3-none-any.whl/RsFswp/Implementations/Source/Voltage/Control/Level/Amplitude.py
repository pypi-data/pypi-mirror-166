...
class Amplitude:
	...
	def set(self, voltage: float, source=repcap.Source.Default) -> None:
		...
		Snippet: driver.source.voltage.control.level.amplitude.set(voltage = 1.0, source = repcap.Source.Default) \n
	...
	def get(self, source=repcap.Source.Default) -> float:
		"""SCPI: SOURce:VOLTage:CONTrol<1|2>:LEVel:AMPLitude \n
		Snippet: value: float = driver.source.voltage.control.level.amplitude.get(source = repcap.Source.Default) \n
		This command defines the output voltage for the Vtune source. \n
			:param source: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Control')
