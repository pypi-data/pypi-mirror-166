...
class State:
	...
	def set(self, state: bool, source=repcap.Source.Default) -> None:
		...
		Snippet: driver.source.voltage.control.level.state.set(state = False, source = repcap.Source.Default) \n
		This command turns the tuning voltage source (Vtune) on and off. Note that DC power is actually supplied only if you
		additionally activate the outputs in general.
			INTRO_CMD_HELP: Prerequisites for this command \n
			- Turn on DC power sources (method RsFswp.Source.Voltage.State.set) . \n
	...
	def get(self, source=repcap.Source.Default) -> bool:
		"""SCPI: SOURce:VOLTage:CONTrol<1|2>:LEVel[:STATe] \n
		Snippet: value: bool = driver.source.voltage.control.level.state.get(source = repcap.Source.Default) \n
		This command turns the tuning voltage source (Vtune) on and off. Note that DC power is actually supplied only if you
		additionally activate the outputs in general.
			INTRO_CMD_HELP: Prerequisites for this command \n
			- Turn on DC power sources (method RsFswp.Source.Voltage.State.set) . \n
