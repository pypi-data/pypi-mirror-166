...
class Present:
	...
	def get(self, window=repcap.Window.Default) -> bool:
		...
		Snippet: value: bool = driver.applications.k70Vsa.calculate.dsp.result.rrange.current.burst.present.get(window = repcap.Window.Default) \n
		Queries whether a burst is present or not in the current result range. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:return: present: ON | OFF | 0 | 1 OFF | 0 Burst not available. ON | 1 Burst available"""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		response = self._core.io.query_str(f'CALCulate{window_cmd_val}:DSP:RESult:RRANge:CURRent:BURSt:PRESent?')
