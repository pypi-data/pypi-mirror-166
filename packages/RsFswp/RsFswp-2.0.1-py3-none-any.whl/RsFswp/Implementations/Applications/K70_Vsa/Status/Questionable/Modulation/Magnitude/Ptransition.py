...
class Ptransition:
	...
	def set(self, bit_definition: int, channel_name: str = None, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k70Vsa.status.questionable.modulation.magnitude.ptransition.set(bit_definition = 1, channel_name = '1', window = repcap.Window.Default) \n
		...
		self._core.io.write(f'STATus:QUEStionable:MODulation{window_cmd_val}:MAGNitude:PTRansition {param}'.rstrip())
	...
	def get(self, window=repcap.Window.Default) -> PtransitionStruct:
		"""SCPI: STATus:QUEStionable:MODulation<Window>:MAGNitude:PTRansition \n
		Snippet: value: PtransitionStruct = driver.applications.k70Vsa.status.questionable.modulation.magnitude.ptransition.get(window = repcap.Window.Default) \n
		These commands control the Positive TRansition part of a register. Setting a bit causes a 0 to 1 transition in the
		corresponding bit of the associated register. The transition also writes a 1 into the associated bit of the corresponding
		EVENt register. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Modulation')
		...
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'STATus:QUEStionable:MODulation{window_cmd_val}:MAGNitude:PTRansition?', self.__class__.PtransitionStruct())
