...
class High:
	...
	def set(self, voltage: float, source=repcap.Source.Default) -> None:
		...
		Snippet: driver.applications.k30NoiseFigure.source.voltage.power.limit.high.set(voltage = 1.0, source = repcap.Source.Default) \n
	...
	def get(self, source=repcap.Source.Default) -> float:
		"""SCPI: SOURce:VOLTage:POWer<i>:LIMit:HIGH \n
		Snippet: value: float = driver.applications.k30NoiseFigure.source.voltage.power.limit.high.get(source = repcap.Source.Default) \n
		No command help available \n
			:param source: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Power')
