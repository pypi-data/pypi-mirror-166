...
class State:
	...
	def set(self, arg_0: bool, powerMeter=repcap.PowerMeter.Default) -> None:
		...
		Snippet: driver.applications.k50Spurious.sense.pmeter.mtime.average.state.set(arg_0 = False, powerMeter = repcap.PowerMeter.Default) \n
	...
	def get(self, powerMeter=repcap.PowerMeter.Default) -> bool:
		"""SCPI: [SENSe]:PMETer<p>:MTIMe:AVERage[:STATe] \n
		Snippet: value: bool = driver.applications.k50Spurious.sense.pmeter.mtime.average.state.get(powerMeter = repcap.PowerMeter.Default) \n
		This command turns averaging for power sensor measurements on and off. \n
			:param powerMeter: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Pmeter')
