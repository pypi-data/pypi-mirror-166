...
class Result:
	...
	def get(self, window=repcap.Window.Default, marker=repcap.Marker.Default, trace=repcap.Trace.Default) -> float:
		...
		from the current measurement data (CLR/WRITE) . The modulation is removed using low pass filtering. The offset thus
		determined differs from that calculated in the FM:OFFSet command which uses averaging to determine the frequency
		deviation. \n
