...
class Spacing:
	...
	def set(self, spacing: enums.SpacingY, window=repcap.Window.Default, trace=repcap.Trace.Default) -> None:
		...
		Snippet: driver.display.window.trace.y.spacing.set(spacing = enums.SpacingY.LDB, window = repcap.Window.Default, trace = repcap.Trace.Default) \n
	...
	def get(self, window=repcap.Window.Default, trace=repcap.Trace.Default) -> enums.SpacingY:
		"""SCPI: DISPlay[:WINDow<n>]:TRACe<t>:Y:SPACing \n
		Snippet: value: enums.SpacingY = driver.display.window.trace.y.spacing.get(window = repcap.Window.Default, trace = repcap.Trace.Default) \n
		No command help available \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:param trace: optional repeated capability selector. Default value: Tr1 (settable in the interface 'Trace')
