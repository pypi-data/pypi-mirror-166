...
class Event:
	...
	def get(self, window=repcap.Window.Default) -> str:
		...
		Snippet: value: str = driver.status.questionable.limit.event.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_str(f'STATus:QUEStionable:LIMit{window_cmd_val}:EVENt?')
