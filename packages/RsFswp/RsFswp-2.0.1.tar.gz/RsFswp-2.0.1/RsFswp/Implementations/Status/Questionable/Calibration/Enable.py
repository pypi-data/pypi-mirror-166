...
class Enable:
	...
	def set(self, bit_definition: int, channel_name: str = None) -> None:
		...
		Snippet: driver.status.questionable.calibration.enable.set(bit_definition = 1, channel_name = '1') \n
		...
		self._core.io.write(f'STATus:QUEStionable:CALibration:ENABle {param}'.rstrip())
	...
	def get(self) -> EnableStruct:
		"""SCPI: STATus:QUEStionable:CALibration:ENABle \n
		Snippet: value: EnableStruct = driver.status.questionable.calibration.enable.get() \n
		No command help available \n
			:return: structure: for return value, see the help for EnableStruct structure arguments."""
		return self._core.io.query_struct(f'STATus:QUEStionable:CALibration:ENABle?', self.__class__.EnableStruct())
