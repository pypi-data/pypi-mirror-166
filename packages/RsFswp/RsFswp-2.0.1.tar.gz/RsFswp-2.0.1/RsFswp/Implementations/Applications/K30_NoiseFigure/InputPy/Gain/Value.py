...
class Value:
	...
	def set(self, gain: float, inputIx=repcap.InputIx.Default) -> None:
		...
		K30_NoiseFigure.InputPy.Gain.State.set) . The command requires the additional preamplifier hardware option. \n
			:param gain: 15 dB | 30 dB The availability of gain levels depends on the model of the R&S FSWP. R&S FSWP8/26: 15dB and 30 dB R&S FSWP50: 30 dB Unit: DB
	...
	def get(self, inputIx=repcap.InputIx.Default) -> float:
		...
		K30_NoiseFigure.InputPy.Gain.State.set) . The command requires the additional preamplifier hardware option. \n
			:param inputIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'InputPy')
			:return: gain: 15 dB | 30 dB The availability of gain levels depends on the model of the R&S FSWP. R&S FSWP8/26: 15dB and 30 dB R&S FSWP50: 30 dB Unit: DB"""
		inputIx_cmd_val = self._cmd_group.get_repcap_cmd_value(inputIx, repcap.InputIx)
