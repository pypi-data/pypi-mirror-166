...
class Spot:
	...
	def set(self, loss: float) -> None:
		...
		Snippet: driver.applications.k30NoiseFigure.sense.correction.loss.output.spot.set(loss = 1.0) \n
		...
		self._core.io.write(f'SENSe:CORRection:LOSS:OUTPut:SPOT {param}')

	def get(self) -> float:
		"""SCPI: [SENSe]:CORRection:LOSS:OUTPut:SPOT \n
		Snippet: value: float = driver.applications.k30NoiseFigure.sense.correction.loss.output.spot.get() \n
		This command defines a constant output loss for all measurement points. \n
			:return: loss: Range: -999.99 to 999.99, Unit: DB"""
		response = self._core.io.query_str(f'SENSe:CORRection:LOSS:OUTPut:SPOT?')
