...
class Ntransition:
	...
	def set(self, bit_definition: int, channel_name: str = None, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k70Vsa.status.questionable.modulation.iqRho.ntransition.set(bit_definition = 1, channel_name = '1', window = repcap.Window.Default) \n
		...
		self._core.io.write(f'STATus:QUEStionable:MODulation{window_cmd_val}:IQRHo:NTRansition {param}'.rstrip())
	...
	def get(self, window=repcap.Window.Default) -> NtransitionStruct:
		"""SCPI: STATus:QUEStionable:MODulation<Window>:IQRHo:NTRansition \n
		Snippet: value: NtransitionStruct = driver.applications.k70Vsa.status.questionable.modulation.iqRho.ntransition.get(window = repcap.Window.Default) \n
		This command controls the Negative TRansition part of a register. Setting a bit causes a 1 to 0 transition in the
		corresponding bit of the associated register. The transition also writes a 1 into the associated bit of the corresponding
		EVENt register. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Modulation')
		...
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
		return self._core.io.query_struct(f'STATus:QUEStionable:MODulation{window_cmd_val}:IQRHo:NTRansition?', self.__class__.NtransitionStruct())
