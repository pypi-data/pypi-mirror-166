...
class State:
	...
	def set(self, arg_0: bool, window=repcap.Window.Default, powerMeter=repcap.PowerMeter.Default) -> None:
		...
		Snippet: driver.applications.k50Spurious.calculate.pmeter.relative.state.set(arg_0 = False, window = repcap.Window.Default, powerMeter = repcap.PowerMeter.Default) \n
	...
	def get(self, window=repcap.Window.Default, powerMeter=repcap.PowerMeter.Default) -> bool:
		"""SCPI: CALCulate<n>:PMETer<p>:RELative:STATe \n
		Snippet: value: bool = driver.applications.k50Spurious.calculate.pmeter.relative.state.get(window = repcap.Window.Default, powerMeter = repcap.PowerMeter.Default) \n
		This command turns relative power sensor measurements on and off. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Calculate')
			:param powerMeter: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Pmeter')
