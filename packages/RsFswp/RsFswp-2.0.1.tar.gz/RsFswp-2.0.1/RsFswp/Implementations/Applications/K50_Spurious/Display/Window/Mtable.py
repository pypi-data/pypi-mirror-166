...
class Mtable:
	...
	def set(self, display_mode: enums.AutoMode, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k50Spurious.display.window.mtable.set(display_mode = enums.AutoMode.AUTO, window = repcap.Window.Default) \n
		This command turns the marker table on and off. \n
			:param display_mode: ON | 1 Turns on the marker table. OFF | 0 Turns off the marker table.
	...
	def get(self, window=repcap.Window.Default) -> enums.AutoMode:
		"""SCPI: DISPlay[:WINDow<n>]:MTABle \n
		Snippet: value: enums.AutoMode = driver.applications.k50Spurious.display.window.mtable.get(window = repcap.Window.Default) \n
		This command turns the marker table on and off. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:return: display_mode: ON | 1 Turns on the marker table. OFF | 0 Turns off the marker table."""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
