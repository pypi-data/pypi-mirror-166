...
class Rvalue:
	...
	def set(self, value: float, window=repcap.Window.Default) -> None:
		...
		Snippet: driver.applications.k60Transient.display.window.trace.y.scale.rvalue.set(value = 1.0, window = repcap.Window.Default) \n
		...
		values are maintained for the various displays. \n
			:param value: numeric value WITHOUT UNIT Unit: dBm
	...
	def get(self, window=repcap.Window.Default) -> float:
		"""SCPI: DISPlay[:WINDow<n>]:TRACe:Y[:SCALe]:RVALue \n
		Snippet: value: float = driver.applications.k60Transient.display.window.trace.y.scale.rvalue.get(window = repcap.Window.Default) \n
		This command defines the reference value assigned to the reference position in the specified window. Separate reference
		values are maintained for the various displays. \n
			:param window: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Window')
			:return: value: numeric value WITHOUT UNIT Unit: dBm"""
		window_cmd_val = self._cmd_group.get_repcap_cmd_value(window, repcap.Window)
