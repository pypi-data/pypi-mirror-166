...
class Trigger:
	...
	def get(self, window=repcap.Window.Default) -> List[float]:
		...
		Snippet: value: List[float] = driver.trace.iq.scapture.tstamp.trigger.get(window = repcap.Window.Default) \n
		...
		response = self._core.io.query_bin_or_ascii_float_list(f'TRACe{window_cmd_val}:IQ:SCAPture:TSTamp:TRIGger?')
		return response
