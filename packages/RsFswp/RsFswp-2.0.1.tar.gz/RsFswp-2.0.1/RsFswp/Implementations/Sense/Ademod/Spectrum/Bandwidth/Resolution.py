...
class Resolution:
	...
	def set(self, bandwidth: float) -> None:
		...
		Defines the resolution bandwidth for data acquisition. From the specified RBW and the demodulation span set by SPECtrum
		or DEMod, the required measurement time is calculated. If the available measurement time is not sufficient for the given
		bandwidth, the measurement time is set to its maximum and the resolution bandwidth is increased to the resulting
		bandwidth. This command is identical to [SENSe:​]BANDwidth[:​RESolution]. \n
