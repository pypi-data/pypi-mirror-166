...
from typing import TYPE_CHECKING, AsyncIterable, Dict, Optional

from tesseract_olap.common.types import AnyDict
...
class Backend(abc.ABC):
    ...
    @abc.abstractmethod
    def get_data(self, query: "DataQuery", **kwargs) -> AsyncIterable[AnyDict]:
        """Sends the :class:`DataQuery` instance to the backend server for
        retrieval of the intended data.
        """
        raise NotImplementedError()

    @abc.abstractmethod
    def get_members(self, query: "MembersQuery", **kwargs) -> AsyncIterable[AnyDict]:
        """Obtains a list of the members for a level, and its related properties.
        """
        raise NotImplementedError()
