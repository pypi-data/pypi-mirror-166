...
def make_dataset_bins(n_valid: int, y: np.ndarray, percentile: float = 1.0) -> Dict[str, Any]:
    ...
    try:
        ...
        inner_edges = [_round_bin_edge(edge, bin_width) for edge in inner_edges]  # type: ignore
...
def _round_bin_edge(edge: float, bin_width: float, decimals: int = 2, direction: str = 'nearest') -> np.float_:
    """
