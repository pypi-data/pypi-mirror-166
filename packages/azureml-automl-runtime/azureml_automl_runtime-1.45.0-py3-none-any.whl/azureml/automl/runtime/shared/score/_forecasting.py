...
class ForecastTable(ForecastingMetric, NonScalarMetric):
    ...
    def compute(self) -> Dict[str, Any]:
        ...
        for igrain, (grain, df_one_train) in enumerate(df_train.groupby(grain_column_names)):
            ...
            # The validation set length is defined by the forecast horizon, which may be short or equal to 1.
            # In this case standard deviation of residuals may be zero if all residuals are the same. To
            # have the confidence intervals in this case we are calculating standard deviation of the residuals
            # obtained by subtracting of the last prediction from the training data set.
            if stddev == 0:
                # If all residuals are the same, we will calculate the residuals of training set.
