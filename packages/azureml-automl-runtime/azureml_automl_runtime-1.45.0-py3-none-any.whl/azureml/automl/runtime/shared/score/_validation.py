...
def _check_array_values(arr: np.ndarray,
    ...
    if arr.dtype.kind in set('bcfiu'):
        ...
        Contract.assert_true(bool(~np.isnan(arr).any()), message=message.format(name, 'NaN'), log_safe=True)
        Contract.assert_true(bool(np.isfinite(arr).all()), message=message.format(name, 'infinite'), log_safe=True)
