from random import choice, randint, uniform, random, shuffle
...
def foxInt(xInt = None, yInt = None):
	...
	try:
		...
		print('Произошла ошибка: Похоже X(икс) или Y(игрик) (foxInt) не являются числом или вы используете десятичные числа и т.п.! Данная функция принимает только обычные цифры например "-1", "5"!')
		return '-----------------------------------------------------------'
	except:
...
def foxUniform(xInt, yInt):
	if xInt == None and yInt == None:
		...
		xInt = 0
	try:
		return uniform(xInt, yInt)
	except ValueError:
		print('Произошла ошибка: Похоже X(икс) или Y(игрик) (foxUniform) не являются числом или вы используете десятичные числа и т.п.! Данная функция принимает только обычные цифры например "-45", "65"!')
		return '-----------------------------------------------------------'
	except:
		print('Произошла неизвестная ошибка с функцией foxUniform! Если вы хотите, можете скинуть ошибку(или скрин ошибки) мне на почту bfire1999@gmail.com!')
		return '-----------------------------------------------------------'
...
def foxPassword():
	return f'{foxInt(0, 30)}{foxInt(0, 30)}{foxChoice(["b", "d", "_", "", "R", "G", "B"])}{foxInt(0, 30)}{foxInt(0, 30)}{foxInt(0, 30)}{foxInt(0, 30)}{foxChoice(["b", "d", "_", "", "R", "G", "B"])}{foxChoice(["b", "d", "", "R", "G", "B"])}'
