...
def translate_net2json( res ):
    ...
    if res.find("gregory")==0:
        # echo "gregory_mea2: b0 ch0 100 30 10" | nc -u 127.0.0.1 8100
        res2 = '"fields":{'
        r = res.split(":")[-1].strip()
        r = r.replace("  "," ").split(" ")
    ...
    # this can be used recursively.....................A PREPARED JSON WILL BE PROCESSED TO DATABASE
    if res.find("influxme ")==0:
...
class Handler_UDPServer(socketserver.DatagramRequestHandler):
    def handle(self):
        ...
        #print("UDP...:",res)
        print(f"{fg.grey}U{fg.default}",end="", flush=True)
