...
def measure_images(image_path, scale=2, metrics= "ALL", channels= 1, decimation="bicubic",
    ...
    """
        ...
        maintained from the images from the path. The images to be evaluated generated using
...
def calc_multi_metrics_of_image(img_ground, image, border=0, channels=1,
                                metrics=["PSNR", "SSIM"], fn_user_metrics=None):
    """
    ...
    Addition to given metrics by the parameter 'metrics', user-defined metrics can
    also be computed. For this, user should provide the function handle with the parameter
    'fn_user_metrics'. This function should take two numpy's ndarray and return
    a dictionary with (name, value) pairs for each computed metric. The should look like this:
        :example:
        def fn_user_metrics(ground_img, output_img):
            res_dict = dict()
            for each metric in user-defined_metrics:
                res_dict[<name of metric>] = computed value for the metric

            return res_dict

    This function can be defined by the user with the same name (fn_user_metrics) in the model file.
    In this case, this function is added as a member method to the DeepSR class while reading
    the model file. It is not necessary to provide the function handle as a parameter in this case.
    The existence of this member method is checked, and if it is found,it is run automatically in any case.
    ...
    :param fn_user_metrics: A function computing the discrepancy between reference image
         and output image by user-defined metric(s). This function returns a dictionary
         with name,value pairs for each metric computed inside the function.
    ...
    if igcol == imcol and igrow == imrow:
        ...
        if "GMSD" in metrics or "ALL" in metrics:
            if len(image.shape) > 2:
                print('An 2d array is required for GMSD but 3d given. The mean of GMSD values over all channels taken.')
                gmsds = [gmsd(img_ground[:,:,i], image[:,:,i], rescale=True, returnMap=False) for i in range(image.shape[2])]

                results['GMSD'] = np.mean(gmsds)
            else:
        ...
        if "MAD" in metrics or "ALL" in metrics:
            if len(image.shape) > 2:
                print('Given image contains 3 channels. MAD expects only one (luminance).'
                      'The first channel is being used for computation of MAD')
                results["MAD"] = mad.mad(img_ground[:,:,0], image[:,:,0])[0]
            else:
                results["MAD"] = mad.mad(img_ground, image)[0]

        if "MSE" in metrics or "ALL" in metrics:
            results["MSE"] = mean_squared_error(img_ground, image)
        ...
        if "NIQE" in metrics or "ALL" in metrics:
            ...
            if (image.shape[0] <= 192 or image.shape[1] <= 192):
                ...
                results["NIQE"]= None
                    ...
                    print('Given image contains 3 channels. NIQE expects only one (luminance).'
                          'The first channel is being used for computation of NIQE')
                    results["NIQE"] = niqe(image[:,:,0])[0]
            else:
        ...
        if "NRMSE" in metrics or "ALL" in metrics:
            results["NRMSE"] = normalized_root_mse(img_ground, image)
        ...
        if "PSNR" in metrics or "ALL" in metrics:
            results["PSNR"] = peak_signal_noise_ratio(img_ground, image)
        ...
        # User-defined metrics can also be calculated. These metrics can be
        # given either with fn_user_metrics parameter, or
        # by providing the function named fn_user_metrics in the model file.
        # The function is expected to return a dictionary containing name, value
        # pairs of each calculated metric.

        # If the user-defined functions is given with the parameter of this method
        if fn_user_metrics is not None:

            if not callable(fn_user_metrics):  # check if a valid function is given
                raise Exception("The parameter 'fn_user_metrics' is not a callable object."
                                "This should be a function returning the <name, computed value> pairs for "
                                "each user-defined metrics")

            res_dict = fn_user_metrics(img_ground, image)
            for name, value in res_dict.items(): #iterate each calculated metric
                results[name] = value
        # ---------------------------------------------------------------
...
def calc_metrics_of_image(img_ground, image, border=0, channels=1, metrics=["PSNR", "SSIM"]):
    ...
    if igcol==imcol and igrow==imrow:
        ...
        psnr = peak_signal_noise_ratio(img_ground, image)
...
def augment_image(image, options, fn_user_augmentation=None):
    """
    ...
    User defined augmentation procedure given via the parameter, fn_augment, is also performed.

    :param image:
    :param options: a list consists of [90,180,270, 'flipud', 'fliplr', 'flipudlr']
    :param fn_user_augmentation: User-defined function for image augmentation. This function
        takes an image as ndarray, and returns a list of images.
    ...
    # user-defined augmentation function
    if fn_user_augmentation is not None :
        if not callable(fn_user_augmentation): # check if a valid function is given
            raise Exception("The parameter 'fn_user_augmentation' is not callable!"
                            "This should be a function returning a list of images (as ndarrays) "
                            "generated in an user-defined augmentation process.")
        else:
            image_list = fn_user_augmentation(image)
