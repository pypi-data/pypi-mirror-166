...
parser.add_argument("--realcase", action="store_true", default=False,
                    help="Boolean. Default True. DeepSR class also stores its value (True, or False) as a member with the same name."
                         "True, to generate simulated Low resolution images from input images which"
                         "are considered as high resolution. False, to separately feed low and high resolution versions of images."
                         "In this case, the path to low resolution images should be given by the command argument --traindir_lr."
                         "LR to HR counter parts are picked from their own paths with the same order, and fed to the DL model in order"
                         "The path to low resolution images is saved in the class member of DeepSR with the name 'traindir_lr'."
                         "When simulate variable is False, pre-process step for generating simulated low resolution images is "
                         "skipped."
                        )
...
parser.add_argument("--traindir_lr", type=str, default='',
                    help="String. The directory path where the training images in.")
...
def getArgs():
    ...
    return vars(parser.parse_known_args()[0]) # parse_known_args returns args, unknown_args
