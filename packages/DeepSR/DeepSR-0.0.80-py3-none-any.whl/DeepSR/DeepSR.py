...
class DeepSR(object):
    ...
    def get_number_of_samples(self, imagedir):
        ...
        if len(files) < 1:
            raise Exception('Not any file found in the path: \n\t\t\t' + str(imagedir))
        ...
        for f in files:
            ...
            for im_aug in utils.augment_image(img, self.augment, self.fn_user_augmentation):
    ...
    def generate_batch_on_fly(self, imagedir, imagedir_lr=None, shuffle = True):
        """
        ...
        :param imagedir_lr: path to low resolution images. Default None. Low resolution images are used
            in the real-life scenario where real low and high resolution images exist. LR - HR image counter
            parts are taken from the paths, imagedir (HR) and imagedir_lr (LR). Real-life scenario is actived by
            setting the class member 'realcase' to True (Default False), or by given the command argument 'realcase'.
        ...
        # the sub patch of the input image must be located at the same region and size.
        count = 0
        # iteration = 0
        #print("\nIteration : ", iteration, "\n")
        ...
        while True:
            ...
            for i in tqdm(range(len(files))):
                ...
                augmented_images =  utils.augment_image(img, self.augment, self.fn_user_augmentation)
    ...
    def get_mean_and_deviation(self, separate=False):
        """
        Gives the overall mean and deviation values from entire training images.
        :param separate: Boolean. Default False. Each color channel is processed individually if it is True.
        This functionality to be added in the future releases.
        :return: The mean and standard deviation values. Both values are stored also as members of the class
        with the names of 'mean' and 'std', respectively.
        """
    ...
    def visualize_all_layer_outputs(self, img_input, name, plot=False, saveimages=False, change_colorspace= False):
        """"
        ...
        for i in range(1, len(self.model.layers)):
            ...
            par_model = Model(self.model.inputs, lay.output)

            out = par_model(img_input, training=False)

            img_out = out.numpy().squeeze()

            if saveimages:
                if len(img_out.shape) < 3 or len(img_out.shape) ==3 and img_out.shape[-1] <= 3:
                    if change_colorspace and self.colormode == 'YCbCr':
                        ...
                        Image.fromarray(img_out, mode='YCbCr').save(name + '_' + lay.name + '.jpg')

                    else:
                        Image.fromarray(img_out, mode='RGB').save(name + '_' + lay.name + '.jpg')

                #
                # if plot:
                #     plt.imshow(img_out, cmap='gray')
                #     plt.title('Layer {} , feature map {}'.format(lay.name))
                #     plt.tight_layout()
                #     plt.show()
    ...
    def visualize_layer_weights(self, saveimages=True, plot=False, name='', dpi=300):
        """
    ...
    def prepare_dataset(self, imagedir=None, datafile=None):
        ...
        for f in files:
            ...
            for ref_image in utils.augment_image(img, self.augment, self.fn_user_augmentation):
    ...
    def prepare_callbacks(self, fn_user_callbacks = None):
        """
        Builds callback delegates for Keras model, used to do certain task(s) in training process.
         Some built-in callback functions are defined in this method. Users can provide additional callbacks
          either with the additional_callbacks parameter, or with the method named 'callbacks' in the model file,
          when the program is run from command line. The 'callbacks' method is expected to return a list of Keras' callbacks.
          This list is stored in the class as member with the name of 'callbacks'.
        :param fn_user_callbacks: List of user-defined callbacks.
        :return: A list of built-in and user-defined callbaks.
        """
        loss= 'loss'
        ...
        # User can extend callbacks by writing the function named 'callbacks' in
        # the model file. The function is expected to return the callbacks within a list
        # append user-defined callbacks
        # Callbacks obtained from the member function fn_user_callbacks:
        if hasattr(self, "fn_user_callbacks") and self.fn_user_callbacks is not None:
            cbks = self.fn_user_callbacks()

            if not callable(self.fn_user_callbacks):  # check if a valid function is given
                raise Exception("The member method 'fn_user_callbacks' is not a callable object."
                                "This should be a function returning a list of user-defined Keras' callbacks")
            else:

                if cbks is not None and len(cbks) > 0:
                    if isinstance(cbks, list):  # user-defined callbacks are given in a list
                        for cbk in cbks:
                            callbacks.append(cbk)  # append each user-defined callback to the local callback list.
                    else:
                        callbacks.append(cbks)  # only one user-defined callback is available

        # Callbacks obtained from the function given with the parameter fn_usr_callbacks
        # if there is at least one user-defined callback
        if fn_user_callbacks is not None:

            if not callable(self.fn_user_callbacks):  # check if a valid function is given
                raise Exception("The parameter 'fn_user_callbacks' is not a callable object."
                                "This should be a function returning a list of user-defined Keras' callbacks")
            else:
                cbks = fn_user_callbacks() # should be a list of callbacks
                if cbks is not None and len(cbks) > 0:
                    if isinstance(cbks, list): # user-defined callbacks are given in a list
                        for cbk in cbks:
                            callbacks.append(cbk) # append each user-defined callback to the local callback list.
    ...
    def make_member(self, function ):
        """Makes a function or list of functions as members of DeepSR Class.
        :parameter: function: a single function or a list of functions."""

        if function is None:
            print('No any function given!')
            return

        if callable(function): # a single function
            function = [function,]

        for fn in function: # a list of functions
            if fn is not None:
    ...
    def set_settings(self, args=None):
        ...
        self.fn_user_callbacks = None # user-defined callbacks for training
        self.realcase = False # low resolution images to be syntetically generated.
        self.fn_user_metrics= None # function to calculate user-defined metrics
        self.fn_user_augmentation = None # user-defined function for image augmentation
    ...
    def test(self, *keys, **params):
        ...
        dir_layer_outputs = join(self.outputdir, 'layer_outputs')
        dir_layer_weights = join(self.outputdir, 'layer_weights')
        ...
        for path in testpath:
            ...
            for i in tqdm(range(len(test_files))):
                ...
                if self.interp_compare is not None and (len(self.interp_compare) > 0 and '' not in self.interp_compare):
                    ...
                    for method in self.interp_compare:
                        ...
                        res = utils.calc_multi_metrics_of_image(img_ground, im_m,
                                                                border=self.crop_test, channels=self.target_channels,
                                                                metrics=self.metrics, fn_user_metrics=self.fn_user_metrics)
                ...
                for j in range(0, len(weights_list)):
                    ...
                    if saveimages:
                        self.save_image(img_result, fileName + '_scale_' + text_scale  + '.png')

                    if self.layeroutputs:
                        if not exists(dir_layer_outputs):
                            makedirs(dir_layer_outputs)
                        name = join(dir_layer_outputs, file_short_name +  '_scale_') + text_scale + '_' + \
                                self.modelname + '_' + sutun

                        self.visualize_all_layer_outputs(img_input, name, saveimages=self.saveimages, plot=False)

                    if self.layerweights :
                        if not exists(dir_layer_weights):
                            makedirs(dir_layer_weights)

                        name = join(dir_layer_weights, 'scale_') + text_scale + '_' + \
                               self.modelname + '_' + sutun

                        self.visualize_layer_weights(saveimages=self.saveimages, name=name, plot=False, dpi=300)

                    collect() # garbage collector

            # write results in an excel file in weight folder
            excel_file = self.modelname + '_' + test_name + "_scale_" + text_scale + ".xlsx"
