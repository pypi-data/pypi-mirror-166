...
async def get_cetus():
    temp_cetus = f"==================" \
                 f"\n{'白天' if data['cetus']['day'] else '黑夜'}\t丨\t{await get_format_time('%H时%M分%S秒', await get_gm_time(data['cetus']['expiry'] - time.time()))}" \
                 f"\n交替：\t丨\t{await get_format_time('%H时%M分%S秒', await get_local_time(data['cetus']['expiry']))}" \
                 f"\n=================="
    return temp_cetus
...
async def get_earth():
    temp_earth = "==================" \
                 f"\n{'白天' if data['earth']['day'] else '黑夜'}\t丨\t{await get_format_time('%H时%M分%S秒', await get_gm_time(data['earth']['earthDate'] - int(time.time())))}" \
                 f"\n交替\t丨\t{await get_format_time('%H时%M分%S秒', await get_local_time(data['earth']['earthDate']))}" \
                 f"\n=================="
    return temp_earth
...
async def get_solaris():
    ...
    if data['solaris']['state'] == 2:
        ...
        state = '极寒'
    return "==================" \
           f"\n{state}\t丨\t{await get_format_time('%H时%M分%S秒', await get_gm_time(data['solaris']['solarisExpiry'] - int(time.time())))}" \
           f"\n交替\t丨\t{await get_format_time('%H时%M分%S秒', await get_local_time(data['solaris']['solarisExpiry']))}" \
           f"\n=================="
...
async def get_fissures():
    ...
    for fissure in data['fissures']:
        temp_fissures += f"\n{fissure['modifier']}\t丨\t{fissure['missionType']}\t丨\t{fissure['node']}\t丨\t{await get_format_time('%H时%M分%S秒', await get_gm_time(fissure['expiry'] - int(time.time())))}"
...
async def get_voidTrader():
    return "==================" \
           ...
           f"\n地点丨{data['voidTrader']['node']}" \
           f"\n剩余丨{await get_format_time('%H时%M分%S秒', await get_gm_time(data['voidTrader']['expiry'] - int(time.time())))}" \
           f"\n=================="


async def get_sortie():
    temp_sortie = "==================" \
                  f"\n{data['sortie']['boss']}  {await get_format_time('%H时%M分%S秒', await get_gm_time(data['sortie']['expiry'] - int(time.time())))}" \
                  f"\n{data['sortie']['faction']}"
