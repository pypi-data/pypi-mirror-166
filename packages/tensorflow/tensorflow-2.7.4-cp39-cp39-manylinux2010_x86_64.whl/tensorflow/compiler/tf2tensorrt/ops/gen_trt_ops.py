  ...
  if tld.is_eager:
    ...
    try:
      _result = _dispatcher_for_serialize_trt_resource(
          (resource_name, filename, delete_resource, name,), None)
      if _result is not NotImplemented:
        return _result
...
def trt_engine_op(in_tensor, serialized_segment, OutT, workspace_size_bytes, precision_mode, segment_func="", input_shapes=[], max_cached_engines_count=1, max_batch_size=1, calibration_data="", use_calibration=True, segment_funcdef_name="", cached_engine_batches=[], fixed_input_size=True, output_shapes=[], static_engine=True, profile_strategy="", name=None):
  ...
  if tld.is_eager:
    ...
    _result = _dispatcher_for_trt_engine_op(
        (in_tensor, serialized_segment, OutT, workspace_size_bytes,
        precision_mode, segment_func, input_shapes, max_cached_engines_count,
        max_batch_size, calibration_data, use_calibration,
        segment_funcdef_name, cached_engine_batches, fixed_input_size,
        output_shapes, static_engine, profile_strategy, name,), None)
    if _result is not NotImplemented:
      return _result
  # Add nodes to the TensorFlow graph.
  ...
  try:
    _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "TRTEngineOp", in_tensor=in_tensor,
                       ...
                       output_shapes=output_shapes,
                       static_engine=static_engine,
                       profile_strategy=profile_strategy, name=name)
  except (TypeError, ValueError):
    _result = _dispatch.dispatch(
          trt_engine_op, (), dict(in_tensor=in_tensor,
                                  serialized_segment=serialized_segment,
