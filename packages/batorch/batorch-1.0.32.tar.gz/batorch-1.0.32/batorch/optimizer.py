...
class Optimization:
    ...
    def __init__(self, optimizer, params_or_model, dataset = None, lr = 1e-3, momentum = 0):
        if isinstance(params_or_model, bt.nn.Module):
            ...
            self.dataset = dataset
        self.hyper = dict(
            lr = lr if callable(lr) else lambda _: lr, 
            momentum = momentum if callable(momentum) else lambda _: momentum
        )
