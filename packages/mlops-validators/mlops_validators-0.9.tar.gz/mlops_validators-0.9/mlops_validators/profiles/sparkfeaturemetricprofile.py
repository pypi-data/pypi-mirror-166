...
class SparkFeatureMetricProfile:
    ...
    def __init__(self, validators=[], include_timestamp=False, **kwargs):        
        ...
        if isinstance(include_timestamp, bool):
            if include_timestamp:
                self.timestamp = f.current_timestamp()
            else:
                self._include_timestamp = False            
        elif isinstance(include_timestamp, str):
            self.timestamp = f.to_timestamp(f.lit(include_timestamp))            
        else:
            self.timestamp = include_timestamp        
