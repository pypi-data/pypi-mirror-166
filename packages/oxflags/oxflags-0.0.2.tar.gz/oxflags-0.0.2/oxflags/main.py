...
class Flag:
    ...
    def new(self, **kwargs):
        ...
        if ('default' not in kwargs and type != "bool") and ('default' not in kwargs and required != True):
            print(f"{a.bl}[{a.r}!{a.bl}] {a.r}Error: {a.rst}Default value required for {full}.")
            sys.exit(1)
    
        self.flags.append(self.flag(kwargs))
