...
{  // called by the sound thread
	...
	if (control_head_data_width > n_pixels) {
		...
		buffer = (uint8_t *)malloc(n_pixels * sizeof(uint8_t));
	}
	copy2pixels(pixels, control_head_data_width, fft_avg, fft_size, zoom, deltaf, fft_sample_rate);
	for (i = 0; i < control_head_data_width; i++) {
		d2 = 20.0 * log10(pixels[i]) - scale;
		if (d2 < -200)
			d2 = -200;
		else if (d2 > 0)
			d2 = 0;
		buffer[i] = (uint8_t)(-d2 + 0.5);
	}
	sent = sendto(graph_data_to_control_head_socket, (const char *)buffer, control_head_data_width, 0,
                        (const struct sockaddr *)&AddrControlHeadGraph, sizeof(AddrControlHeadGraph));
	if (sent != control_head_data_width)
		QuiskPrintf("send_remote_sound_socket(), send(): %s\n", strerror(errno));
}
...
{  // called by the sound thread
	...
	if (n_pixels < data_width + 128) {
		n_pixels = data_width + 128;
		if (buffer)
			free(buffer);
		buffer = (uint8_t *)malloc(n_pixels * sizeof(uint8_t));
	}
	count = recv(graph_data_from_remote_radio_socket, (char *)buffer, n_pixels, 0);
	if (count != data_width)
		return 0;
	for (i = 0; i < data_width; i++)
		fft_avg[i] = - (double)buffer[i];
	return data_width;
}
