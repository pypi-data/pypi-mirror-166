...
class AccountPositionDao(BaseDao):
    ...
    replaceSave = UpdateMethod("""
        replace into account_position(id,account_id,shares_code,enable_count)
        values(:id,':accountId',':sharesCode',':enableCount')
    """)
...
class StrategyDao(BaseDao):
    ...
    countTodayUpdate = SelectMethod(
        "select count(1) from strategy where shares_code=':sharesCode' and enable=1 and version='1.2' and date_format(create_time,'%Y-%m-%d')=date_format(now(),'%Y-%m-%d')",
        int)
