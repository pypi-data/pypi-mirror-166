...
def Build_Peak_Dict(core_data, peak_low, len_peak, len_region):
    ...
    label_data = core_data > peak_low / 3
    label = measure.label(label_data)
    regions = measure.regionprops(label)
    # print('Length region:', len(regions))
...
def Rotate(angle, x_center, y_center, coords, data):
    for i in range(np.array(coords)[2].min(), np.array(coords)[2].max()):
...
def Generate(n, xres, yres, zres, nearest_dist, peak_low, peak_high, sigma_one, sigma_two, sigma_three, angle=None):
    peak = []
    peak_value = []
    peak_location = []
    angle_t = []
    ...
    while len(peak_value) < n:
        ...
        rotate_data, region = Rotate(angle_t[i], y_center[i], x_center[i], coords, pd_data)
    ...
    infor_dict['center'] = np.array(center_record)[sorted_id].tolist()
...
if __name__ == '__main__':
    ...
    start = time.time()
    origin_data, infor_dict, clump_region = Generate(n, xres, yres, zres, nearest_dist, peak_low, peak_high, sigma_one,
