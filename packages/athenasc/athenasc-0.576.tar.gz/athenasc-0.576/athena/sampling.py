...
class Sampling:
    ...
    def sample(self, sim_meta=None, res_dir=None, ncells=10000, lambda_ls=0, cache=True, cache_dir=None, pop_fp=None, targeted_transcriptome=None, map_ls=True):    
        ...
        cells_meta, gene_expr = [], []
        grnas = sim_meta.grna.unique()
        ncells_per_grna = math.ceil(ncells / len(grnas))
        
        for grna in grnas:
            grna_sim_meta = sim_meta.loc[sim_meta.grna == grna].copy()
            grna_cells_meta = sim_cells_meta.loc[sim_cells_meta.grna == grna].copy()
            
            grna_cells_meta, expr = self.sampling_cells(grna_sim_meta, grna_cells_meta, ncells_per_grna, res_dir, targeted_transcriptome)
            expr, lib_sizes = self.sampling_molecules(expr, map_ls, lambda_ls, pop_fp)
            grna_cells_meta = self.clean_cells_metadata(grna_cells_meta, lib_sizes)
            
            gene_expr.append(expr)
            cells_meta.append(grna_cells_meta)
        
        gene_expr = pd.concat(gene_expr)
        cells_meta = pd.concat(cells_meta)
    ...
    def sampling_cells(self, sim_grna_meta, cell_grna_meta, ncells_per_grna, sim_dir, targeted_transcriptome):
        sim_fp = os.path.join(sim_dir, 'simulated_counts.csv.gz')
        
        cell_grna_meta = self.sample_cells_per_simulation(sim_grna_meta, cell_grna_meta, ncells_per_grna)
    ...
    def sample_cells_per_simulation(self, sim_meta, cells_meta, ncells_per_grna):
        sampled_cells = []
