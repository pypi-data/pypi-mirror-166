...
def modelar_clasificacion_binaria(strategy="", X_train=None,y_train=None,X_test=None,y_test=None,params=None,max_iter_cls=None, cls_ini_opt=[] ,enable_cls_opt = False, enable_umbral_opt=False,metric='average_precision',num_rounds=10000,early_stop=400,log=0,api="train_api",sesgo=0.15,random_state=42,url=None, cv=False,cv_n_split=None, print_consola=True):
    start = time.time()
    ...
    if cv : 
        ...
        if url is None:
            ...
            args_cv = { 'X_train':X_train, 'y_train': y_train, 'X_test': X_test, 'y_test': y_test, 'params':params,
                        ...
                        "api":api , "url":url , "cv_n_split" :cv_n_split
                        }      
...
def generate_cv_result(X_train=None,y_train=None,X_test=None,y_test=None,api=None, num_rounds = None , metric=None, 
                       params =None ,  url=None, cv_n_split=None, random_state=42):
...
def kpis_cross_validation():
    ...
    for i in range(5):
       ...
       df_cv_fold = pd.read_csv(f"{path_cv}/y_train_cv_{i}.csv", index_col=False)
       ...
       df_cv_fold = pd.read_csv(f"{path_cv}/y_test_cv_{i}.csv", index_col=False)
...
def roc_curva_cross_validation(list_path=[], n_splits=5,url_out=None):
    ...
    if url_out is None:
        ...
        plt.savefig(url_out+'roc_curva_cv.png', bbox_inches='tight')
        plt.close()
