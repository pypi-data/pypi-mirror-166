...
class Application(BaseHandler):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        dispatcher = DispatcherMiddleware(self._handlers["/"], self._handlers)
        middleware = BeforeFirstCallMiddleware(dispatcher, self.before_first_call)
    ...
    def test_client(self) -> object:
        ...
        self.flask.test_client_class = MockClient
        self.flask.config["TESTING"] = True
        client = self.flask.test_client()
        client.flasket = weakref.proxy(self)
        return client
