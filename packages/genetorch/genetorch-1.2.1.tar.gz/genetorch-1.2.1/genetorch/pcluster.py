...
pdb = pd.DataFrame(pdb, columns=['atom', 'p', 'ele', 'aa', 'chain', 'num', 'x', 'y', 'z', 'occupation', 'temp', 'ele2'])
...
def cluster(model_path,result,gene):
    ...
    model = pd.DataFrame(model,
                       columns=['atom', 'p', 'ele', 'aa', 'chain', 'num', 'x', 'y', 'z', 'occupation', 'temp', 'ele2'])
...
if __name__ == "__main__":
    tbb4 = gt.reader.readfile(r"C:\Users\YOUNG\Desktop\tbb-4")
    gt.finder.filter(tbb4, 4)
    aa_lst = []
    num_lst = get_num(tbb4.result,'tbb-4')
    for i in num_lst:
        aa_lst.append(AminoAcid(pdb,i))
    matrix = mat(aa_lst)
    cluster_plot(matrix)
# 从result行中提取数字
# for i in l:
