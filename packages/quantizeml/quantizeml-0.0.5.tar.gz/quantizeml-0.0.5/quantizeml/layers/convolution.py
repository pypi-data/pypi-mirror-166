...
@tf.keras.utils.register_keras_serializable()
class QuantizedConv2D(Calibrable, PaddedConv2D):
    ...
    def __init__(self, *args, padding_value=None, quant_config={}, **kwargs):
        # padding_value is handled differently in PaddedConv2D and QuantizedConv2D so it is not
        # passed to the super() call.
        super().__init__(*args, padding_value=None, **kwargs)
    ...
    def call(self, inputs, training=None):
        ...
        if self._padding_value is not None:
            ...
            # Note that we use the custom padding even when padding value is 0.
            inputs_padded = self._apply_padding(inputs.values,
                                                list(self.strides),
