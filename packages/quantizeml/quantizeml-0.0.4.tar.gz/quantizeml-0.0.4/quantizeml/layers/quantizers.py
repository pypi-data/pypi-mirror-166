    ...
    def call(self, inputs, training=None):
        ...
        if isinstance(inputs, QTensor):
            if inputs.value_bits <= self.value_bits:
                raise ValueError(f"Quantizing a {inputs.value_bits}-bit QTensor to {self.value_bits}-bit"
                    " is pointless: use a promotion instead.")
        if self.calibration:
            if isinstance(inputs, QTensor):
...
class FixedPointQuantizer(Quantizer):
    ...
    def quantize(self, x):
        ...
        if isinstance(x, FixedPoint):
            return x.downscale(frac_bits, self.value_bits)
