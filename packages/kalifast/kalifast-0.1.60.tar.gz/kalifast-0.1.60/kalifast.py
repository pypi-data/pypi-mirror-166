...
#Helps to delete positions that are within a margin of Error, avoids multiple identical clicks
    ...
    def getFirstWordPosition(self, word=False, ref=CENTER, at=[0,0], caseSensitive=False, colorFilter=False, minConfidence=0, zone=False, img=False, onError=False, tesseract_exe='C:\\Program Files\\Tesseract-OCR\\tesseract.exe', readOnlyTransp=True, border_size=20, tesseract_config=False) :
        wordZone = self.getWordZone(word=word, caseSensitive=caseSensitive, colorFilter=colorFilter, minConfidence=minConfidence, zone=zone, img=img, onError=onError, tesseract_exe=tesseract_exe, readOnlyTransp=readOnlyTransp, border_size=border_size, tesseract_config=tesseract_config)
    ...
    def getAllWordPosition(self, word=False, ref=CENTER, at=[0,0], caseSensitive=False, colorFilter=False, minConfidence=0, zone=False, img=False, onError=False, tesseract_exe='C:\\Program Files\\Tesseract-OCR\\tesseract.exe', readOnlyTransp=True, border_size=20, tesseract_config=False) :
        wordZone = self.getWordZone(word=word, caseSensitive=caseSensitive, colorFilter=colorFilter, minConfidence=minConfidence, zone=zone, img=img, onError=onError, tesseract_exe=tesseract_exe, readOnlyTransp=readOnlyTransp, border_size=border_size, tesseract_config=tesseract_config)
        result_positions = []
    ...
    def getWordZone(self, word=False, caseSensitive=False, colorFilter=False, minConfidence=0, zone=False, img=False, onError=False, tesseract_exe='C:\\Program Files\\Tesseract-OCR\\tesseract.exe', readOnlyTransp=True, border_size=20, tesseract_config=False) :
        if(isinstance(word, str) == False) :
            if onError != False :
                return onError
            self.writeLog(text="getWordZone(word="+str(word)+", caseSensitive="+str(caseSensitive)+", colorFilter="+str(colorFilter)+", minConfidence="+str(minConfidence)+", zone="+str(zone)+", onError=\""+str(onError)+"\", tesseract_exe=\""+str(tesseract_exe)+"\")", type=self.ERROR, comment="tesseract.exe not found !")
        ...
        if (self._checkTypeZone(zone) == False and self._checkTypeZones(zone) == False and zone != self.ENTIRE_SCREEN and zone != False) and (img != False):
             if onError != False :
                 return onError
             self.writeLog(text="getWordZone(word="+str(word)+", caseSensitive="+str(caseSensitive)+", colorFilter="+str(colorFilter)+", minConfidence="+str(minConfidence)+", zone="+str(zone)+", onError=\""+str(onError)+"\", tesseract_exe=\""+str(tesseract_exe)+"\")", type=self.ERROR, comment=self.ERROR_PARAMETERS + "")
             raise Exception(self.ERROR_PARAMETERS)
        ...
        if self._checkTypeZones(zone) == True :
            for z in zone :                
                ...
                try :
                    ...
                    #rgbcv2_ = cv2.cvtColor(imagecv2, cv2.COLOR_BGR2GRAY)
                    #print(rgbcv2_)
                    ...
                    #rgb_filter = np.array([2,72,141])
                    #rgb_filter = np.array([89,85,78])
                   #rgb_filter = np.array([79,72,62])
                    #margin = 10
                    #rgb_filter = np.array([69:89,62:82,52:72])
                    #rgb_filter = np.array([114,109,101])

                    

                    #res = imagecv2 != rgb_filter


                    #res[np.all(res != [141,72,2], axis=2)] = [0,0,0]
                    #condition = logical_or(logical_and(imagecv2[0] >= rgb_filter[0]-10), logical_and(imagecv2[0] <= rgb_filter[0]+10))
                    #print(condition)

                    #res = np.where(np.all(imagecv2 == rgb_filter, axis=2), 0,255)

                    #res = res.astype(np.uint8)
                    #print(res[30][30])
                    #print(imagecv2[30][30])

                    #cv2.imshow("filter", res)
                    #cv2.imshow("not filter", imagecv2)
                    #cv2.waitKey(0)
                    ...
                    if onError != False :
                        return onError
                    self.writeLog(text="getWordZone(word="+str(word)+", caseSensitive="+str(caseSensitive)+", colorFilter="+str(colorFilter)+", minConfidence="+str(minConfidence)+", zone="+str(zone)+", onError=\""+str(onError)+"\", tesseract_exe=\""+str(tesseract_exe)+"\")", type=self.ERROR, comment=str(e))
                    raise Exception(str(e))
        ...
        if self.VERBOSE_MODE == True :
            self.writeLog(text="getWordZone(word="+str(word)+", caseSensitive="+str(caseSensitive)+", colorFilter="+str(colorFilter)+", minConfidence="+str(minConfidence)+", zone="+str(zone)+", onError=\""+str(onError)+"\", tesseract_exe=\""+str(tesseract_exe)+"\")", type=self.VERBOSE)
