			...
			if formset.is_valid():
				...
				if form_crm.is_valid():
					...
					if llamada:
						...
						doc_registro= Registry.objects.get( nombre = 'SIC_Pedidos_Doc_Registro').get_value()
						form.instance.tipo=doc_registro
					if preprogramado:
...
def actualiza_base_datos(request):
	...
	if not Registry.objects.filter( nombre = 'SIC_Pedidos_Doc_Registro' ).exists():
			Registry.objects.create(
				nombre = 'SIC_Pedidos_Doc_Registro',
				tipo = 'V',
				padre = padre,
				valor= '',
			)
