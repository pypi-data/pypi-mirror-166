	...
	def prot_and_hmm_to_besthits(self):
		...
		if self.crystalize:
			...
			if self.do_compress:
				...
				crystal_writer = gzip.open(crystal_path, "wb")
...
def miga_preproc_opts():
	...
	parser.add_argument('--verbose',        dest = 'verbose', action='store_true', help = 'Print minor updates to console. Major updates are printed regardless.')
	parser.add_argument('--compress', dest = "do_comp", action = 'store_true', help = 'Gzip compress generated proteins, HMMs. Off by default.')
	parser.add_argument('--crystalize', dest = "crystalize", action = 'store_true', help = 'Create outputs containing fully preprocessed FastAAI inputs.')
...
def miga_preproc(genomes, proteins, hmms, output, threads, verbose, do_compress, make_crystals):
	success = True
	
	imported_files = fastaai_file_importer(genomes = genomes, proteins = proteins, hmms = hmms, output = output, compress = do_compress, crystalize = make_crystals)
	imported_files.determine_inputs()
	...
	if imported_files.status == "genome":
		miga_dirs(output, "predicted_proteins")
		miga_dirs(output, "hmms")
		miga_dirs(output, "crystals")
...
def main():
	...
	if selection == "version":
		sys.exit("FastAAI version=0.1.16")
