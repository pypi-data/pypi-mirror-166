...
class Dataset_KeyValue(Storage):
    ...
    def nested_dict(self, Dict, _last_group = None, _key = None, _nested_group = None):# if last group was data then -> last_group=data -> data.
...
#### run
            ...
            if isinstance(Dict[key], (list, tuple, dict)) and _last_group == None and _key == None:            # first dict name is the name of the run (e.g. run_01)  
                key = Run(key)
...
#### param
            # TODO 
            if isinstance(Dict[key], (list, tuple, dict)) and key=="parameters" and _last_group != "pipeline":            # check if param
                return self.nested_dict(Dict[key], "parameters")

            elif isinstance(Dict[key], (list, tuple, dict)) and _last_group == "parameters":      # create param class  
...
#### dataset
            # TODO
            if isinstance(Dict[key], (list, tuple, dict)) and key =="data" and _last_group != "pipeline":                    # check if data
                return self.nested_dict(Dict[key], "data")

            elif isinstance(Dict[key], (list, tuple, dict)) and _last_group == "data":              # create data class
            ...
            elif not isinstance(Dict[key], (list, tuple, dict)) and _last_group == "data":         # add the meta data 
                _key.attrs[key] = Dict[key]
            # TODO    
            elif _last_group == "data":                     # and not isinstance?                   # add data to dataset
...
#### model
            ...
            if isinstance(Dict[key], (list, tuple, dict)) and key == "model" and _last_group != "pipeline":               # check if model
...
#### instrument
            # TODO
            if isinstance(Dict[key], (list, tuple, dict)) and key == "instrument" and _last_group != "pipeline":               # check if instrument
                return self.nested_dict(Dict[key], "instrument")

    # inside the instrument dict

            elif isinstance(Dict[key], (list, tuple, dict)) and _last_group == "instrument":              
                key = Instrument(key)                                                                   # create instrument class
                self.nested_dict(Dict, "data", _key = key)                                              # check for meta data
                key.add([self.nested_dict(Dict[key], _last_group = "")])                                # add instruments
                return key
...
#### pipeline
            # TODO
            if isinstance(Dict[key], (list, tuple, dict)) and key == "pipeline":               # check if pipeline
                return self.nested_dict(Dict[key], "pipeline", _nested_group = None)

    # inside the pipeline dict
    
        # check for nested groups 
            elif isinstance(Dict[key], (list, tuple, dict)) and _last_group == "pipeline" and key!="instrument" and key!= "data" and key!= "model" and key!= "parameters":  
                if _nested_group == None:
                    return self.nested_dict(Dict[key],"pipeline", _nested_group = key)
                elif _nested_group != None:
                    return self.nested_dict(Dict[key],"pipeline", _nested_group = _nested_group + "/" + key)    # measured/capa1/raw

        # create the pipeline with nested name
                
            elif isinstance(Dict[key], (list, tuple, dict)) and _last_group == "pipeline":              
                _nested_group = Pipeline(_nested_group)                                                         # create instrument class
                self.nested_dict(Dict, "data", _key = _nested_group)                                                      # check for meta data
                _nested_group.add([self.nested_dict(Dict, _last_group = "")])  # last group has to be != none   # add instruments
