...
class Storage:
    ...
    def store_HDF5(self, root=None):
        ...
        if self.is_dataset:
            ...
            if 'kkn_DATASET_SUBCLASS' in self.attrs.keys() and self.attrs['kkn_DATASET_SUBCLASS'] == 'VIDEO':
    ...
    def store_json(self, root=None):
        ...
        if self.is_dataset:
            ...
            condition1 = 'kkn_DATASET_SUBCLASS' in self.attrs.keys() and self.attrs['kkn_DATASET_SUBCLASS'] == 'IMAGE'
            condition2 = 'kkn_DATASET_SUBCLASS' in self.attrs.keys() and self.attrs['kkn_DATASET_SUBCLASS'] == 'VIDEO'
            
            # encode the data attribute when this is an instance of dataset_image of dataset_video
            if condition1 or condition2:
                root['data'] = self.encode(self.data)
            else:
                if not (isinstance(self.data, list) or isinstance(self.data, int) or isinstance(self.data, float)):
