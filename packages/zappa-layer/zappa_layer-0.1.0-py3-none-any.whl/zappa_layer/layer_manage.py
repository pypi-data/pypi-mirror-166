...
@click.option("--region", "-r", default="ap-northeast-2", help="AWS region")
@click.pass_context
def allinone(context, profile, layer_name, region):
    context.invoke(installpkgs)
    context.invoke(makelayer)
    context.invoke(publishlayer, profile=profile, layer_name=layer_name, region=region)
    context.invoke(updatezappalayer, profile=profile, layer_name=layer_name, region=region)
