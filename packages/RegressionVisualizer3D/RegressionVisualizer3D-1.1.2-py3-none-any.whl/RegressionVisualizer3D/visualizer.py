...
def model_plot(model_type="linear_regression",
    ...
    if verbose == True:
        ...
        print("""
The lefthand graph below shows some population data and the population
regression surface for that data. (E.g. if a regression model were fit
to all of the population data).
""")
        print("""
The righthand graph shows data from a sample which was randomly drawn
from the population data. It also shows the sample regression surface 
(e.g. from a regression model fit to the sample data).
""")

        # if the statsmodels table should be shown, let the user know where it will be displayed
        if show_statsmodels == True:
            print("""
Beneath the graphs is the regression table (with slopes and 
p-values etc.) from the sample data. The true population regression
equation used to generate the data is also shown.
""")
