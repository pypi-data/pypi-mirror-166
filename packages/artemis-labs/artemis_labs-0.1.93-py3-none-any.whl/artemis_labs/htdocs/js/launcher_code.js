(function($) {
    ...
    function createInputTextCard(inputCardTitle, inputCardLineStart, inputCardLineEnd) {
        ...
        if (previousCard) {
            ...
            // Get previous card preview
            let previousCardPreview = previousCard.querySelector('.card-body-preview');

            // Add input
            previousCardPreview.appendChild(inputElement);

            // Add submit button if not in archive mode
            if (!archiveMode) {
                previousCardPreview.appendChild(createInputSubmitButton());
            }
            $(previousCard).attr('line-end', inputCardLineEnd);
            let lineCardHeaderDOM = qs("#" + previousCard.id + " .card-header-element-bold");
            if ($(previousCard).attr('line-start') != $(previousCard).attr('line-end')) {
                $(lineCardHeaderDOM).html($(previousCard).attr('line-start') + " - " + $(previousCard).attr('line-end'));
            } 

            // Group lines
            let lineStart = parseInt($(previousCard).attr('line-start'));

            // Select card
            setTimeout(function() {
                editor.gotoLine(lineStart);
                console.log(previousCard);
                if ($(previousCard).hasClass('card-selected')) {
        ...
        if (newCardName !== '') cardName = newCardName;
        let inputCard = createInputCard(cardName, inputCardLineStart, inputCardLineEnd, inputElement);
        previousCard = inputCard;
        if (archiveMode) {
