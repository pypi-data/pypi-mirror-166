(function($) {
    ...
    function createCardSection(sectionName, sectionIndex) {
    ...
    function createCardComponent(outputLineStart, outputLineEnd, outputName, outputValue, outputComment, outputComponentType) {
        ...
        if (displayMode == 'content') {
            ...
            // Get card title
            let cardName = $(outputCard).find(".card-header-element").text();

            // Check if section exists
            let sectionIndex = -1;
            let cardSections = qsa('.accordion-button');
            for (let i = 0; i < cardSections.length; i++) {
                if (cardSections[i].innerHTML == cardName) {
                    sectionIndex = i;
                    break;
                }
            }
            ...
            if (sectionIndex == -1) {
                ...
                sectionIndex = sectionCounter - 1;
            }

            // Add card to section
            let accordionSection = qs("#card-section-" + sectionIndex + " .component-card-section");
            console.log("#card-section-" + sectionIndex);
            console.log("#card-section-" + sectionIndex + " .component-card-section");
        ...
        if (message.type != 'ping' && message.type != 'heartbeat') {
            ...
            if (message.type == "quit") {
                ...
                if (systemCardFound && !systemCardCollapsed) {
                    ...
                    if (message.value != undefined && message.value.indexOf('__**Run Date**__') != -1) {
    ...
    function switchToContentMode() {
         ...
         // Aggregate sections
         let cardHeaders = qsa(".card-header-element");
         sectionCounter = 0;
         for (let i = 0; i < cardHeaders.length; i++) {
             if (sectionMap[cardHeaders[i].innerText] == undefined) {
                sectionMap[cardHeaders[i].innerText] = sectionCounter;
                sectionMapInverse[sectionCounter] = cardHeaders[i].innerText;
                sectionArray[sectionCounter++] =  [ cardHeaders[i].parentElement.parentElement.parentElement ];
             } else {
                let sectionIndex = sectionMap[cardHeaders[i].innerText];
                sectionArray[sectionIndex].push(cardHeaders[i].parentElement.parentElement.parentElement);
             }
         }
