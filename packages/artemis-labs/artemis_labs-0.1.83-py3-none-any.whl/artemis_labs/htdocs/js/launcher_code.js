    ...
    function createOutputComponentDOM(in_componentType, outputValue, parentElementSelector, startTop=false) {
        ...
        if (componentType == "card") {
            ...
            $(component.getId()).on('contextmenu', function(e) {
                ...
                // Update DOM
                if ($(componentDOM.parentElement).attr('type') == 'table') {
                    $(copyCard.firstChild).css('height', '100%');
                    let numRows = $(copyCard.firstChild).find('tr').length;
                    let maxHeight = 40 * numRows;
                    $(copyCard.firstChild).css('max-height', maxHeight + 'px');
                    console.log(copyCard.firstChild);
                }
