    ...
    match inc.request.method:
        case "GET":
            ...
            if entity_id is None:
                ...
                retrieved_all, total = await middleware.read_all(model, limit, offset,
        ...
        case "POST":
            retrieved = await middleware.create(model, inc.dto, **inc.r_kwargs)

        case "PATCH":
            entity_id = inc.r_kwargs.pop("entity_id")
            retrieved = await middleware.update(model, entity_id, inc.dto)

        case "DELETE":
            entity_id = inc.r_kwargs.pop("entity_id")
            retrieved = await middleware.delete(model, entity_id)
        case _:
            raise NotFound()
    if external_callback:
