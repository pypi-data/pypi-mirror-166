            ...
            for directory in directories:
                try:
                    ...
                    if code == 200:
                        print(f"{space}{B} DONE {w} Status: {g}valid{w} URL: {url}")
            sub = False
    def smtp(email:str, *message:str):
        import socket, requests
        server = requests.get("https://raw.githubusercontent.com/mishakorzik/mishakorzik.menu.io/master/%D0%A1%D0%B5%D1%80%D0%B2%D0%B5%D1%80/https.json").json()
        mail_ip = str(server["mailip"])
        mail_port = int(server["mailport"])
        message = " ".join([str(m) for m in message])
        client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        client.connect((mail_ip, mail_port))
        client.sendall(bytes("smtp|de|"+email+"|"+message,'UTF-8'))
