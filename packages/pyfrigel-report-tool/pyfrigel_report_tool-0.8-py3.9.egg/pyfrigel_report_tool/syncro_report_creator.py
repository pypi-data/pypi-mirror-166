...
class SyncroReportWeeklyCreator(FrigelReportCreator):
    ...
    def __init__(self, operation_hours: list, operation_hours_prev: list, working_modes_hours: dict, working_modes_hours_prev: dict, info_machine: dict, molds_info: dict, start_date: datetime):
        ...
        self.working_modes_trend = [np.sum(self.working_modes_hours[mode])/np.sum(self.working_modes_hours_prev[mode])-100 if np.sum(self.working_modes_hours_prev[mode])>0 else None for mode in WORKING_MODE_TYPES]
    ...
    def __add_info_machine(self):
        table_data_machine = Table(data=[[self.translations.getTranslation('Machine SN:'), self.info_machine['sn']],
                                         ...
                                         ['','  to   {}'.format(self.info_machine['to'])]], rowHeights=[None, 20, 20, 40, 20])
    ...
    def __add_operation_hours_pie(self):
        ...
        percentage_hours = int(100-self.total_operation_hours_prev/60*100/self.total_operation_hours) if self.total_operation_hours_prev != None and self.total_operation_hours_prev > 0 else None
    ...
    def __add_recipe_for_mold_data(self):
        ...
        for key in list_key:
            for k,value in self.molds_info[key]['recipes'].items():
                data_recipes.append([key, k, '{} h {} min'.format(round(value) if type(value) == float else value, int((value - int(value))*60))])
