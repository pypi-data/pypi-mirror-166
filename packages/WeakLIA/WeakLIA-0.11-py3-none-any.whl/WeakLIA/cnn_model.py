...
from WeakLIA.data_processing import process_class, create_training_set
from WeakLIA import optimization
...
class Classifier:
    ...
    Args:
        class1 (ndarray, optional): 2D array of size (n x m), where n is the
        ...
        class2 (ndarray, optional): 1D array containing the corresponing labels. Defaults to None,
            in which case a model can be loaded using the class attributes..
    ...
    """
    def __init__(self, class1=None, class2=None, val_class1=None, val_class2=None, img_num_channels=1, 
    ...
    def create(self):
        ...
        if self.optimize is False:
            ...
            self.model, self.history = pyBIA_model(self.class1, self.class2, img_num_channels=self.img_num_channels, normalize=self.normalize,
                min_pixel=self.min_pixel, max_pixel=self.max_pixel, val_class1=self.val_class1, val_class2=self.val_class2, epochs=self.epochs)
            return      

        self.best_params, self.optimization_results = optimization.hyper_opt(self.class1, self.class2, metric=self.metric, n_iter=self.n_iter, 
            return_study=True, img_num_channels=self.img_num_channels, normalize=self.normalize, min_pixel=self.min_pixel, 
            max_pixel=self.max_pixel, val_class1=self.val_class1, val_class2=self.val_Class2, train_epochs=self.train_epochs, patience=self.patience, limit_search=self.limit_search,
        ...
        if self.limit_search:
            self.model, self.history = pyBIA_model(self.class1, self.class2, img_num_channels=self.img_num_channels, normalize=self.normalize,
                min_pixel=min_pix, max_pixel=max_pix, val_class1=self.val_class1, val_class2=self.val_class2, epochs=self.epochs, batch_size=self.best_params['batch_size'],
    ...
    def predict(self, data, target='LENS'):
        """
        ...
        Args:
            ...
            target (str): The name of the target class, assuming binary classification in 
                which there is an 'OTHER' class. Defaults to 'LENS'. 
        Returns:
            The class prediction(s), either 'LENS' or 'OTHER'.
        """
...
def pyBIA_model(class1, class2, img_num_channels=1, normalize=True, 
        min_pixel=0, max_pixel=100, val_class1=None, val_class2=None, epochs=100, 
        ...
        Args:
            class1 (ndarray): 3D array containing more than one image of diffuse objects.
            class2 (ndarray): 3D array containing more than one image of non-diffuse objects.
            ...
            max_pixel (int, optional): The maximum pixel count, defaults to 3000. 
                Pixels with counts above this threshold will be set to this limit.
            val_class1 (array, optional): 3D matrix containing the 2D arrays (images)
                to be used for validation, for the first class. Defaults to None.
            val_class2 (array, optional): 3D matrix containing the 2D arrays (images)
                to be used for validationm, for the first class. Defaults to None.
        ...
        if len(class1.shape) != len(class2.shape):
            raise ValueError("Shape of class1 and class2 data must be the same.")
        if batch_size < 16:
        ...
        if val_class1 is not None:
            val_X1, val_Y1 = process_class(val_class1, label=0, min_pixel=min_pixel, max_pixel=max_pixel, img_num_channels=img_num_channels)
            if val_class2 is None:
                val_X, val_Y = val_X1, val_Y1
            else:
                val_X2, val_Y2 = process_class(val_class2, label=1, min_pixel=min_pixel, max_pixel=max_pixel, img_num_channels=img_num_channels)
        ...
        img_width = class1[0].shape[0]
        img_height = class1[0].shape[1]
        #decay = lr / epochs

        ix = np.random.permutation(len(class1))
        class1 = class1[ix]

        ix = np.random.permutation(len(class2))
        class2 = class2[ix]

        X_train, Y_train = create_training_set(class1, class2, normalize=normalize, min_pixel=min_pixel, max_pixel=max_pixel, img_num_channels=img_num_channels)
